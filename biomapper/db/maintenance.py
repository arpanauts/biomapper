...
class DatabaseMaintenance:
    ...
    def restore_backup(self, backup_path: str) -> bool:
        ...
        if not current_backup:
            logger.warning("Could not create backup of current database before restoration")
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db_manager.create_session() as session:
                ...
                stats["tables"]["entity_mappings"] = session.query(EntityMapping).count()
                stats["tables"]["cache_stats"] = session.query(CacheStats).count()
                stats["tables"]["transitive_jobs"] = session.query(TransitiveJobLog).count()
                
                # Count derived mappings
                derived_count = session.query(EntityMapping).filter(
                    EntityMapping.is_derived == True
                ).count()
                
                stats["tables"]["derived_mappings"] = derived_count
                
                # Get latest cache stats
                latest_stats = session.query(CacheStats).order_by(
                    CacheStats.date.desc()
                ).first()
                ...
                if latest_stats:
                    stats["cache_stats"] = {
                        ...
                        "hit_ratio": round(latest_stats.hit_ratio, 2) if latest_stats.hit_ratio else 0,
    ...
    def delete_expired_mappings(self) -> int:
        ...
        try:
            ...
            with self.db_manager.create_session() as session:
                expired = session.query(EntityMapping).filter(
...
def run_maintenance(data_dir: Optional[str] = None, vacuum: bool = True, 
                   backup: bool = True, analyze: bool = True) -> Dict[str, Any]:
