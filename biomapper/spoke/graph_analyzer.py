...
class ArangoDBGraphAnalyzer(KnowledgeGraphAnalyzer):
    ...
    async def discover_node_types(self) -> Dict[str, NodeTypeMetadata]:
        ...
        collections = await asyncio.to_thread(
            lambda: [c['name'] for c in self._db.collections() if not c['name'].startswith('_')]
        )
        ...
        if 'Nodes' in collections:
                    ...
                    for sample in samples:
                        for key, value in sample.items():
                            if key not in ('_id', '_key', '_rev'):
                                ...
                                if isinstance(value, (str, int, float, bool)) and value not in sample_values[key]:
                                ...
                                if isinstance(value, dict):
                                    for subkey, subvalue in value.items():
                                        ...
                                        if isinstance(subvalue, (str, int, float, bool)) and subvalue not in sample_values[prop_path]:
            ...
            for collection in collections:
                collection_data = await asyncio.to_thread(lambda: self._db.collection(collection))
                collection_info = await asyncio.to_thread(lambda: collection_data.properties())
                ...
                if collection_info['type'] == 2:  # Document collection
                    ...
                    samples = await asyncio.to_thread(
                        lambda: [doc for doc in collection_data.all(limit=self.sample_size)]
                    )
            ...
            for result in results:
                ...
                if samples:
                    ...
                    for sample in samples:
                        ...
                        # Analyze properties
                        if 'properties' in sample and isinstance(sample['properties'], dict):
                            for key, value in sample['properties'].items():
                                properties[key] = type(value).__name__
                    
                    relationship_types[relationship_type].properties = properties
                    relationship_types[relationship_type].source_node_types = source_node_types
                    relationship_types[relationship_type].target_node_types = target_node_types
                    relationship_types[relationship_type].sample_sources = sample_sources
                    relationship_types[relationship_type].sample_targets = sample_targets
        else:
                ...
                if collection_info['type'] == 3:  # Edge collection
                    ...
                    if samples:
                        ...
                        for sample in samples:
                            ...
                            source_collection = source_id.split('/')[0] if '/' in source_id else None
                            target_collection = target_id.split('/')[0] if '/' in target_id else None
                        ...
                        relationship_types[collection].source_node_types = source_node_types
                        relationship_types[collection].target_node_types = target_node_types
    ...
    async def identify_ontology_fields(self) -> Dict[str, List[Tuple[str, IdentifierConfidence]]]:
            ...
            for field_name, samples in metadata.sample_values.items():
                if any(isinstance(sample, str) for sample in samples):
                    ...
                    if confidence != IdentifierConfidence.LOW:
                        ...
                        existing = next((c for f, c in field_confidences if f == field_name), None)
    ...
    async def sample_node_data(self, node_type: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if 'Nodes' in collections:
            ...
            # Standard ArangoDB database where each node type is its own collection
            collection_data = await asyncio.to_thread(lambda: self._db.collection(node_type))
            samples = await asyncio.to_thread(
    ...
    async def sample_relationship_data(self, relationship_type: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if 'Edges' in collections:
            ...
            collection_data = await asyncio.to_thread(lambda: self._db.collection(relationship_type))
            samples = await asyncio.to_thread(
                lambda: [doc for doc in collection_data.all(limit=limit)]
            )
    ...
    def _assess_field_name_confidence(self, field_name: str) -> IdentifierConfidence:
        ...
        high_confidence_patterns = [
            r'id$', r'^id_', 'identifier', 'accession', 'chebi', 'hmdb', 
            'pubchem', 'inchikey', 'inchi', 'smiles', 'kegg', 'uniprot', 
            'ensembl', 'gene_symbol', 'symbol', 'mondo', 'doid', 'mesh',
            'drugbank', 'cas', 'unii', 'rxnorm', 'ndc'
        ]
        
        medium_confidence_patterns = [
            'code', 'reference', 'external', 'database', 'ontology'
        ]
    ...
    def _assess_sample_values_confidence(self, samples: List[Any]) -> IdentifierConfidence:
