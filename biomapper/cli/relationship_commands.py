...
def list_relationships():
    ...
    try:
        ...
        for row in rows:
            ...
            """, {"relationship_id": row[0]}).fetchall()
...
def create_relationship(name, description, source, target):
    ...
    try:
        ...
        """, {"endpoint_id": source}).fetchone()
        
        target_endpoint = session.execute("""
            SELECT name FROM endpoints WHERE endpoint_id = :endpoint_id
        """, {"endpoint_id": target}).fetchone()
...
def discover_relationship_paths(relationship_id, force):
    ...
    async def run():
        ...
        try:
            ...
            click.echo(f"Discovered {len(paths)} mapping paths for relationship {relationship_id}")
            for i, path in enumerate(paths):
                source_ont = path["path"]["source_type"]
                target_ont = path["path"]["target_type"]
                score = path["score"]
                click.echo(f"Path {i+1}: {source_ont} → {target_ont} (score: {score:.2f})")
                
                # Show path steps
                steps = path["path"].get("path_steps", [])
                for j, step in enumerate(steps):
                    click.echo(f"  Step {j+1}: {step.get('source_type', step.get('from_type'))} → {step.get('target_type', step.get('to_type'))}")
