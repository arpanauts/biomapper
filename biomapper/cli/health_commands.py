...
from biomapper.db.session import get_db_session
...
def health_check(endpoint, ontology):
    ...
    result = asyncio.run(monitor.run_health_check(
        ...
        ontology_types=ontology if ontology else None
    ))
    ...
    click.echo(f"  Success: {result['success_count']} ({result['success_count']/result['configs_checked']*100:.1f}%)")
    click.echo(f"  Failures: {result['failure_count']} ({result['failure_count']/result['configs_checked']*100:.1f}%)")
    ...
    for endpoint in result.get("endpoints", []):
        ...
        click.echo(f"  {status_symbol} {endpoint['name']} (ID: {endpoint['endpoint_id']})")
        
        if endpoint.get("configs_checked", 0) > 0:
            success_rate = endpoint.get("success_count", 0) / endpoint["configs_checked"]
            click.echo(f"    Status: {endpoint.get('status', 'unknown')} ({success_rate*100:.1f}% success)")
            click.echo(f"    Configs: {endpoint['configs_checked']} checked, {endpoint['success_count']} success, {endpoint['failure_count']} failure")
            
            # List failing configs
            failed = [c for c in endpoint.get("configs", []) if not c.get("success", False)]
            if failed:
                click.echo("\n    Failed configurations:")
                for config in failed:
                    click.echo(f"      {config['ontology_type']}:{config['property_name']}")
...
@click.option("--format", help="Output format", type=click.Choice(['json', 'text', 'html']), default='text')
...
@click.option("--json", "json_output", help="Output as JSON", is_flag=True)
def health_analyze(endpoint, json_output):
    """Analyze and suggest improvements for endpoints."""
    click.echo("Analyzing endpoint configurations...")
    ...
    if json_output:
        ...
        for i, suggestion in enumerate(suggestions, 1):
            ...
            click.echo("\n   Suggested pattern:")
            click.echo(f"   {json.dumps(suggestion['suggested_pattern'], indent=2)}")
    ...
    try:
        ...
        for ep in result.get("endpoints", []):
            if ep["endpoint_id"] == endpoint:
                for config in ep.get("configs", []):
                    if (config["ontology_type"] == ontology and 
                        config["property_name"] == property):
                        test_result = config
                        break
        ...
        if test_result["success"]:
            ...
            if test_result.get("sample_values"):
                click.echo(f"  Sample values: {', '.join(str(v) for v in test_result['sample_values'])}")
        else:
            click.echo(f"  Error: {test_result.get('error_message', 'Unknown error')}")
