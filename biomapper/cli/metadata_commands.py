...
@click.option("--db-path", help="Path to SQLite database", default=None)
@click.option("--config-file", required=True, help="Path to resource configuration file")
...
def list_resources(db_path, format):
    ...
    if format == 'json':
        ...
        click.echo(f"{'Name':<20} {'Type':<10} {'Priority':<10} {'Success Rate':<15} {'Avg Time (ms)':<15}")
        click.echo("-" * 70)
...
def show_statistics(db_path, resource):
    ...
    click.echo("\nPerformance Metrics:\n")
    click.echo(f"{'Resource':<20} {'Operation':<10} {'Source':<10} {'Target':<10} "
