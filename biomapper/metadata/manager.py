...
from biomapper.db.models_metadata import (
    ResourceMetadata, OntologyCoverage, PerformanceMetrics, OperationLog,
    ResourceType, SupportLevel, OperationType, OperationStatus
)
...
class ResourceMetadataManager:
    ...
    def register_resource(
        ...
        with self._session_scope() as session:
            ...
            resource = session.query(ResourceMetadata).filter_by(
    ...
    def register_ontology_coverage(
        ...
        with self._session_scope() as session:
            ...
            coverage = session.query(OntologyCoverage).filter_by(
    ...
    def _update_performance_metrics(
        ...
        """
        # Get metrics
        metrics = session.query(PerformanceMetrics).filter_by(
    ...
    def get_preferred_resource_order(
        ...
        with self._session_scope() as session:
            ...
            resources = session.query(ResourceMetadata).filter_by(
                is_active=True
            ).all()
            ...
            for resource in resources:
                source_support = session.query(OntologyCoverage).filter_by(
                    resource_id=resource.id,
                    ontology_type=source_type,
                    support_level=SupportLevel.NONE,
                ).first() is None
                
                target_support = session.query(OntologyCoverage).filter_by(
                    resource_id=resource.id,
            ...
            for resource in supported_resources:
                ...
                if metrics:
                    ...
                    if min_success_rate is not None and metrics.success_rate < min_success_rate:
                        continue
    ...
    def get_performance_metrics(
        ...
        with self._session_scope() as session:
            ...
            if operation_type:
                query = query.filter(PerformanceMetrics.operation_type == operation_type)
