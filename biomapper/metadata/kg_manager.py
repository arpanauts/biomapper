...
from biomapper.metadata.models import (
    ResourceRegistration, ResourceCapability, ResourcePerformanceMetrics, ResourceType
)
...
class KnowledgeGraphManager:
    ...
    def __init__(self):
        ...
        self._capabilities_map: Dict[str, List[str]] = {}  # Capability name to resource names
    ...
    def register_resource(self, resource: ResourceRegistration) -> None:
        ...
        logger.info(f"Registered knowledge graph resource '{resource.name}' with {len(resource.capabilities)} capabilities")
    ...
    def get_resources_for_capability(self, capability_name: str) -> List[ResourceRegistration]:
    ...
    def best_resource_for_capability(self, capability_name: str) -> Optional[ResourceRegistration]:
        ...
        sorted_resources = sorted(
            ...
            key=lambda r: (
                ...
                if r.get_capability(capability_name) else 0.0
            ),
            reverse=True
        )
    ...
    def update_metrics(
        ...
        if metrics.total_operations > 0:
            metrics.success_rate = 1.0 - (metrics.failed_operations / metrics.total_operations)
        ...
        if latency_ms is not None:
            ...
            if latency_ms > metrics.p95_latency_ms:
                metrics.p95_latency_ms = latency_ms * 0.05 + metrics.p95_latency_ms * 0.95
            ...
            if latency_ms is not None:
                ...
                if cap_metrics['total_operations'] > 1:
                    cap_metrics['avg_latency_ms'] = (
                        (cap_metrics['avg_latency_ms'] * (cap_metrics['total_operations'] - 1) + latency_ms)
                ...
                if latency_ms > cap_metrics['p95_latency_ms']:
                    cap_metrics['p95_latency_ms'] = latency_ms * 0.05 + cap_metrics['p95_latency_ms'] * 0.95
            ...
            if cost is not None:
                ...
                if cap_metrics['total_operations'] > 1:
                    cap_metrics['avg_cost'] = (
                        (cap_metrics['avg_cost'] * (cap_metrics['total_operations'] - 1) + cost)
                        / cap_metrics['total_operations']
                    )
                else:
