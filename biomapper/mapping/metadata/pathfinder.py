...
class RelationshipPathFinder:
    ...
    async def get_relationship_by_id(self, relationship_id: int) -> Optional[Dict]:
        ...
        result = await session.execute(
            text(query), {"relationship_id": relationship_id}
        )
    ...
    async def get_endpoint_details(self, endpoint_id: int) -> Optional[Dict]:
        ...
        if hasattr(endpoint, "connection_info") and endpoint.connection_info:
            try:
                ...
                logger.warning(
                    f"Failed to decode connection_info for endpoint {endpoint.name}"
                )
    ...
    async def _is_endpoint_type(self, endpoint_id: int, endpoint_type: str) -> bool:
        ...
        """
        result = await session.execute(
            text(query), {"endpoint_id": endpoint_id, "endpoint_type": endpoint_type}
        )
        count = result.scalar()
        return count > 0
    ...
    async def get_existing_mapping_paths(
        ...
        """
        result = await session.execute(
            text(query), {"source_type": source_type, "target_type": target_type}
        )
        ...
        for row in result.fetchall():
            ...
            try:
                ...
                logger.warning(
                    f"Failed to decode path_steps for mapping path ID {row.id}"
                )
    ...
    async def discover_relationship_paths(
        self, relationship_id: int, force_rediscover: bool = False
    ) -> List[Dict]:
        ...
        logger.info(
            f"Source ontologies for {source_endpoint['name']}: {source_ontologies}"
        )
        logger.info(
            f"Target ontologies for {target_endpoint['name']}: {target_ontologies}"
        )
        ...
        if not force_rediscover:
            ...
            if existing_paths:
                logger.info(
                    f"Found {len(existing_paths)} existing mapping paths for relationship {relationship_id}"
                )
                return [
                    {
                        "id": path.id,
        ...
        for source_ontology in source_ontologies:
            for target_ontology in target_ontologies:
                ...
                mapping_paths = await self.get_existing_mapping_paths(
                    source_ontology, target_ontology
                )

                if not mapping_paths:
                    # If no existing path, create a basic one (for demonstration purposes)
                    if source_ontology.lower() in [
                        "hmdb",
                        "pubchem",
                    ] and target_ontology.lower() in ["chebi", "pubchem"]:
                        path_id = await self._create_sample_mapping_path(
                            source_ontology, target_ontology
                        )
                        if path_id:
                    ...
                    if not mapping_paths:
                        logger.info(
                            f"No mapping path found for {source_ontology} -> {target_ontology}"
                        )
                        continue
                ...
                try:
                    ...
                    if existing:
                        ...
                        if (
                            existing.ontology_path_id != best_path["id"]
                            or force_rediscover
                        ):
                            query = """
                            ...
                            logger.info(
                                f"Updated relationship mapping path for {source_ontology} -> {target_ontology}"
                            )
                        ...
                        logger.info(
                            f"Created new relationship mapping path for {source_ontology} -> {target_ontology}"
                        )

                except SQLAlchemyError as e:
                    logger.error(
                        f"Database error while processing path {source_ontology} -> {target_ontology}: {e}"
                    )
    ...
    async def _create_sample_mapping_path(
        self, source_type: str, target_type: str
    ) -> Optional[int]:
        ...
        if source_type.lower() == "hmdb" and target_type.lower() == "pubchem":
            ...
            # Direct mapping with UniChem
            path_steps = [
                {"resource": "unichem", "source_type": "pubchem", "target_type": "hmdb"}
            ]
        elif source_type.lower() == "name" and target_type.lower() in [
            "pubchem",
            "chebi",
            "hmdb",
        ]:
        ...
        try:
            ...
            logger.info(
                f"Created sample mapping path for {source_type} -> {target_type} with ID {path_id}"
            )
            return path_id
    ...
    async def get_best_mapping_path(
        ...
        if not path or force_discover:
            ...
            paths = await self.discover_relationship_paths(
                relationship_id, force_rediscover=force_discover
            )

            # Find the path for our specific ontology types
            for discovered_path in paths:
                if (
                    discovered_path["source_ontology"] == source_ontology
                    and discovered_path["target_ontology"] == target_ontology
                ):
                    ...
                    """
                    result = await session.execute(
                        text(query), {"path_id": discovered_path["ontology_path_id"]}
                    )
                    ...
                    if mapping_path:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to decode path_steps for mapping path ID {mapping_path.id}"
                            )
            ...
            logger.warning(
                f"No mapping path found for {source_ontology} -> {target_ontology} in relationship {relationship_id}"
            )
            return None
        ...
        try:
            ...
            logger.warning(
                f"Failed to decode path_steps for mapping path ID {path.ontology_path_id}"
            )
    ...
    async def update_path_metrics(
        ...
        try:
            ...
            if not path:
                logger.warning(
                    f"No mapping path found to update metrics for {source_ontology} -> {target_ontology}"
                )
                return
            ...
            if path.success_rate is not None:
                ...
                new_success_rate = (
                    path.success_rate * (1 - alpha) + (1.0 if success else 0.0) * alpha
                )
            ...
            if execution_time_ms is not None and execution_time_ms > 0:
                ...
                if path.performance_score is not None:
                    ...
                    new_performance_score = (
                        path.performance_score * (1 - alpha) + normalized_score * alpha
                    )
            ...
            logger.info(
                f"Updated metrics for mapping path {source_ontology} -> {target_ontology}"
            )
