...
class UniChemAdapter(ResourceAdapter):
    ...
    def __init__(self, connection_info: Dict[str, Any]):
        ...
        """
        self.base_url = connection_info.get(
    ...
    async def map_id(
        ...
        if key in sample_mappings:
            logger.info(
                f"UniChem mapping {source_id} from {source_type} to {target_type}"
            )
            return sample_mappings[key]

        logger.warning(
            f"No UniChem mapping found for {source_id} from {source_type} to {target_type}"
        )
        return []
    ...
    def get_supported_mappings(self) -> List[Dict[str, str]]:
        ...
        for source in self.source_datasource_ids.keys():
            for target in self.source_datasource_ids.keys():
                if source != target:
                    mappings.append({"source_type": source, "target_type": target})
        return mappings
...
class KEGGAdapter(ResourceAdapter):
    ...
    async def map_id(
        ...
        logger.warning(
            f"No KEGG mapping found for {source_id} from {source_type} to {target_type}"
        )
        return []
            ...
            if "transformations" in step_config:
                for transform in step_config["transformations"]:
                    if (
                        ...
                        and "from" in transform
                        and "to" in transform
                    ):
                        processed_source_id = processed_source_id.replace(
...
class StepExecutorFactory:
    ...
    @staticmethod
    async def create_step_executor(
        ...
        for resource in resources:
            ...
            if resource.connection_info:
                try:
                    ...
                    logger.warning(
                        f"Failed to decode connection_info for resource {resource.name}"
                    )
