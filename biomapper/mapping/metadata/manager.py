...
class ResourceMetadataManager:
    ...
    def register_resource(
        ...
        try:
            ...
            cursor.execute(
                "SELECT id FROM resource_metadata WHERE resource_name = ?", (name,)
            )
    ...
    def register_ontology_coverage(
        ...
        try:
            ...
            if coverage:
                ...
                logger.info(
                    f"Updated ontology coverage for {resource_name}: {ontology_type}"
                )
            else:
                # Create new coverage
                cursor.execute(
                ...
                logger.info(
                    f"Added ontology coverage for {resource_name}: {ontology_type}"
                )
    ...
    def update_performance_metrics(
        ...
        try:
            ...
            if metrics:
                ...
                new_success_rate = (1 - weight) * success_rate + weight * (
    ...
    def get_resources_by_priority(
        ...
        try:
            ...
            if source_type:
                conditions.append(
                    "(source_oc.id IS NULL OR source_oc.support_level != 'none')"
                )
            if target_type:
                conditions.append(
                    "(target_oc.id IS NULL OR target_oc.support_level != 'none')"
                )
            ...
            for row in cursor.fetchall():
                ...
                if row[4]:  # connection_info
                    try:
                        ...
                        logger.warning(
                            f"Invalid JSON in connection_info for resource {row[1]}"
                        )
