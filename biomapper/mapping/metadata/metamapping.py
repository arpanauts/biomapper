...
class MetamappingEngine:
        ...
        while queue:
            ...
            for next_type in possible_intermediates:
                ...
                resources = self.metadata_manager.find_resources_by_capability(
                    source_type=current_type, target_type=next_type
                )
    ...
    async def execute_mapping_path(
        self, source_id: str, mapping_path: List[Dict[str, Any]], **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        for step in mapping_path:
                    ...
                    try:
                        ...
                        if resource_results:
                            ...
                            await self._cache_intermediate_results(
                                current_id, source_type, target_type, resource_results
                            )
                            break

                    except Exception as e:
                        logger.warning(
                            f"Error in metamapping step with {resource_name}: {e}"
                        )
            ...
            if not next_ids:
                logger.warning(
                    f"Mapping path failed at step {source_type} to {target_type}"
                )
                return []
    ...
    async def _cache_intermediate_results(
        ...
        cache_adapters = [
            adapter
            for name, adapter in self.dispatcher.resource_adapters.items()
