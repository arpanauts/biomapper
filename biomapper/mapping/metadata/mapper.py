...
class RelationshipMappingExecutor:
    ...
    async def get_step_executor(self) -> StepExecutor:
        ...
        if self._step_executor is None:
            ...
            self._step_executor = await StepExecutorFactory.create_step_executor(self._session)
    ...
    async def get_mapping_resources(self) -> Dict[int, Dict]:
        ...
        return {
            resource.resource_id: {
                ...
                "connection_info": json.loads(resource.connection_info) if resource.connection_info else {},
        ...
        for row in result.fetchall():
            ...
            if row.mapping_path:
                try:
                    ...
                    logger.warning(f"Failed to decode mapping_path for cached mapping {row.mapping_id}")
                ...
                if existing.confidence < confidence:
                    ...
                    await session.execute(
                        text(query),
                        {
                            ...
                            "mapping_path": json.dumps(mapping_path) if mapping_path else None,
                            "resource_id": resource_id
                        }
                    )
    ...
    async def execute_mapping_step(
        ...
        if (source_type.lower() == "hmdb" and 
            step_config.get("resource") == "unichem" and 
            normalized_source_id.startswith("HMDB")):
        ...
        try:
            ...
            logger.error(f"Error executing mapping step {source_type} -> {target_type}: {e}")
            return []
    ...
    async def execute_mapping_path(
        ...
        """
        if not path_steps:
            logger.warning(f"No path steps to execute for {source_type} -> {target_type}")
            return []
        ...
        for i, step in enumerate(path_steps):
            ...
            for result in current_results:
                ...
                for step_result in step_results:
                    step_result["confidence"] = step_result.get("confidence", 1.0) * result["confidence"]
            ...
            if not current_results:
                logger.warning(f"No results after step {i+1} of {len(path_steps)} for {source_type} -> {target_type}")
                return []
    ...
    async def map_with_relationship(
        ...
        if not force_refresh:
            ...
            if cached:
                logger.info(f"Found {len(cached)} cached mappings for {source_id} ({source_type} -> {target_type})")
                return cached
        ...
        if not mapping_path or not mapping_path.get("path_steps"):
            logger.warning(f"No suitable mapping path found for {source_type} -> {target_type}")
            return []
    ...
    async def map_endpoint_value(
        ...
        source_ontologies = await pathfinder.get_supported_ontology_types(source_endpoint_id)
        if not source_ontologies:
            source_ontologies = ["NAME"]
        
        # Get target ontology types
        target_ontologies = await pathfinder.get_supported_ontology_types(target_endpoint_id)
        if not target_ontologies:
            logger.error(f"No ontology types available for target endpoint {target_endpoint_id}")
            return []
        
        logger.info(f"Mapping between source ontologies {source_ontologies} and target ontologies {target_ontologies}")
