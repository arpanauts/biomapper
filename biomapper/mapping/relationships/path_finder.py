...
class RelationshipPathFinder:
    ...
    async def discover_paths_for_relationship(self, relationship_id: int) -> List[Dict[str, Any]]:
        ...
        source_preferences = await self._get_ontology_preferences(source_endpoint["endpoint_id"])
        target_preferences = await self._get_ontology_preferences(target_endpoint["endpoint_id"])
        ...
        for source_ont, source_pref in source_preferences:
            for target_ont, target_pref in target_preferences:
                ...
                if scored_paths:
                    ...
                    logger.debug(f"Stored best path for {source_ont} to {target_ont} with score {best_path['score']}")
        
        return discovered_paths
    
    async def _find_ontology_paths(self, source_type: str, target_type: str) -> List[Dict[str, Any]]:
        ...
        """)
        try:
            result = await self.db_session.execute(query, {"source_type": source_type, "target_type": target_type})
    ...
    async def _get_relationship(self, relationship_id: int) -> Optional[Dict[str, Any]]:
        ...
        """)
        try:
            result = await self.db_session.execute(query, {"relationship_id": relationship_id})
            row = result.fetchone()
    ...
    async def _get_endpoint_by_role(self, relationship_id: int, role: str) -> Optional[Dict[str, Any]]:
        ...
        """)
        try:
            result = await self.db_session.execute(query, {"relationship_id": relationship_id, "role": role})
            row = result.fetchone()
    ...
    async def _get_ontology_preferences(self, endpoint_id: int) -> List[Tuple[str, int]]:
        """
    ...
    async def _store_relationship_path(self, relationship_id: int, source_ontology: str,
        ...
        try:
            ...
            logger.debug(f"Stored/Updated path for rel {relationship_id}: {source_ontology} -> {target_ontology}")
            return True
        except Exception as e:
