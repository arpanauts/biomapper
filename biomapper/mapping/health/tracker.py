...
class ErrorCategorizer:
    ...
    def categorize(error_message: str) -> str:
        ...
        if any(
            ...
            for word in ["column", "field", "not found", "missing"]
        ):
            return ErrorCategory.MISSING_COLUMN.value

        if any(
            word in error_message for word in ["no match", "not match", "zero matches"]
        ):
    ...
    def flush_metrics(self) -> bool:
        ...
        try:
            ...
            for (
                ...
                health_record = (
                    self.db_session.query(EndpointPropertyHealth)
                    .filter_by(
                        endpoint_id=endpoint_id,
                        ontology_type=ontology_type,
                        property_name=property_name,
                ...
                if successful_metrics:
                    health_record.last_success_time = max(
                        m["timestamp"] for m in successful_metrics
                    )

                if failed_metrics:
                    health_record.last_failure_time = max(
                        m["timestamp"] for m in failed_metrics
                    )
                ...
                for metric in failed_metrics:
                    if metric["error_message"]:
                        error_type = ErrorCategorizer.categorize(
