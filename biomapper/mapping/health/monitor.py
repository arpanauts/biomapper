...
class SampleDataFetcher:
    ...
    @staticmethod
    async def fetch_sample_data(endpoint_id: int, db_session: Session) -> Dict[str, Any]:
        """
...
class ConfigTester:
    ...
    async def test_config(
        ...
        try:
            ...
            if not config:
                raise ValueError(f"Property config not found: {endpoint_id}/{ontology_type}/{property_name}")
                
            # Get sample data
            sample_data = await SampleDataFetcher.fetch_sample_data(endpoint_id, db_session)
            
            # Test extraction based on method
            if config.extraction_method == "column":
                extracted_values = ConfigTester._test_column_extraction(config, sample_data)
            elif config.extraction_method == "pattern":
                extracted_values = ConfigTester._test_pattern_extraction(config, sample_data)
            elif config.extraction_method == "query":
                extracted_values = ConfigTester._test_query_extraction(config, sample_data)
            else:
                raise ValueError(f"Unsupported extraction method: {config.extraction_method}")
...
class EndpointHealthMonitor:
        ...
        try:
            ...
            for endpoint in endpoints:
                ...
                if endpoint_results["configs_checked"] > 0:
                    endpoint_results["success_rate"] = endpoint_results["success_count"] / endpoint_results["configs_checked"]
                    endpoint_results["status"] = "healthy" if endpoint_results["success_rate"] >= 0.8 else "at_risk"
            ...
            health_log.details_dict = {
                "endpoints": [results_by_endpoint[eid] for eid in sorted(results_by_endpoint.keys())]
            }
    ...
    def _get_endpoints_to_check(self, session: Session, endpoint_id: Optional[int] = None) -> List[Any]:
