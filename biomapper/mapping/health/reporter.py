...
class HealthReportGenerator:
    ...
    def generate_endpoint_health_report(self, endpoint_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for record in results:
                ...
                total_attempts = record.extraction_success_count + record.extraction_failure_count
                success_rate = record.extraction_success_count / total_attempts if total_attempts > 0 else 0
                ...
                endpoint_report["ontologies"][record.ontology_type].append({
                    ...
                    "last_success": record.last_success_time.isoformat() if record.last_success_time else None,
            ...
            report["summary"] = {
                ...
                "healthy_endpoints": sum(1 for e in report["endpoints"].values() 
                                         if e["total_configs"] > 0 and e["healthy_configs"] / e["total_configs"] >= 0.8),
                "at_risk_endpoints": sum(1 for e in report["endpoints"].values() 
                                         if e["total_configs"] > 0 and 0.5 <= e["healthy_configs"] / e["total_configs"] < 0.8),
                "failed_endpoints": sum(1 for e in report["endpoints"].values() 
                                        if e["total_configs"] > 0 and e["healthy_configs"] / e["total_configs"] < 0.5),
                "healthy_configs": sum(e["healthy_configs"] for e in report["endpoints"].values()),
                "at_risk_configs": sum(e["at_risk_configs"] for e in report["endpoints"].values()),
                "failed_configs": sum(e["failed_configs"] for e in report["endpoints"].values())
            }
                ...
                trend_data["ontology_breakdown"] = [
                    {
                        ...
                        "success_rate": stat.success_count / (stat.success_count + stat.failure_count)
                            if (stat.success_count + stat.failure_count) > 0 else 0
                    }
...
class ReportFormatter:
    ...
    def to_text(report: Dict[str, Any]) -> str:
        ...
        if "summary" in report:
            ...
            lines.append(f"  Healthy: {summary['healthy_configs']} configs, {summary['healthy_endpoints']} endpoints")
            lines.append(f"  At risk: {summary['at_risk_configs']} configs, {summary['at_risk_endpoints']} endpoints")
            lines.append(f"  Failed:  {summary['failed_configs']} configs, {summary['failed_endpoints']} endpoints")
            lines.append("")
        ...
        for endpoint_name, endpoint in report.get("endpoints", {}).items():
            ...
            lines.append(f"    Type: {endpoint['endpoint_type']}, Subtype: {endpoint['endpoint_subtype'] or 'N/A'}")
            ...
            if endpoint["total_configs"] > 0:
                ...
                lines.append(f"    Status: {status} ({endpoint['healthy_configs']} of {endpoint['total_configs']} configs healthy)")
            else:
            ...
            for ontology, properties in endpoint.get("ontologies", {}).items():
                ...
                for prop in properties:
                    status = prop["status"].upper() if prop["status"] == "failed" else prop["status"]
                    lines.append(f"      {prop['property_name']} - {status} (Success rate: {prop['success_rate']:.2f})")
                    
                    if prop["common_errors"]:
                        lines.append(f"        Common errors: {', '.join(prop['common_errors'])}")
        ...
        for endpoint_name, endpoint in report.get("endpoints", {}).items():
            ...
            for ontology, properties in endpoint.get("ontologies", {}).items():
                for prop in properties:
                    ...
                    html.append(f"      <td>{', '.join(prop['common_errors']) if prop['common_errors'] else 'None'}</td>")
                    html.append("    </tr>")
