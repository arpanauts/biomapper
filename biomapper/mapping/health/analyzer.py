...
class PerformanceClassifier:
    ...
    def classify_config(health_record: EndpointPropertyHealth) -> str:
        ...
        total = (
            health_record.extraction_success_count
            + health_record.extraction_failure_count
        )
        if total == 0:
        ...
        try:
            ...
            if stats["total"] > 0:
                for key in ["healthy", "at_risk", "failed", "unknown"]:
                    stats[f"{key}_percent"] = round(
                        stats[key] / stats["total"] * 100, 1
                    )
...
class PatternAnalyzer:
    ...
    @staticmethod
    def analyze_pattern(
        pattern_json: str, extraction_method: str, common_errors: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if extraction_method == "column":
                issues.extend(
                    PatternAnalyzer._analyze_column_pattern(pattern, common_errors)
                )
            elif extraction_method == "pattern":
                issues.extend(
                    PatternAnalyzer._analyze_regex_pattern(pattern, common_errors)
                )
            elif extraction_method == "query":
                issues.extend(
                    PatternAnalyzer._analyze_query_pattern(pattern, common_errors)
                )
    ...
    def _analyze_column_pattern(
        pattern: Dict[str, Any], common_errors: List[str]
    ) -> List[str]:
    ...
    def _analyze_regex_pattern(
        ...
        if (
            ...
            issues.append(
                "Strict pattern (^...$) without wildcards may be too restrictive"
            )
...
class ConfigImprover:
        ...
        try:
            ...
            for health_record in health_data:
                ...
                analysis = PatternAnalyzer.analyze_pattern(
                    config.extraction_pattern, config.extraction_method, error_types
                )
                ...
                suggestion = self._generate_suggestion(config, health_record, analysis)
    ...
    def _get_property_config(
        self, session: Session, endpoint_id: int, ontology_type: str, property_name: str
    ) -> Optional[Any]:
    ...
    def _generate_suggestion(
        ...
        if config.extraction_method == "column":
            suggested_pattern = self._suggest_column_pattern(
    ...
    def _suggest_column_pattern(
        self, pattern: Dict[str, Any], issues: List[str]
    ) -> Optional[Dict[str, Any]]:
        ...
        for issue in issues:
            if "Column not found" in issue:
                ...
                if original:
                    ...
                    suggested["comment"] = (
                        "Check if this column name is correct. Consider similar names like: "
        ...
        for issue in issues:
            if "too restrictive" in issue:
                ...
                if original.startswith("^") and original.endswith("$"):
                    ...
                    suggested[
                        "comment"
                    ] = "Removed strict anchors (^ and $) to make the pattern more flexible."
                    return suggested
                ...
                if "(" not in original and ")" not in original:
                    ...
                    suggested[
                        "comment"
                    ] = "Added capture group around the entire pattern."
                    return suggested
                ...
                if original.startswith("^") and original.endswith("$"):
                    ...
                    suggested[
                        "comment"
                    ] = "Added wildcards before and after to improve matching."
                    return suggested
