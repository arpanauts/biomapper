...
class HealthAwareEndpointManager:
    ...
    def get_healthy_ontology_preferences(
        self, endpoint_id: int, min_success_rate: float = 0.8
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_property_configs_with_health(
        self, endpoint_id: int, ontology_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for config in configs:
            ...
            if key in health_by_config:
                ...
                config["health"] = {
                    ...
                    "error_types": record.error_types_list
                    if hasattr(record, "error_types_list")
                    else [],
                }
            else:
                config["health"] = {
...
class ValidPreferenceSelector:
    ...
    def get_valid_preferences(
        self, endpoint_id: int, min_success_rate: float = 0.8, min_sample_size: int = 5
    ) -> List[Dict[str, Any]]:
        """
