...
class UniProtIDMappingClient:
    ...
    def __init__(self, from_db: str = "ACC", to_db: str = "ACC",
        ...
        logger.info(f"UniProtIDMappingClient initialized: {from_db} -> {to_db}, URL: {base_url}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if isinstance(ids_to_log, str):
                    ids_to_log = ids_to_log.split(',') # Assuming ids is comma-separated for logging
                logger.debug(f"Attempt {attempt + 1} to GET from {self.base_url} with {len(ids_to_log)} IDs (params: {payload})...") 
                # Use GET and pass payload as params
                async with self.session.get(self.base_url, params=payload, headers=self.headers, timeout=request_timeout) as response:
                    if response.status == 200:
                        return await response.text()
                    
                    retriable_statuses = [500, 502, 503, 504]
                    if response.status in retriable_statuses:
                        logger.warning(f"Request failed with status {response.status} (retriable). Attempt {attempt + 1} of {max_retries}.")
                        if attempt + 1 == max_retries:
                            logger.error(f"Max retries reached. Request failed with status {response.status}: {await response.text()}")
                            return None
                        ...
                        if response.status == 405:
                            logger.error(f"Request failed with 405 Method Not Allowed for GET to {self.base_url}. Params: {payload}")
                        else:
                            logger.error(f"Request failed with non-retriable status {response.status}: {error_text}")
                        logger.debug(f"Failed params for GET: {payload}")
                        return None
            except aiohttp.ClientError as e:
                logger.warning(f"AIOHTTP client error: {e}. Attempt {attempt + 1} of {max_retries}.")
                if attempt + 1 == max_retries:
                ...
                logger.error(f"Unexpected error during request: {e}. Attempt {attempt + 1} of {max_retries}.")
                if attempt + 1 == max_retries:
    ...
    def _parse_response(self, response_text: str, input_ids_map: Dict[str, str]) -> Dict[str, Tuple[Optional[List[str]], Optional[str]]]:
        """Parses the tab-separated response from UniProt ID Mapping service."""
        final_mapping: Dict[str, Tuple[Optional[List[str]], Optional[str]]] = {k: (None, None) for k in input_ids_map.keys()}
        count = 0
        ...
        for line in lines:
            ...
            if len(parts) >= 2:
                ...
                if original_input_id in final_mapping:
                    ...
                    if primary_ids:
                        final_mapping[original_input_id] = (primary_ids, original_input_id) 
                        count += len(primary_ids)
                        logger.debug(f"Resolved {original_input_id} -> {primary_ids}")
                    else:
                        logger.warning(f"Input ID {original_input_id} returned empty mapping result part.")
                else:
                    logger.warning(f"Mapped ID '{mapped_from_id}' from UniProt results was not in the original input list (or not expected). Original inputs: {list(input_ids_map.keys())[:5]}...")
        ...
        logger.info(f"Successfully parsed {count} primary mappings for {len(input_ids_map)} original input identifiers.")
        return final_mapping

    async def map_identifiers(self, identifiers: List[str], config: Optional[Dict[str, Any]] = None) -> Dict[str, Tuple[Optional[List[str]], Optional[str]]]:
        ...
        if response_text:
            ...
            logger.error(f"No response text received from _make_request for {len(identifiers)} IDs. Returning empty mappings.")
            return {k: (None, None) for k in identifiers}

    async def close_session(self):
        """Closes the aiohttp session if it was managed internally."""
        if self._session_managed_internally and self.session and not self.session.closed:
...
async def run_example() -> None:
    ...
    gn_client_6 = UniProtIDMappingClient(from_db="gene_name")  # Test lowercase with underscore
    ...
    gn_client_10 = UniProtIDMappingClient(from_db="GeneSymbol") # Test GeneSymbol option
    ...
    try:
        ...
        for client_name, client in gn_clients:
            ...
            results = await client.map_identifiers(identifiers=well_known_genes, config=taxon_config)
        ...
        for input_id, result_tuple in results_acc.items():
            primary_ids, component_id = result_tuple
            print(f"  Input: {input_id} -> Mapped Primary: {primary_ids}, Component: {component_id}")
        primary_results_acc = {k: v[0] for k, v in results_acc.items() if v[0] is not None} 
        ...
        results_ensp = await ensp_client.map_identifiers(identifiers=ensembl_protein_ids_to_test)
        ...
        clients_to_close = [
            default_client,
            gn_client_1, gn_client_2, gn_client_3, gn_client_4, gn_client_5,
            gn_client_6, gn_client_7, gn_client_8, gn_client_9, gn_client_10,
            ensp_client, enst_client
        ]
        for client in clients_to_close:
            if hasattr(client, '_session_managed_internally') and client._session_managed_internally:
...
async def map_gene_names_to_uniprot_acs(
    ...
    try:
        results = await client.map_identifiers(identifiers=gene_names, config=mapping_config)
        for input_id, result_tuple in results.items():
            primary_ids, _ = result_tuple # We only care about the primary IDs list here
        ...
        if client._session_managed_internally: # Check if the client created the session
            await client.close_session()
...
async def map_ensembl_ids_to_uniprot_acs(
    ...
    client = UniProtIDMappingClient(from_db=ensembl_db_type, to_db="ACC", session=session)
    primary_results: Dict[str, Optional[List[str]]] = {}
    try:
