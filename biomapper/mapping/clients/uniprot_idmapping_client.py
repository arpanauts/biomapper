...
class UniProtIDMappingClient:
    ...
    def __init__(self, from_db: str = "UniProtKB_AC-ID", to_db: str = "Ensembl", base_url: str = UNIPROT_IDMAPPING_API_BASE_URL, config: Optional[Dict] = None):
        """
    ...
    async def _submit_job(self, session: aiohttp.ClientSession, ids: List[str]) -> Optional[str]:
        ...
        try:
            async with session.post(url, data=payload) as response:
                ...
                if not job_id:
                    logger.error(f"Failed to submit UniProt ID Mapping job. Response: {data}")
                    return None
    ...
    async def _check_job_status(self, session: aiohttp.ClientSession, job_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_job_results(self, session: aiohttp.ClientSession, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with session.get(url_results, params={"format": "json"}) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(f"Retrieved results for job {job_id} via /results endpoint.")
                    return data
                elif response.status == 404: # Not found, maybe results are at /stream?
                    logger.warning(f"Job {job_id} results not found at /results, potentially expired or very large. Status: {response.status}")
            ...
            logger.error(f"Error retrieving results for job {job_id} from {url_results}: {e}")
            return None

    async def map_identifiers(self, identifiers: List[str], config: Optional[Dict] = None) -> Dict[str, Optional[str]]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not job_id:
                return {identifier: None for identifier in identifiers} # Job submission failed
            ...
            while True:
                ...
                if "results" in status_response:
                    logger.info(f"Job {job_id} results found directly in status response.")
                ...
                if job_status == "FINISHED":
                    logger.info(f"Job {job_id} reported status FINISHED. Retrieving results...")
                    ...
                    logger.error(f"Job {job_id} failed or encountered an issue. Status: {job_status}. Response: {status_response}")
                    # results_data remains None
                    break # Exit polling loop

                if time.time() - start_time > MAX_WAIT_SECONDS:
                    logger.error(f"Job {job_id} timed out after {MAX_WAIT_SECONDS} seconds.")
                    # results_data remains None
                    break # Exit polling loop
            ...
            # Process results
            final_mapping: Dict[str, Optional[str]] = {identifier: None for identifier in identifiers}
            if results_data and "results" in results_data:
                ...
                for item in results_data["results"]:
                    ...
                    if input_id in final_mapping:
                        ...
                        if mapped_id_info:
                            if isinstance(mapped_id_info, dict) and mapped_id_info.get("primaryAccession"):
                        ...
                        if target_id is not None and final_mapping[input_id] is None: 
                            ...
                            logger.debug(f"Skipping update for {input_id}, already mapped to {final_mapping[input_id]}")
                ...
                logger.info(f"Successfully processed {count} mappings out of {len(identifiers)} potential results for job {job_id}.")
            elif results_data is None:
                logger.error(f"No results were obtained for job {job_id} (failed, timed out, or error retrieving).")
            else:
                 logger.warning(f"Job {job_id} finished but results format was unexpected: {results_data}")
