...
class GenericFileLookupClient(
    BaseMappingClient, FileLookupClientMixin, CachedMappingClientMixin
):
    """
    ...
    def _load_data(self) -> None:
        ...
        logger.info(
            f"{self.__class__.__name__}: Loading data from '{file_path}' using delimiter '{repr(delimiter)}'"
        )
        ...
        try:
            ...
            logger.info(
                f"{self.__class__.__name__}: Successfully loaded {len(self._lookup_data)} unique keys from '{file_path}'."
            )
    ...
    async def map_identifiers(
        ...
        for identifier in identifiers:
            ...
            if cached_result is not None:
                ...
                logger.debug(
                    f"{self.__class__.__name__}: Cache hit for identifier '{identifier}'"
                )
            else:
                identifiers_to_fetch_from_source.append(identifier)
                logger.debug(
                    f"{self.__class__.__name__}: Cache miss for identifier '{identifier}'"
                )
        ...
        if identifiers_to_fetch_from_source:
            for identifier in identifiers_to_fetch_from_source:
                ...
                formatted_result = self.format_result(
                    target_ids=target_ids, component_id=None
                )
    ...
    async def _get_from_cache(
        self, identifier: str
    ) -> Optional[Tuple[Optional[List[str]], Optional[str]]]:
