...
PUBCHEM_ATTRIBUTES_TO_FETCH = [
    ...
    "InChIKey",
    "Description",
    "Synonym",  # Note: Synonym might return a list
...
async def fetch_single_cid_annotation(
    client: httpx.AsyncClient, cid: int
) -> Optional[PubChemAnnotation]:
    """
    ...
    async with RATE_LIMIT_SEMAPHORE:
        try:
            ...
            if synonyms_response.status_code == 200:
                ...
                synonyms_data = (
                    synonyms_json.get("InformationList", {})
                    .get("Information", [{}])[0]
                    .get("Synonym", [])[:10]
                )  # Limit to 10
            ...
            if description_response.status_code == 200:
                ...
                descriptions = desc_json.get("InformationList", {}).get(
                    "Information", []
                )
            ...
            logger.error(
                f"Error fetching annotations for CID {cid}: {type(e).__name__}: {e}"
            )
            return None
...
async def fetch_pubchem_annotations(cids: List[int]) -> Dict[int, PubChemAnnotation]:
    ...
    logger.info(
        f"Successfully annotated {len(annotations_map)} out of {len(cids)} CIDs"
    )
