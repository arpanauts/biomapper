...
async def search_qdrant_for_biochemical_name(
    biochemical_name: str, top_k: int, client: Optional[PubChemRAGMappingClient] = None
    ...
    logger.info(
        f"Searching Qdrant for biochemical name: '{biochemical_name}' with top_k={top_k}"
    )
    ...
    try:
        ...
        all_scores = (
            mapping_result.metadata.get("all_scores", [])
            if mapping_result.metadata
            else []
        )
            ...
            if target_id.startswith("PUBCHEM:"):
                try:
                    ...
                    score = (
                        all_scores[i]
                        if i < len(all_scores)
                        else mapping_result.qdrant_similarity_score
                    )
        ...
        logger.info(
            f"Found {len(results)} results for '{biochemical_name}' (scores: {[r.score for r in results]})"
        )
