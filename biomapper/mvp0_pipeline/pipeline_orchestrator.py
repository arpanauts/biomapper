...
class PipelineOrchestrator:
    ...
    def _validate_configuration(self) -> None:
        ...
        if not self.config.anthropic_api_key:
            raise ValueError(
                "ANTHROPIC_API_KEY is required but not set in configuration"
            )
        ...
        if not self.config.qdrant_collection_name:
            raise ValueError(
                "Qdrant collection name is required but not set in configuration"
            )
    ...
    def _check_qdrant_connectivity(self) -> None:
        ...
        try:
            ...
            client = QdrantClient(
                host=host, port=port, api_key=self.config.qdrant_api_key, timeout=5.0
            )
            ...
            try:
                ...
                logger.info(
                    f"Successfully connected to Qdrant at {self.config.qdrant_url}"
                )
            except Exception as e:
                # Collection might not exist, but we should be able to connect
                logger.warning(
                    f"Qdrant collection '{self.config.qdrant_collection_name}' check failed: {e}"
                )
    ...
    async def run_pipeline(self, biochemical_names: List[str]) -> BatchMappingResult:
        ...
        logger.info(
            f"Starting batch pipeline for {len(biochemical_names)} biochemical names"
        )
    ...
    async def run_single_mapping(self, biochemical_name: str) -> PipelineMappingResult:
        ...
        try:
            ...
            try:
                ...
                result.processing_details["qdrant_search_time"] = (
                    time.time() - qdrant_start
                )
                ...
                logger.info(
                    f"Found {len(qdrant_results)} Qdrant candidates for '{biochemical_name}'"
                )
            ...
            logger.info(
                f"Stage 2: Fetching PubChem annotations for {len(qdrant_results)} candidates"
            )
            ...
            try:
                ...
                result.processing_details["pubchem_annotation_time"] = (
                    time.time() - pubchem_start
                )

                if not annotations:
                    logger.warning(
                        f"No PubChem annotations retrieved for '{biochemical_name}'"
                    )
                    result.status = PipelineStatus.INSUFFICIENT_ANNOTATIONS
                    result.error_message = (
                        "Failed to retrieve annotations from PubChem for any candidates"
                    )
                    return result
            ...
            # Stage 3: LLM Mapping
            logger.info(
                f"Stage 3: LLM evaluation of {len(annotations)} annotated candidates"
            )
            llm_start = time.time()
            ...
            try:
                ...
                if not llm_candidates:
                    logger.warning(
                        "No annotated candidates available for LLM evaluation"
                    )
                    result.status = PipelineStatus.INSUFFICIENT_ANNOTATIONS
                    result.error_message = (
                        "No candidates with annotations available for LLM evaluation"
                    )
                    return result
                ...
                if llm_choice.selected_cid is None:
                    ...
                    result.confidence = self._get_confidence_level(
                        llm_choice.llm_confidence
                    )

                    result.status = PipelineStatus.SUCCESS
                    logger.info(
                        f"Successfully mapped '{biochemical_name}' to CID {result.selected_cid}"
                    )
            ...
            logger.info(
                f"Pipeline completed for '{biochemical_name}' with status: {result.status}"
            )
    ...
    try:
        ...
        for result in batch_result.results:
            ...
            if result.selected_cid:
                print(
                    f"    → CID {result.selected_cid} ({result.confidence} confidence)"
                )
