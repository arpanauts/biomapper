...
class CalculateThreeWayOverlapParams(BaseModel):
    ...
    input_key: str = Field(
        ..., description="Key for combined matches from COMBINE_METABOLITE_MATCHES"
    )
    ...
    confidence_threshold: float = Field(
        0.8, ge=0.0, le=1.0, description="Minimum confidence for overlap inclusion"
    )
    output_dir: str = Field(..., description="Directory for output files")
    mapping_combo_id: str = Field(
        ..., description="Identifier for this mapping combination"
    )
    generate_visualizations: List[str] = Field(
        default_factory=list, description="List of visualizations to generate"
    )
...
class CalculateThreeWayOverlapAction(
    ...
    def _get_confidence_distribution(
        self, metabolite_ids: Set[str], all_matches: List[Dict[str, Any]]
    ) -> Dict[str, int]:
    ...
    def _calculate_overlap_statistics(
            ...
            stats[dataset] = {
                ...
                "unique_metabolites": len(
                    ...
                    - (
                        ...
                        | memberships[
                            f"{dataset}_UKBB"
                            if dataset == "Israeli10K"
                            else f"Israeli10K_{dataset}"
                        ]
                        | memberships[
                            f"{dataset}_Arivale"
                            if dataset != "Arivale"
                    ...
                    if dataset == "Israeli10K"
                    else memberships[dataset]
                    - (
                        memberships["Israeli10K_UKBB_Arivale"]
                        | memberships[
                            f"Israeli10K_{dataset}"
                            if dataset == "UKBB"
                            else f"{dataset}_UKBB"
                        ]
                        | memberships[
                            f"{dataset}_Arivale"
                            if dataset != "Arivale"
                            else "Israeli10K_Arivale"
                        ]
                    )
                    if dataset == "UKBB"
                    else memberships[dataset]
        ...
        for i, ds1 in enumerate(dataset_names):
            for j, ds2 in enumerate(dataset_names[i + 1 :], i + 1):
                ...
                stats[pair_key] = OverlapStatistics(
                    count=len(overlap),
                    percentage_of_first=100 * len(overlap) / len(memberships[ds1])
                    if memberships[ds1]
                    else 0,
                    percentage_of_second=100 * len(overlap) / len(memberships[ds2])
                    if memberships[ds2]
                    else 0,
                    jaccard_index=len(overlap)
                    / len(memberships[ds1] | memberships[ds2])
                    if (memberships[ds1] | memberships[ds2])
                    else 0,
        ...
        stats["three_way"] = OverlapStatistics(
            count=len(three_way),
            percentage_of_first=100 * len(three_way) / len(memberships["Israeli10K"])
            if memberships["Israeli10K"]
            else 0,
            percentage_of_second=100 * len(three_way) / len(memberships["UKBB"])
            if memberships["UKBB"]
            else 0,
            percentage_of_third=100 * len(three_way) / len(memberships["Arivale"])
            if memberships["Arivale"]
            else 0,
            jaccard_index=len(three_way)
            / len(
                memberships["Israeli10K"] | memberships["UKBB"] | memberships["Arivale"]
            )
            if (
                memberships["Israeli10K"] | memberships["UKBB"] | memberships["Arivale"]
            )
    ...
    def _generate_visualizations(
        ...
        if "venn_diagram_3way" in visualizations:
            viz_paths["venn_diagram"] = self._create_venn_diagram(
                memberships, output_dir
            )

        if "confidence_heatmap" in visualizations:
            viz_paths["confidence_heatmap"] = self._create_confidence_heatmap(
                stats, output_dir
            )

        if "provenance_sankey" in visualizations:
            viz_paths["provenance_sankey"] = self._create_provenance_sankey(
                stats, output_dir
            )

        if "method_breakdown_pie" in visualizations:
            viz_paths["method_breakdown"] = self._create_method_breakdown(
                stats, output_dir
            )

        if "overlap_progression_chart" in visualizations:
            viz_paths["overlap_progression"] = self._create_progression_chart(
                stats, output_dir
            )
    ...
    def _create_venn_diagram(
        self, memberships: Dict[str, Set[str]], output_dir: Path
    ) -> str:
        ...
        ukbb_only = (
            memberships["UKBB"] - memberships["Israeli10K"] - memberships["Arivale"]
        )
        arivale_only = (
            memberships["Arivale"] - memberships["Israeli10K"] - memberships["UKBB"]
        )

        i10k_ukbb_only = (
            memberships["Israeli10K"] & memberships["UKBB"]
        ) - memberships["Arivale"]
        i10k_arivale_only = (
            memberships["Israeli10K"] & memberships["Arivale"]
        ) - memberships["UKBB"]
        ukbb_arivale_only = (
            memberships["UKBB"] & memberships["Arivale"]
        ) - memberships["Israeli10K"]
        ...
        plt.text(
            0.5,
            -0.1,
            f"Total unique metabolites: {total_unique}\nThree-way overlap: {three_way_count}",
    ...
    def _create_confidence_heatmap(
        self, stats: Dict[str, Any], output_dir: Path
    ) -> str:
        ...
        for overlap_type in overlap_types:
            if overlap_type in stats and isinstance(
                stats[overlap_type], OverlapStatistics
            ):
                row = []
        ...
        plt.title(
            "Confidence Distribution by Overlap Type", fontsize=14, fontweight="bold"
        )
    ...
    def _create_progression_chart(self, stats: Dict[str, Any], output_dir: Path) -> str:
        ...
        for pair_key in ["Israeli10K_UKBB", "Israeli10K_Arivale", "UKBB_Arivale"]:
            if pair_key in stats and isinstance(stats[pair_key], OverlapStatistics):
                ...
                pairwise_data.append(
                    (pair_key.replace("_", "-"), stats[pair_key].count)
                )
        ...
        plt.title(
            "Metabolite Distribution Across Datasets", fontsize=14, fontweight="bold"
        )
    ...
    def _export_detailed_results(
        ...
        for m in all_matches:
            ...
            # Add Arivale data
            if m.get("arivale"):
                row["arivale_biochemical_name"] = m["arivale"].get(
                    "biochemical_name", ""
                )
        ...
        matches_df.to_csv(
            output_dir / f"{mapping_combo_id}_three_way_matches.csv", index=False
        )
                ...
                stats_data.append(
                    {
                        ...
                        "high_confidence_count": stat.confidence_distribution.get(
                            "high", 0
                        ),
                        "medium_confidence_count": stat.confidence_distribution.get(
                            "medium", 0
                        ),
                        "low_confidence_count": stat.confidence_distribution.get(
                            "low", 0
                        ),
                    }
                )
        ...
        if stats_data:
            pd.DataFrame(stats_data).to_csv(
                output_dir / f"{mapping_combo_id}_overlap_statistics.csv", index=False
            )
    ...
    async def execute_typed(
        ...
        memberships = self._extract_dataset_memberships(
            three_way_matches, two_way_matches, params.confidence_threshold
        )

        # Calculate overlap statistics
        all_matches = three_way_matches + two_way_matches
        stats = self._calculate_overlap_statistics(
            memberships, params.dataset_names, all_matches
        )
        ...
        if params.generate_visualizations:
            viz_paths = self._generate_visualizations(
                memberships, stats, output_dir, params.generate_visualizations
            )
