...
class VectorEnhancedMatchParams(BaseModel):
    ...
    target_dataset_key: Optional[str] = Field(
        default=None,
        description="Optional target dataset for additional matching"
    )
...
class VectorEnhancedMatchAction(TypedStrategyAction[VectorEnhancedMatchParams, StandardActionResult]):
    ...
    def _initialize_clients(self, params: VectorEnhancedMatchParams) -> None:
        ...
        if not self.qdrant_client:
            ...
            try:
                collection_info = self.qdrant_client.get_collection(params.qdrant_collection)
                logger.info(
    ...
    def _prepare_search_texts(self, metabolite: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    async def execute_typed(
        ...
        matched_metabolites = {
            m.metabolite.get('BIOCHEMICAL_NAME', '')
        ...
        for match in unique_matches:
            match_record = {
                ...
                'matched_text': match.metabolite.get(
                    'BIOCHEMICAL_NAME' if match.matched_on == 'original_name' 
                    else 'cts_enriched_names', 
                    ['']
                )[0] if match.matched_on == 'cts_enriched' else match.metabolite.get('BIOCHEMICAL_NAME', '')
            }
        ...
        avg_similarity = total_similarity / len(unique_matches) if unique_matches else 0.0
        
        metrics = VectorMatchMetrics(
            stage='vector_enhanced',
            total_unmatched_input=len(unmatched_data),
            total_matched=len(unique_matches),
            match_rate=len(unique_matches) / len(unmatched_data) if unmatched_data else 0.0,
        ...
        if hasattr(context, 'set_action_data'):
            ...
            unmatched_key = f"unmatched.vector.{params.unmatched_dataset_key.split('.')[-1]}"
