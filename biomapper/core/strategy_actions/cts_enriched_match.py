...
class CtsEnrichedMatchParams(BaseModel):
    ...
    target_column: str = Field(
        default="unified_name", description="Column in target dataset to match against"
    )
    cts_timeout: int = Field(
        default=30, description="Timeout for CTS API calls in seconds"
    )
    batch_size: int = Field(default=50, description="Batch size for CTS API calls")
    output_key: str = Field(description="Key to store enriched matches")
    track_metrics: bool = Field(default=True, description="Track detailed metrics")
    match_threshold: float = Field(
        default=0.80, ge=0.0, le=1.0, description="Minimum score for matches"
    )
    cts_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Configuration for CTS client"
    )
...
class CtsEnrichedMatchAction(
    TypedStrategyAction[CtsEnrichedMatchParams, StandardActionResult]
):
    ...
    def _fuzzy_match_enriched(
        ...
        if best_match:
            return {
                ...
                "enrichment_used": best_source_name
                in enriched_metabolite.get("cts_enriched_names", []),
            }
    ...
    async def execute_typed(
        ...
        enriched_metabolites, api_calls = await self._enrich_metabolites(
            unmatched_data, params.identifier_columns, params.batch_size
        )

        # Count enrichment success
        enriched_count = sum(
            1 for m in enriched_metabolites if m.get("cts_enriched_names")
        )
        ...
        for metabolite in enriched_metabolites:
            match_result = self._fuzzy_match_enriched(
                metabolite, target_data, params.target_column, params.match_threshold
            )
        ...
        if hasattr(context, "set_action_data"):
            ...
            unmatched_key = (
                f"unmatched.api.{params.unmatched_dataset_key.split('.')[-1]}"
            )
