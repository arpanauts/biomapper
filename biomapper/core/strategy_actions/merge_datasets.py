...
class MergeDatasetsParams(BaseModel):
    ...
    output_key: str = Field(
        ...,
        description="Key for storing merged dataset"
    )
    deduplication_column: Optional[str] = Field(
        None,
        description="Column to use for deduplication"
    )
    keep: str = Field(
        "first",
    ...
    join_on: Optional[str] = Field(
        None,
        description="Column to join on when using 'join' strategy"
    )
    join_how: str = Field(
        "outer",
...
class MergeDatasetsAction(
    ...
    async def execute_typed(
        ...
        try:
            ...
            if params.deduplication_column and params.deduplication_column in merged_df.columns:
                if params.keep != 'all':
                    merged_df = merged_df.drop_duplicates(
                        subset=[params.deduplication_column],
                        keep=params.keep
                    )
            ...
            provenance = [{
                "action": "MERGE_DATASETS",
                "status": "success",
                "datasets_merged": [k for k in params.dataset_keys if k not in missing_datasets],
