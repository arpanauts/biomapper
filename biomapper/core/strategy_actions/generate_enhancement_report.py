...
class GenerateEnhancementReportParams(BaseModel):
    ...
    include_visualizations: bool = Field(
        default=True,
        description="Whether to include ASCII charts in the report"
    )
    
    include_detailed_stats: bool = Field(
        default=True,
        description="Whether to include detailed statistics per dataset"
    )
...
class GenerateEnhancementReport(TypedStrategyAction[GenerateEnhancementReportParams, StandardActionResult]):
    """
    ...
    def _aggregate_metrics(self, metrics_keys: List[str], context: Any) -> List[Dict[str, Any]]:
    ...
    def _generate_markdown_report(
        ...
        if metrics:
            ...
            if baseline_idx > 0:
                baseline_rate = metrics[baseline_idx].get('cumulative_match_rate', metrics[baseline_idx].get('match_rate', 0))
            else:
                baseline_rate = metrics[baseline_idx].get('match_rate', 0) if baseline_idx < len(metrics) else 0
            
            final_idx = len(metrics) - 1
            final_rate = metrics[final_idx].get('cumulative_match_rate', metrics[final_idx].get('match_rate', 0))
            
            # Calculate totals
            total_metabolites = metrics[0].get('total_unmatched_input', 1000) if metrics else 1000
            total_matched = int(final_rate * total_metabolites)
            
            # Calculate improvements
            abs_improvement = self._calculate_absolute_improvement(baseline_rate, final_rate)
            rel_improvement = self._calculate_relative_improvement(baseline_rate, final_rate)
            ...
            lines.extend([
                ...
                f"- ✅ Demonstrated clear value of API and vector enhancement approaches",
        ...
        if metrics:
            ...
            for i, (metric, stage_name) in enumerate(zip(metrics, stage_names[:len(metrics)])):
                ...
                if i == 0:
                    ...
                    prev_cumulative = metrics[i-1].get('cumulative_match_rate', metrics[i-1].get('match_rate', 0))
                    improvement = self._calculate_relative_improvement(prev_cumulative, cumulative_rate)
        ...
        if include_detailed_stats and metrics:
            ...
            for i, (metric, stage_name) in enumerate(zip(metrics, stage_names[:len(metrics)])):
                ...
                if i > 0:
                    lines.append(f"- Cumulative match rate: {metric.get('cumulative_match_rate', 0)*100:.1f}%")
                
                lines.append(f"- Processing time: {metric.get('execution_time', 0):.1f} seconds")
                
                # Add stage-specific details
                if 'avg_confidence' in metric:
                    lines.append(f"- Average confidence: {metric['avg_confidence']:.2f}")
                ...
                if 'avg_similarity_score' in metric:
                    lines.append(f"- Average similarity score: {metric['avg_similarity_score']:.3f}")
        ...
        if metrics and len(metrics) >= 2:
            ...
            final_rate = metrics[-1].get('cumulative_match_rate', metrics[-1].get('match_rate', 0))
            improvement = self._calculate_relative_improvement(baseline_rate, final_rate)
    ...
    async def execute_typed(
        ...
        logger.info(f"Generating enhancement report from {len(params.metrics_keys)} metric sources")
        ...
        return StandardActionResult(
            ...
            details={
                ...
                'baseline_used': params.metrics_keys[baseline_idx] if baseline_idx < len(params.metrics_keys) else 'first',
                'timestamp': timestamp.isoformat()
            }
        )