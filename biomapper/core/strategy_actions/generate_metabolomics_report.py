...
class GenerateMetabolomicsReportParams(BaseModel):
    ...
    metrics_keys: List[str] = Field(
        default_factory=list, description="Keys for stage metrics"
    )
    ...
    export_formats: List[str] = Field(
        default=["markdown", "html"], description="Export formats for the report"
    )
    template_dir: Optional[str] = Field(None, description="Custom template directory")
    include_visualizations: bool = Field(
        True, description="Embed visualizations in report"
    )
...
class GenerateMetabolomicsReportAction(
        ...
        **kwargs,
        ...
        try:
            ...
            for section_name in params.include_sections:
                try:
                    ...
                    if generator_method:
                        ...
                        logger.warning(
                            f"No generator found for section: {section_name}"
                        )
            ...
            generated_sections = len(
                [s for s in sections if not s.startswith("*Error")]
            )
            ...
            return StandardActionResult(
                ...
                details={
                    ...
                    "sections_success_rate": generated_sections / total_sections
                    if total_sections > 0
                    else 0,
    ...
    def _collect_report_data(
        self, context: Any, params: GenerateMetabolomicsReportParams
    ) -> Dict[str, Any]:
        ...
        nightingale_data = context.get_action_data("datasets", {}).get(
            params.nightingale_reference, {}
        )
    ...
    def _format_statistics(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        formatted = {
            ...
            "three_way_overlap": stats.get(
                "three_way_overlap", {"count": 0, "percentage": 0}
            ),
...
"""
    ...
    def _extract_key_insights(self, data: Dict[str, Any]) -> str:
        ...
        if three_way_pct > 30:
            insights.append(
                f"- Strong three-way agreement ({three_way_pct:.1f}%) indicates high mapping quality"
            )
        elif three_way_pct > 15:
            insights.append(
                f"- Moderate three-way overlap ({three_way_pct:.1f}%) suggests good core metabolite coverage"
            )
        else:
            insights.append(
                f"- Limited three-way overlap ({three_way_pct:.1f}%) indicates dataset heterogeneity"
            )
        ...
        if matches["total"] > 0:
            ...
            if high_conf_pct > 70:
                insights.append(
                    f"- {high_conf_pct:.0f}% high-confidence matches demonstrate reliable mapping"
                )
            elif high_conf_pct < 40:
                insights.append(
                    f"- Only {high_conf_pct:.0f}% high-confidence matches suggest need for validation"
                )
        ...
        if by_method:
            ...
            insights.append(
                f"- {best_method[0]} method was most effective ({best_method[1]} matches)"
            )
    ...
    def _generate_recommendations_summary(self, data: Dict[str, Any]) -> str:
        ...
        if matches["total"] > 0:
            ...
            if low_conf_pct > 20:
                recommendations.append(
                    "- Validate low-confidence matches with domain experts"
                )
        ...
        if three_way_pct < 20:
            recommendations.append(
                "- Investigate dataset-specific metabolites for harmonization opportunities"
            )
...
"""
        ...
        overview_data = {
            "israeli10k_count": dataset_counts.get("Israeli10K", {}).get(
                "total", "N/A"
            ),
            "israeli10k_unique": dataset_counts.get("Israeli10K", {}).get(
                "unique", "N/A"
            ),
...
"""
        ...
        two_way_percent = (
            (two_way_count / total_unique * 100) if total_unique > 0 else 0
        )

        one_way_count = overlap_summary.get("only_one_dataset", 0)
        one_way_percent = (
            (one_way_count / total_unique * 100) if total_unique > 0 else 0
        )
        ...
        # Get example matches
        example_matches = self._format_example_matches(
            data.get("matches", {}).get("examples", [])
        )
...
"""
        ...
        method_table = self._create_method_confidence_table(
            matches.get("by_method", {})
        )
    ...
    def _create_method_confidence_table(self, by_method: Dict[str, int]) -> str:
        ...
        for method, count in by_method.items():
            ...
            lines.append(
                f"| {method} | {config['avg']:.2f} | {count} | {config['range']} |"
            )
...
"""
        ...
        quality_data = {
            ...
            "method_effectiveness_table": self._create_method_effectiveness_table(
                metrics
            ),
...
"""
    ...
    def _assemble_report(
        self, sections: Dict[str, str], params: GenerateMetabolomicsReportParams
    ) -> str:
...
"""
    ...
    def _export_report(
        ...
        exported_files = {}
        base_name = (
            f"metabolomics_mapping_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
