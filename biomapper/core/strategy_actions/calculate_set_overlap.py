...
class CalculateSetOverlapParams(BaseModel):
    ...
    confidence_threshold: float = Field(0.8, description="Minimum confidence for 'high quality' stats", ge=0.0, le=1.0)
...
class CalculateSetOverlapAction(
    ...
    async def execute_typed(
        ...
        try:
            ...
            if params.input_key not in datasets:
                raise ValueError(f"Input dataset '{params.input_key}' not found in context")
            ...
            statistics = self._calculate_statistics(df, params, action_start_time, context)
    ...
    def _create_empty_statistics(self, params: CalculateSetOverlapParams) -> Dict[str, Any]:
    ...
    def _calculate_statistics(self, df: pd.DataFrame, params: CalculateSetOverlapParams, action_start_time: float = None, context: Any = None) -> Dict[str, Any]:
        ...
        high_conf_matches = len(matched_df[matched_df["match_confidence"] >= params.confidence_threshold])
        
        # Calculate average confidence (only for matched rows)
        avg_match_confidence = float(matched_df["match_confidence"].mean()) if len(matched_df) > 0 else 0.0
    ...
    def _create_statistics_csv(self, file_path: Path, statistics: Dict[str, Any]) -> None:
    ...
    def _create_breakdown_csv(self, file_path: Path, statistics: Dict[str, Any]) -> None:
    ...
    def _create_merged_dataset_csv(self, file_path: Path, merged_data: List[Dict[str, Any]]) -> None:
