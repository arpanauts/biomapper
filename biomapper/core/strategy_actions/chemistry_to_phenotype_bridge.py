...
class ChemistryToPhenotypeBridgeParams(BaseModel):
    ...
    source_key: str = Field(
        description="Key containing source chemistry/lab data"
    )
    target_key: str = Field(
        description="Key containing target phenotype data"
    )
    ...
    phenotype_id_column: str = Field(
        default="id",
        description="Column containing phenotype IDs in target"
    )
    phenotype_xrefs_column: str = Field(
        default="xrefs",
        description="Column containing phenotype cross-references"
    )
    association_threshold: float = Field(
        default=0.7,
        description="Minimum confidence threshold for associations"
    )
    use_semantic_matching: bool = Field(
        default=True,
        description="Whether to use semantic matching for unmapped items"
    )
    output_key: str = Field(
        description="Key to store mapping results"
    )


@register_action("CHEMISTRY_TO_PHENOTYPE_BRIDGE")
class ChemistryToPhenotypeBridge(TypedStrategyAction[ChemistryToPhenotypeBridgeParams, ActionResult]):
    ...
    async def execute_typed(
        ...
        try:
            ...
            logger.info(f"Mapping {len(source_df)} chemistry tests to {len(target_df)} phenotypes")
                    ...
                    if params.use_semantic_matching:
                        ...
                        if phenotype_match:
                            mappings.append({
                                ...
                                'match_type': 'semantic',
                                'confidence': phenotype_match.get('confidence', 0.5)
                            })
                        else:
                ...
                if associated_phenotypes:
                    ...
                    for hp_code in associated_phenotypes:
                        matched_phenotypes = target_df[
                            target_df[params.phenotype_id_column].str.contains(hp_code, na=False) |
                            target_df[params.phenotype_xrefs_column].str.contains(hp_code, na=False)
                        ]
                        ...
                        for _, phenotype_row in matched_phenotypes.iterrows():
                            mappings.append({
                                ...
                                'phenotype_id': phenotype_row[params.phenotype_id_column],
            ...
            stats = {
                ...
                'mapped_tests': len(results_df['chemistry_id'].unique()) if len(results_df) > 0 else 0,
                'total_mappings': len(mappings),
                'unmapped_tests': len(unmapped),
                'mapping_types': results_df['match_type'].value_counts().to_dict() if len(results_df) > 0 else {}
            }
    ...
    def _load_loinc_hp_associations(self) -> Dict[str, List[str]]:
        ...
        return {
            "1742-6": ["HP:0002910", "HP:0001410"],  # ALT -> Elevated hepatic transaminase, Decreased liver function
            "1751-7": ["HP:0003073"],  # Albumin -> Hypoalbuminemia
            "6768-6": ["HP:0002910"],  # Alkaline Phosphatase -> Elevated hepatic transaminase
            "2345-7": ["HP:0003074", "HP:0003076"],  # Glucose -> Hyperglycemia, Glycosuria
            "2160-0": ["HP:0001900", "HP:0003565"],  # Creatinine -> Increased serum creatinine
            "3094-0": ["HP:0002153", "HP:0003259"],  # BUN -> Hyperkalemia, Elevated serum creatinine
            "2823-3": ["HP:0003362", "HP:0002153"],  # Potassium -> Hypokalemia, Hyperkalemia
            "2951-2": ["HP:0002900", "HP:0002901"],  # Sodium -> Hyponatremia, Hypernatremia
            "718-7": ["HP:0001871", "HP:0001873"],   # Hemoglobin -> Abnormality of blood hemoglobin
            "1759-0": ["HP:0003073", "HP:0012211"],  # A/G Ratio -> Hypoalbuminemia, Abnormal albumin/globulin ratio
        }
    ...
    def _semantic_match_to_phenotype(
        ...
        test_keywords = set(chemistry_name.lower().split() + display_name.lower().split())
        ...
        for _, phenotype_row in target_df.iterrows():
            ...
            phenotype_keywords = set(phenotype_name.split() + phenotype_desc.split()[:20])
    ...
    def _rule_based_mapping(
        ...
        rules = {
            'glucose': ['HP:0003074', 'HP:0011015'],  # Hyperglycemia, Abnormality of blood glucose
            'cholesterol': ['HP:0003124', 'HP:0003107'],  # Hypercholesterolemia, Abnormal cholesterol
            'triglyceride': ['HP:0002155'],  # Hypertriglyceridemia
            'hemoglobin': ['HP:0001871', 'HP:0011902'],  # Abnormality of hemoglobin
            'creatinine': ['HP:0003259', 'HP:0000083'],  # Elevated creatinine, Renal insufficiency
            'bilirubin': ['HP:0002904', 'HP:0000952'],  # Hyperbilirubinemia, Jaundice
            'calcium': ['HP:0002901', 'HP:0002900'],  # Hypercalcemia, Hypocalcemia
            'iron': ['HP:0011031', 'HP:0011032'],  # Abnormality of iron, Iron deficiency
        }
