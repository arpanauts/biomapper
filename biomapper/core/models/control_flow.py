...
class Condition(BaseModel):
    ...
    type: ConditionType = Field(
        default=ConditionType.SIMPLE, description="Type of condition evaluation"
    )
    ...
    all: Optional[List[Union[str, "Condition"]]] = Field(
        None, description="List of conditions that must all be true (AND logic)"
    )
    any: Optional[List[Union[str, "Condition"]]] = Field(
        None, description="List of conditions where any must be true (OR logic)"
    )
    ...
    def validate_condition_structure(self) -> "Condition":
        ...
        if self.type == ConditionType.SIMPLE:
            ...
            if self.expression or self.any:
                raise ValueError(
                    "ALL condition cannot have 'expression' or 'any' fields"
                )
            ...
            if self.expression or self.all:
                raise ValueError(
                    "ANY condition cannot have 'expression' or 'all' fields"
                )
        return self
...
class ErrorHandling(BaseModel):
    ...
    action: ErrorAction = Field(..., description="Action to take when error occurs")
    max_attempts: int = Field(
        3, ge=1, le=10, description="Maximum number of retry attempts"
    )
    backoff: BackoffStrategy = Field(
        BackoffStrategy.LINEAR, description="Backoff strategy for retries"
    )
    delay: int = Field(
        5, ge=1, le=300, description="Base delay in seconds between retries"
    )
    fallback: Optional[Dict[str, Any]] = Field(
        None, description="Fallback action if all retries fail"
    )
    message: Optional[str] = Field(None, description="Custom error message to log")
    set_variable: Optional[str] = Field(
        None, description="Variable to set on error (e.g., 'step_failed=true')"
    )
...
class LoopConfig(BaseModel):
    ...
    max_iterations: Optional[int] = Field(
        None, ge=1, le=1000, description="Maximum number of iterations"
    )
    ...
    def validate_loop_type(self) -> "LoopConfig":
        ...
        if self.items is not None and (self.max_iterations or self.while_condition):
            raise ValueError(
                "Cannot mix for_each (items) with repeat (max_iterations/while_condition)"
            )
        if self.items is None and not (self.max_iterations or self.while_condition):
            raise ValueError(
                "Loop must have either 'items' for for_each or 'max_iterations'/'while_condition' for repeat"
            )
        return self
...
class ParallelConfig(BaseModel):
    ...
    max_workers: int = Field(
        3, ge=1, le=10, description="Maximum number of parallel workers"
    )
    fail_fast: bool = Field(False, description="Stop all parallel tasks if one fails")
    timeout: Optional[int] = Field(
        None, ge=1, le=3600, description="Timeout in seconds for parallel execution"
    )
...
class GlobalErrorHandling(BaseModel):
    ...
    default: ErrorAction = Field(
        ErrorAction.STOP, description="Default error action for all steps"
    )
    max_retries: int = Field(
        3, ge=1, le=10, description="Default maximum retries for all steps"
    )
    retry_delay: int = Field(
        5, ge=1, le=300, description="Default retry delay in seconds"
    )
    continue_on_error: bool = Field(
        False, description="Whether to continue execution on unhandled errors"
    )
...
class CheckpointingConfig(BaseModel):
    ...
    enabled: bool = Field(False, description="Whether checkpointing is enabled")
    strategy: str = Field(
    ...
    path: Optional[str] = Field(
        None, description="Path for checkpoint storage (local) or bucket (s3)"
    )
...
class ExecutionConfig(BaseModel):
    ...
    mode: ExecutionMode = Field(
        ExecutionMode.SEQUENTIAL, description="Execution mode for steps"
    )
    parallel_default: Optional[ParallelConfig] = Field(
        None, description="Default parallel configuration when mode is DAG"
    )
    timeout: Optional[int] = Field(
        None,
        ge=1,
    ...
    dry_run: bool = Field(
        False, description="Whether to run in dry-run mode (no actual execution)"
    )
...
class EnhancedStepDefinition(BaseModel):
    ...
    name: str = Field(
        ..., pattern="^[a-zA-Z_][a-zA-Z0-9_]*$", description="Unique step name"
    )
    ...
    condition: Optional[Union[str, Condition]] = Field(
        None, description="Condition for step execution"
    )
    on_error: Optional[Union[ErrorAction, ErrorHandling]] = Field(
        None, description="Error handling for this step"
    )

    # Loops
    for_each: Optional[LoopConfig] = Field(
        None, description="For-each loop configuration"
    )
    repeat: Optional[LoopConfig] = Field(None, description="Repeat loop configuration")
    ...
    depends_on: Optional[List[str]] = Field(
        None, description="Step names this step depends on"
    )

    # Checkpointing
    checkpoint: Optional[CheckpointTiming] = Field(
        None, description="When to create checkpoint for this step"
    )

    # Variable manipulation
    set_variables: Optional[Dict[str, Any]] = Field(
        None, description="Variables to set after step execution"
    )

    # Step control
    is_critical: bool = Field(
        False, description="Whether this step is critical (failure stops execution)"
    )
    skip_if_exists: Optional[str] = Field(
        None, description="Skip step if this variable/file exists"
    )
    timeout: Optional[int] = Field(
        None, ge=1, le=3600, description="Timeout for this step in seconds"
    )
    ...
    def validate_step_configuration(self) -> "EnhancedStepDefinition":
        ...
        if self.for_each and self.repeat:
            raise ValueError(
                "Step cannot have both 'for_each' and 'repeat' configurations"
            )
