...
from ..models.control_flow import (
    ErrorAction, BackoffStrategy, ExecutionMode,
    CheckpointTiming, ParallelConfig
)
...
class ControlFlowExecutor:
    ...
    def _initialize_context(self, initial_context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """Initialize execution context."""
        ...
        context['execution'] = {
            ...
            'dry_run': self.strategy.execution.dry_run if self.strategy.execution else False
        }
                ...
                for condition in self.strategy.pre_conditions:
                    if not self.evaluator.evaluate_condition(condition):
                        raise StepExecutionError(
                            "pre_conditions",
                            f"Pre-condition not met: {condition}"
                        )
            
            # Execute based on mode
            if self.strategy.execution and self.strategy.execution.mode == ExecutionMode.DAG:
    ...
    async def _process_step(self, step: EnhancedStepDefinition) -> None:
        ...
        if step.skip_if_exists:
            if self._check_skip_condition(step.skip_if_exists):
                logger.info(f"Skipping step '{step.name}' - condition met: {step.skip_if_exists}")
    ...
    async def _execute_step_with_error_handling(self, step: EnhancedStepDefinition) -> Any:
        ...
        if isinstance(error_config, str):
            ...
            error_action = error_config.action if hasattr(error_config, 'action') else ErrorAction.STOP
        ...
        while attempts < max_attempts:
            try:
                ...
                attempts += 1
                last_error = e
                logger.error(f"Step '{step.name}' failed (attempt {attempts}/{max_attempts}): {e}")
                ...
                if error_action == ErrorAction.STOP or attempts >= max_attempts:
                    ...
                    if hasattr(error_config, 'set_variable') and error_config.set_variable:
        ...
        raise StepExecutionError(step.name, f"Failed after {max_attempts} attempts", last_error)
        ...
        try:
            if step.timeout:
                result = await asyncio.wait_for(
                    self._execute_action(action, resolved_params),
            ...
            raise StepExecutionError(step.name, f"Step timed out after {step.timeout} seconds")
        except Exception as e:
            self.context['steps'][step.name] = {
                'error': str(e),
                'executed_at': datetime.now().isoformat(),
    ...
    async def _execute_foreach(self, step: EnhancedStepDefinition) -> None:
        ...
        logger.info(f"Executing for_each loop for step '{step.name}' with {len(items)} items")
        ...
        while iterations < max_iterations:
            ...
            if step.repeat.while_condition:
                if not self.evaluator.evaluate_condition(step.repeat.while_condition):
                    logger.info(f"Repeat loop condition no longer met after {iterations} iterations")
                    break
    ...
    async def _execute_parallel_foreach(self, step: EnhancedStepDefinition) -> None:
        ...
        logger.info(f"Executing parallel for_each for step '{step.name}' with {len(items)} items, max workers: {max_workers}")
    ...
    def _topological_levels(self, graph: Dict[str, Set[str]]) -> List[List[str]]:
        ...
        if sum(in_degree.values()) > 0:
            raise StepExecutionError("dag", "Circular dependency detected in step dependencies")
    ...
    def _resolve_parameters(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in params.items():
            if isinstance(value, str) and '${' in value:
                ...
                resolved[key] = [
                    self.evaluator.evaluator.evaluate(item) if isinstance(item, str) and '${' in item else item
                    for item in value
                ]
            else:
                resolved[key] = value
    ...
    async def _create_checkpoint(self, checkpoint_name: str) -> None:
        ...
        checkpoint_path = self.checkpoint_dir / f"{self.strategy.name}_{checkpoint_name}_{int(time.time())}.json"
