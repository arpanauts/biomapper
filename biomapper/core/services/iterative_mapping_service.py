...
from ...db.models import MappingPath
...
class IterativeMappingService:
    ...
    async def perform_iterative_mapping(
        ...
        self.logger.info(
            f"Starting iterative mapping for {len(unmapped_ids)} unmapped identifiers"
        )

        # --- Step 1: Find and prioritize available secondary ontology types ---
        all_properties = await mapping_executor._get_endpoint_properties(
            meta_session, source_endpoint_name
        )
        ...
        preferences = await mapping_executor._get_ontology_preferences(
            meta_session, source_endpoint_name
        )
        ...
        if preferences:
            ...
            secondary_properties.sort(
                key=lambda prop: priority_map.get(prop.ontology_type, 999)
            )
            self.logger.info(
                f"Sorted {len(secondary_properties)} secondary properties by endpoint preference priority."
            )
        else:
            self.logger.info(
                f"No ontology preferences found for '{source_endpoint_name}'. Using default property order."
            )
        ...
        for secondary_prop in secondary_properties:
            ...
            unmapped_ids_without_derived = [
                uid for uid in unmapped_ids if uid not in derived_primary_ids
            ]
            if not unmapped_ids_without_derived:
                self.logger.info(
                    "All unmapped identifiers now have derived primary IDs. Skipping remaining secondary properties."
                )
                break
            ...
            self.logger.info(
                f"Processing secondary property '{secondary_source_property_name}' with ontology type '{secondary_source_ontology}'"
            )
            self.logger.info(
                f"Remaining unmapped entities without derived primaries: {len(unmapped_ids_without_derived)}"
            )
            ...
            self.logger.info(
                f"Found secondary-to-primary path: {secondary_to_primary_path.name} (ID: {secondary_to_primary_path.id})"
            )
            self.logger.info(
                f"Executing secondary-to-primary conversion for {len(unmapped_ids_without_derived)} identifiers."
            )
        ...
        if not derived_primary_ids:
            self.logger.info(
                "No derived primary IDs available. Skipping re-mapping step."
            )
            return successful_mappings, processed_ids, derived_primary_ids

        self.logger.info(
            f"Re-attempting primary mapping using derived IDs for {len(derived_primary_ids)} entities."
        )
        ...
        for source_id, derived_data in derived_primary_ids.items():
            ...
            for derived_primary_id in derived_primary_id_list:
                self.logger.debug(
                    f"Attempting mapping for {source_id} using derived primary ID {derived_primary_id}"
                )
                ...
                if use_cache:
                    ...
                    (
                        cache_results_for_derived,
                        _,
                    ) = await mapping_executor.cache_manager.check_cache(
                    ...
                    if (
                        cache_results_for_derived
                        and derived_primary_id in cache_results_for_derived
                    ):
                        cached_derived_mapping = cache_results_for_derived[
                            derived_primary_id
                        ]
                        self.logger.info(
                ...
                if cached_derived_mapping:
                    derived_mapping_results = {
                        derived_primary_id: cached_derived_mapping
                    }
                else:
                    self.logger.debug(
                        f"Cache miss or not used for derived ID {derived_primary_id}. Executing primary_path."
                    )
                ...
                if (
                    derived_mapping_results
                    and derived_primary_id in derived_mapping_results
                ):
                    ...
                    if result_data and result_data.get("target_identifiers"):
                        source_result = {
                            ...
                            * 0.9,  # Lower confidence for indirect
                            "hop_count": (
                                result_data.get("hop_count", 0) + 1
                                if result_data.get("hop_count") is not None
                                else 2
                            ),
                            "mapping_direction": result_data.get(
                                "mapping_direction", "forward"
                            ),
                            "derived_path": True,
                            "intermediate_id": derived_primary_id,
                            "mapping_path_details": result_data.get(
                                "mapping_path_details"
                            ),
                        }

                        # Enrich path details with derivation provenance
                        current_path_details_str = source_result.get(
                            "mapping_path_details"
                        )
                        new_path_details = self._parse_path_details(
                            current_path_details_str
                        )
                        new_path_details["derived_step_provenance"] = provenance_info
                        source_result["mapping_path_details"] = json.dumps(
                            new_path_details
                        )
        ...
        # Log summary
        newly_mapped = len(
            [sid for sid in derived_primary_ids.keys() if sid in processed_ids]
        )
        self.logger.info(
    ...
    def _parse_path_details(self, path_details_str: Any) -> Dict:
        ...
        if isinstance(path_details_str, str):
            try:
                ...
                self.logger.warning(
                    f"Could not parse path_details JSON: {path_details_str}"
                )
            ...
            self.logger.warning(
                f"Unexpected type for path_details: {type(path_details_str)}. Storing as string."
            )
