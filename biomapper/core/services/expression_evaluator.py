...
class SafeExpressionEvaluator:
    ...
    SAFE_METHODS = {
        ...
        'has_file': lambda ctx, path: ctx.get('file_system', {}).get('exists', lambda p: False)(path),
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Attribute):
                if node.attr.startswith('_'):
                    raise ExpressionError(f"Private attribute access not allowed: {node.attr}")
    ...
    def evaluate_condition(self, condition: Union[str, Dict[str, Any]]) -> bool:
        ...
        if isinstance(condition, str):
            ...
            if condition_type == 'simple':
                ...
                return all(
                    self.evaluate_condition(c) for c in all_conditions
                )
                ...
                return any(
                    self.evaluate_condition(c) for c in any_conditions
                )
