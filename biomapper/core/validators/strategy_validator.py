...
class StrategyValidator:
    ...
    ACTION_SCHEMAS: Dict[str, Dict[str, List[str]]] = {
        ...
        "POPULATE_CONTEXT": {"required": ["context_updates"], "optional": []},
    ...
    def validate_action(self, action: StepAction, prefix: str) -> List[str]:
        ...
        param_errors = self.validate_action_parameters(
            action.type, action.params or {}, prefix
        )
    ...
    def validate_action_parameters(
        self, action_type: str, params: Dict[str, Any], prefix: str
    ) -> List[str]:
        """
        ...
        for required_param in schema.get("required", []):
            if required_param not in params:
                errors.append(
                    f"{prefix}: Missing required parameter '{required_param}'"
                )
    ...
    def validate_yaml_file(
        cls, yaml_path: Path, strict: bool = True
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def validate_yaml_string(
        cls, yaml_content: str, strict: bool = True
    ) -> Tuple[bool, List[str]]:
        """
