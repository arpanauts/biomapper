...
class SPOKEConfig(BaseModel):
    ...
    base_url: str = Field(
        "https://spoke.rbvi.ucsf.edu/api/v1", description="SPOKE API base URL"
    )
...
class SPOKEEntity(BaseModel):
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Mapping confidence score"
    )
    source: str = Field(
        default="direct", description="Source of the mapping (direct/inferred)"
    )
...
class SPOKERelation(BaseModel):
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Relationship confidence score"
    )
...
class AQLQuery(BaseModel):
    ...
    expected_node_types: List[str] = Field(
        default_factory=list, description="Expected node types in results"
    )
...
class SPOKEMappingResult(BaseModel):
    ...
    unmapped_entities: List[str] = Field(
        default_factory=list, description="Entities that couldn't be mapped"
    )
    mapping_sources: Set[str] = Field(
        default_factory=set, description="Sources used for mapping"
    )
...
class BaseSPOKEMapper(ABC):
    ...
    async def map_to_spoke(
        self, entities: List[str], entity_type: Optional[str] = None
    ) -> SPOKEMappingResult:
