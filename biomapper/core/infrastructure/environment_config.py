...
class EnvironmentConfig:
    ...
    def from_environment(cls) -> "EnvironmentConfig":
        ...
        return cls(
            ...
            enable_vector_fallback=os.getenv("ENABLE_VECTOR_FALLBACK", "true").lower()
            == "true",
            enable_api_fallbacks=os.getenv("ENABLE_API_FALLBACKS", "true").lower()
            == "true",
            enable_file_path_fallbacks=os.getenv(
                "ENABLE_FILE_PATH_FALLBACKS", "true"
            ).lower()
            == "true",
            validate_file_paths=os.getenv("VALIDATE_FILE_PATHS", "true").lower()
            == "true",
            validate_parameters=os.getenv("VALIDATE_PARAMETERS", "true").lower()
            == "true",
    ...
    def create_directories(self) -> None:
        ...
        directories = [self.data_dir, self.cache_dir, self.output_dir, self.log_dir]
...
class EnvironmentManager:
    ...
    def get_service_config(self, service_name: str) -> Dict[str, Any]:
        ...
        service_configs = {
            ...
            "logging": {"directory": self.config.log_dir, "level": "INFO"},
        }
        ...
        if service_name == "qdrant":
            ...
            try:
                ...
                result = sock.connect_ex(
                    (self.config.qdrant_host, self.config.qdrant_port)
                )
                sock.close()
                return result == 0
            except Exception:
