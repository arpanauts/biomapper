...
class PathResolver:
    ...
    def get_safe_output_path(self, path_str: str) -> Path:
        ...
        try:
            ...
            self.logger.error(f"Could not create safe output path for '{path_str}': {e}")
...
class EnhancedStrategyLoader:
    ...
    def load_strategy(self, strategy_name: str, validate: bool = True) -> Dict[str, Any]:
        ...
        if not strategy_file:
            raise StrategyLoadError(f"Strategy '{strategy_name}' not found in {self.strategies_dir}")
        ...
        try:
            ...
            if not raw_content:
                raise StrategyLoadError(f"Strategy file '{strategy_file}' is empty or invalid")
            ...
            try:
                ...
                self.logger.error(f"Parameter resolution failed for strategy '{strategy_name}': {e}")
                raise ParameterResolutionError(f"Failed to resolve parameters in strategy '{strategy_name}': {e}")
    ...
    def list_available_strategies(self) -> List[Dict[str, str]]:
        ...
        for strategy_file in self.strategies_dir.rglob("*.yaml"):
            try:
                ...
                self.logger.warning(f"Could not read strategy file {strategy_file}: {e}")
    ...
    def _find_strategy_file(self, strategy_name: str) -> Optional[Path]:
        ...
        if not self.strategies_dir.exists():
            self.logger.error(f"Strategies directory does not exist: {self.strategies_dir}")
            return None
        ...
        for i, step in enumerate(steps):
            ...
            for param_ref in param_refs:
                if param_ref not in parameters:
                    validation_errors.append(f"Step {i} references undefined parameter: {param_ref}")
        
        if validation_errors:
            error_msg = f"Strategy validation failed:\n" + "\n".join(f"  - {error}" for error in validation_errors)
            raise StrategyValidationError(error_msg)


# Convenience functions
def load_strategy_with_resolution(strategy_name: str, strategies_dir: Optional[str] = None) -> Dict[str, Any]:
