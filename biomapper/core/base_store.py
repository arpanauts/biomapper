...
class BaseVectorStore(Generic[T], ABC):
    ...
    async def add_documents(
        self, documents: List[T], embeddings: Optional[List[np.ndarray]] = None
    ) -> None:
...
class BaseDocumentStore(Generic[T], ABC):
    ...
    @abstractmethod
    async def get(self, document_ids: List[str]) -> List[Optional[T]]:
        """Get documents by ID.
    ...
    @abstractmethod
    async def search(
        self, query: Dict[str, Any], limit: int = 10, offset: int = 0
    ) -> List[T]:
