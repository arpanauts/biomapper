...
class CacheConfig:
    ...
    # Database settings
    data_dir: str = field(default_factory=lambda: os.path.join(Path.home(), ".biomapper", "data"))
    ...
    entity_type_priority: List[str] = field(default_factory=lambda: [
        "chebi", "hmdb", "pubchem.compound", "kegg", "inchikey", "smiles", "cas"
    ])
    
    # Expiration time overrides by entity type (in days)
    entity_ttl_overrides: Dict[str, int] = field(default_factory=lambda: {
    ...
    def load(cls, file_path: Optional[str] = None) -> "CacheConfig":
        ...
        """
        if not file_path:
            default_path = os.path.join(Path.home(), ".biomapper", "config", "cache_config.json")
...
def get_default_config() -> CacheConfig:
    ...
    if "BIOMAPPER_CACHE_CONFIDENCE" in os.environ:
        try:
            config.confidence_threshold = float(os.environ["BIOMAPPER_CACHE_CONFIDENCE"])
        except ValueError:
            pass
