...
class CacheManager:
    ...
    def bidirectional_lookup(
        ...
        with self._session_scope() as session:
            # Query as source
            source_query = session.query(EntityMapping).filter(
            ...
            # Query as target
            target_query = session.query(EntityMapping).filter(
            ...
            if target_type:
                ...
                target_query = target_query.filter(
                    EntityMapping.source_type == target_type
                )
    ...
    def add_mapping(
        ...
        with self._session_scope() as session:
            ...
            if existing:
                ...
                existing.expires_at = datetime.datetime.utcnow() + datetime.timedelta(
                    days=ttl_days
                )
                ...
                if metadata:
                    ...
                    for key, value in metadata.items():
                        meta = MappingMetadata(
                            mapping_id=existing.id, key=key, value=str(value)
                        )
                        session.add(meta)
                ...
                if metadata:
                    for key, value in metadata.items():
                        meta = MappingMetadata(
                            mapping_id=new_mapping.id, key=key, value=str(value)
                        )
                        session.add(meta)
                ...
                # Update statistics
                self._update_stats(session, transitive_derivation=is_derived)
            ...
            if bidirectional and source_id != target_id:
                ...
                if reverse_existing:
                    ...
                    reverse_ttl = self._get_ttl(
                        session, reverse_source_type, reverse_target_type
                    )
                    reverse_expires = datetime.datetime.utcnow() + datetime.timedelta(
                        days=reverse_ttl
                    )
                    ...
                    if metadata and reverse_mapping.id:
                        for key, value in metadata.items():
                            meta = MappingMetadata(
                                mapping_id=reverse_mapping.id, key=key, value=str(value)
                            )
                            session.add(meta)
        ...
        with self._session_scope() as session:
            ...
            if config:
                ...
                config = EntityTypeConfig(
                    source_type=source_type, target_type=target_type, ttl_days=ttl_days
                )
    ...
    def bulk_add_mappings(
        self, mappings: List[Dict[str, Any]], bidirectional: bool = True
    ) -> int:
    ...
    def delete_expired_mappings(self) -> int:
        ...
        with self._session_scope() as session:
            ...
            query = session.query(EntityMapping).filter(EntityMapping.expires_at < now)
            count = query.count()
            query.delete()
