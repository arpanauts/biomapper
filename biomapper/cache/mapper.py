...
class CachedMapper(BaseMapper[T]):
    ...
    async def batch_map(
        ...
        if cache_misses:
            logger.debug(f"Cache miss for {len(cache_misses)} out of {len(texts)} items")
    ...
    async def _map_with_api(
        ...
        if self.track_api_usage:
            self.cache_manager._update_stats(
                self.cache_manager._session_scope().__enter__(),
                api_call=True
            )
    ...
    def _add_to_cache(self, text: str, result: MappingResult) -> None:
        ...
        metadata = {}
        if result.metadata:
            metadata = {k: str(v) for k, v in result.metadata.items() if k != "cache_hit"}
