        ...
        try:
            ...
            if direction in (SyncDirection.SPOKE_TO_CACHE, SyncDirection.BIDIRECTIONAL):
                with track_cache_operation(
                    ...
                    spoke_type = self.config.cache_to_spoke_type_map.get(entity_type, entity_type)
                ...
                for mapping in spoke_mappings:
                    ...
                    confidence = mapping.get("confidence", self.config.default_confidence)
                    ...
                    try:
                        ...
                        results["errors"].append(f"Error adding mapping to cache: {str(e)}")
                ...
                for mapping in cache_mappings:
                    ...
                    try:
                        ...
                        with track_cache_operation(
                            operation_type=CacheEventType.API_CALL,
                            entity_type=entity_type,
                            metadata={"target": "spoke", "operation": "add_entity_mapping"}
                        ):
                        ...
                        results["errors"].append(f"Error adding mapping to SPOKE: {str(e)}")
    ...
    async def sync_all_entities(
        ...
        try:
            ...
            if self.config.excluded_entity_types:
                entity_types = [t for t in entity_types if t not in self.config.excluded_entity_types]
            ...
            for entity_type in entity_types:
                try:
                    ...
                    logger.info(f"Found {len(entities)} entities of type {spoke_type} in SPOKE")
                    ...
                    for i in range(0, len(entities), self.config.sync_batch_size):
                        ...
                        for entity in batch:
                            ...
                            results["total_mappings_added_to_cache"] += entity_result["mappings_added_to_cache"]
                            results["total_mappings_added_to_spoke"] += entity_result["mappings_added_to_spoke"]
                    ...
                    results["errors"].append(f"Error processing entity type {entity_type}: {str(e)}")
        ...
        try:
            ...
            with self.cache_manager._session_scope() as session:
                ...
                query = session.query(EntityMapping).filter(
                    ...
                    EntityMapping.mapping_source != "spoke",  # Don't sync mappings from SPOKE
                )
                ...
                for mapping in query.yield_per(self.config.sync_batch_size):
                    try:
                        ...
                        error_msg = (f"Error syncing mapping {mapping.source_type}:{mapping.source_id} -> "
...
async def sync_entities_from_list(
    ...
    try:
        ...
        for entity in entities:
            ...
            results["mappings_added_to_cache"] += entity_result["mappings_added_to_cache"]
            results["mappings_added_to_spoke"] += entity_result["mappings_added_to_spoke"]
        ...
        # Build transitive relationships if requested
        if build_transitive:
            transitive_count = sync_manager.transitivity_builder.build_transitive_mappings()
            results["transitive_mappings_created"] = transitive_count
