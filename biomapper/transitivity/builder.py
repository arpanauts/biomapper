...
class TransitivityBuilder:
    ...
    def _find_candidate_pairs(self, session: Session) -> List[Tuple[int, int]]:
        ...
        target_to_mapping: Dict[Tuple[str, str], List[EntityMapping]] = defaultdict(list)
        source_to_mapping: Dict[Tuple[str, str], List[EntityMapping]] = defaultdict(list)
        ...
        for mapping1 in mappings:
            ...
            for mapping2 in source_to_mapping.get(mapping1_target, []):
                ...
                if mapping1.source_id == mapping2.target_id and mapping1.source_type == mapping2.target_type:
                    continue
                
                # Skip if the pair would create a mapping that already exists
                existing = False
                source_target_key = ((mapping1.source_id, mapping1.source_type), 
    ...
    def _derive_mapping(
        self, 
        session: Session, 
        mapping_chain: List[EntityMapping]
