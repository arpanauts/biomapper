...
class PipelineMappingResult(BaseModel):
    ...
    rationale: Optional[str] = Field(
        default=None, description="Explanation for the mapping decision or lack thereof"
    )

    # Intermediate results from pipeline stages
    qdrant_results: Optional[List[QdrantSearchResultItem]] = Field(
        default=None, description="Raw results from Qdrant similarity search"
    )
    pubchem_annotations: Optional[Dict[int, PubChemAnnotation]] = Field(
        default=None, description="PubChem annotations keyed by CID"
    )
    ...
    error_message: Optional[str] = Field(
        default=None, description="Detailed error message if any component failed"
    )
    ...
    def summary(self) -> str:
        ...
        if self.is_successful() and self.has_mapping():
            ...
            return (
                f"Failed to map '{self.input_biochemical_name}': {self.error_message}"
            )
        else:
