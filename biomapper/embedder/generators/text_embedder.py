...
try:
    ...
    logging.warning("sentence-transformers package not found. TextEmbedder will use mock embeddings.")
...
class TextEmbedder(BaseEmbedder):
    ...
    def _initialize(self) -> bool:
        ...
        if self.init_attempts > self.max_init_attempts:
            logging.error(f"Failed to initialize embedding model after {self.max_init_attempts} attempts")
            return False
            
        try:
            if not HAS_SENTENCE_TRANSFORMERS:
                logging.warning("Using mock embeddings - install sentence-transformers for real embeddings")
