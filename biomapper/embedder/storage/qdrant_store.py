...
try:
    ...
    HAS_QDRANT = False
    logging.warning(
        "qdrant-client package not found. QdrantVectorStore will not work correctly."
    )
...
class QdrantVectorStore(BaseVectorStore):
    ...
    def _initialize(self):
        ...
        if not HAS_QDRANT:
            logging.error(
                "Qdrant client is not available. Please install qdrant-client."
            )
            self.client = None
            return
        ...
        try:
            ...
            if self.url:
                ...
                storage_path = self.local_path or os.path.join(
                    default_config.storage_dir, "qdrant"
                )
    ...
    def add(self, embeddings: np.ndarray, metadata: List[Dict[str, Any]]) -> List[str]:
        ...
        with self.lock:
            try:
                ...
                logging.info(
                    f"Added {len(points)} embeddings to Qdrant collection '{self.collection_name}'"
                )
                return ids
    ...
    def delete(self, ids: List[str]) -> bool:
        ...
        with self.lock:
            try:
                ...
                logging.info(
                    f"Deleted {len(ids)} embeddings from Qdrant collection '{self.collection_name}'"
                )
                return True
    ...
    def filter_search(
        self, query_vector: np.ndarray, filter_conditions: Dict[str, Any], k: int = 10
    ) -> List[Dict[str, Any]]:
                    ...
                    if isinstance(value, list):
                        ...
                        filter_parts.append(
                            models.FieldCondition(
                                key=field_path, match=models.MatchAny(any=value)
                            )
                        )
                    else:
                        # Exact match filter
                        filter_parts.append(
                            models.FieldCondition(
                                key=field_path, match=models.MatchValue(value=value)
                            )
                        )

                # Combine filters with AND logic
                if filter_parts:
                    filter_condition = models.Filter(must=filter_parts)
                else:
    ...
    def create_payload_index(self, field: str) -> bool:
        ...
        try:
            ...
            logging.info(
                f"Created index on field '{field_path}' in collection '{self.collection_name}'"
            )
            return True
