    ...
    def process_items(self, items: List[EmbedderItem]) -> Dict[str, Any]:
        ...
        if self.stats["duration"] > 0:
            self.stats["items_per_second"] = (
                self.stats["processed"] / self.stats["duration"]
            )
            logging.info(
                f"Processing rate: {self.stats['items_per_second']:.2f} items/second"
            )

        return self.stats

    def process_from_jsonl(
        self, jsonl_path: str, max_items: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.stats["batches"] % 10 == 0:
                ...
                if elapsed > 0:
                    ...
                    logging.info(
                        f"Processing batch {self.stats['batches']} - {rate:.2f} items/sec"
                    )
