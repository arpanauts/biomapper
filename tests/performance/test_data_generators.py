    ...
    for _ in range(size):
        ...
        for _ in range(num_refs):
            ...
            if ref_type == "GO":
                ...
                refs.append(
                    f"KEGG:{random.choice(['hsa', 'mmu', 'rno'])}{random.randint(1000, 99999)}"
                )
...
def generate_clinical_test_names(
    size: int, vendor_variations: bool = True
) -> List[str]:
        ...
        return pd.DataFrame(
            {
                ...
                "vendor": np.random.choice(
                    vendors, size, p=[0.25, 0.25, 0.15, 0.15, 0.1, 0.1]
                ),
            }
        )
...
def save_test_data(
    entity_type: str, size: int, output_dir: str = "/tmp/integration_test_data"
):
