...
class BiomapperAPIClient:
    ...
    def execute_strategy(
        self, strategy_name: str, parameters: Dict[str, Any], timeout: int = 300
    ):
        """Execute a strategy and return results."""
        # Start execution
        response = self.session.post(
            f"{self.base_url}/api/strategies/v2/execute",
            json={"strategy": strategy_name, "parameters": parameters},
        )
...
class TestCompleteStrategiesEnhanced:
    ...
    def validate_protein_mapping_quality(self, result_data: Dict) -> QualityMetrics:
        ...
        if "uniprot_id" in df.columns:
            ...
            uniprot_success = (
                df["uniprot_id"].str.match(r"^[A-Z0-9]{6}$", na=False).sum()
            )
        ...
        if "xrefs_processed" in df.columns:
            xrefs_processed = (
                df["xrefs_processed"].sum()
                if df["xrefs_processed"].notna().any()
                else 0
            )
        ...
        coverage_metrics = {
            ...
            "xrefs_processed_rate": xrefs_processed / total_rows
            if total_rows > 0
            else 0,
        }
    ...
    def validate_metabolite_mapping_quality(self, result_data: Dict) -> QualityMetrics:
        ...
        if "inchikey" in df.columns:
            ...
            inchikey_coverage = (
                df["inchikey"].str.match(r"^[A-Z]{14}-[A-Z]{10}-[A-Z]$", na=False).sum()
            )

        if "cts_mapped" in df.columns:
            cts_success = (
                df["cts_mapped"].sum() if df["cts_mapped"].notna().any() else 0
            )
        ...
        coverage_metrics = {
            ...
            "inchikey_coverage": inchikey_coverage / total_rows
            if total_rows > 0
            else 0,
    ...
    def validate_chemistry_mapping_quality(self, result_data: Dict) -> QualityMetrics:
        ...
        if "loinc_code" in df.columns:
            ...
            loinc_success = (
                df["loinc_code"].str.match(r"^\d{1,5}-\d{1}$", na=False).sum()
            )

        if "fuzzy_matched" in df.columns:
            fuzzy_matches = (
                df["fuzzy_matched"].sum() if df["fuzzy_matched"].notna().any() else 0
            )
        ...
        coverage_metrics = {
            "loinc_extraction_rate": loinc_success / total_rows
            if total_rows > 0
            else 0,
    ...
    def get_available_strategies(self) -> Dict[str, List[str]]:
        ...
        strategies_dir = Path("/home/ubuntu/biomapper/src/configs/strategies")
        ...
        for yaml_file in strategies_dir.glob("**/*.yaml"):
            ...
            if "prot" in strategy_name.lower():
                ...
                term in strategy_name.lower()
                for term in ["met", "metabol", "nightingale", "nmr"]
            ):
                ...
                if "protein" in strategy_name.lower():
                    ...
                    "metabolite" in strategy_name.lower()
                    or "metabolomics" in strategy_name.lower()
                ):
                    metabolite_strategies.append(strategy_name)
                else:
                    # For multi-type or unclear strategies, add to all categories
                    if "multi" in strategy_name.lower():
                        metabolite_strategies.append(
                            strategy_name
                        )  # Most multi strategies are metabolite-focused
                    else:
                        metabolite_strategies.append(
                            strategy_name
                        )  # Default to metabolite
    ...
    @pytest.mark.skip(reason="Requires API server running and complete action implementations")
    def test_all_strategies_execution(self, api_client, test_data_cache):
            ...
            for strategy_name in strategy_list[:3]:  # Test first 3 of each type
                for dataset_size in test_sizes:
                    ...
                    if result.status == "passed":
                        ...
                        print(
                            f"    Execution time: {result.performance.execution_time:.1f}s"
                        )
        ...
        if len(results) > 0:
            ...
            assert (
                passed / len(results) > 0.5
            ), f"Too many test failures: {passed}/{len(results)}"
        else:
            print("  Success rate: No tests run")
            # If no strategies found, that's also a failure condition
            assert False, "No strategies found to test"
    ...
    def test_scalability_benchmark(self, api_client, test_data_cache):
        ...
        for entity_type, strategy_name in test_strategies.items():
            ...
            for size in test_sizes:
                ...
                result = self.run_strategy_test(
                    strategy_name, entity_type, size, api_client, test_data_cache
                )
                ...
                if result.status == "passed":
                    ...
                    print(
                        f"    Throughput: {result.performance.throughput_rows_per_second:.1f} rows/s"
                    )
                else:
                    print(f"    Status: {result.status}")
        ...
        for entity_type, entity_results in scalability_results.items():
            passed_results = {
                k: v for k, v in entity_results.items() if v.status == "passed"
            }
    ...
    def test_memory_efficiency(self, api_client, test_data_cache):
        ...
        if test_result.status == "passed":
            memory_per_row = (
                test_result.performance.peak_memory_mb / large_size * 1000
            )  # KB per row
            ...
            assert (
                memory_per_row < 50
            ), f"Excessive memory usage: {memory_per_row:.2f}KB per row"

            # Assert total memory under reasonable limit
            assert (
                test_result.performance.peak_memory_mb < 2048
            ), f"Peak memory too high: {test_result.performance.peak_memory_mb}MB"
