...
class TestFAISSVectorStore:
    ...
    def test_init_custom_parameters(self):
        ...
        store = FAISSVectorStore(
            dimension=512, normalize=False, index_type="HNSW", metric="IP"
        )
    ...
    def test_search_with_filter(self, sample_embeddings):
        ...
        metadata = [{"category": "A" if i % 2 == 0 else "B"} for i in range(len(ids))]
        ...
        results = store.search(
            query, k=5, filter_func=lambda m: m.get("category") == "A"
        )
    ...
    def test_save_and_load(self, temp_dir, sample_embeddings):
        ...
        # Load into new store
        store2 = FAISSVectorStore(index_path=index_path, metadata_path=metadata_path)
