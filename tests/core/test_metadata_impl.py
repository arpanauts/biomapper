...
@pytest.fixture
def async_cache_session_factory(async_cache_engine):
...
async def cache_test_results(
    ...
    # First create the session factory
    session_factory = await session_factory_func()
...
@pytest.mark.asyncio
async def test_cache_results_populates_metadata_fields(async_cache_engine):
    """Test that _cache_results properly populates all metadata fields."""
    # Create a session factory and a session directly from the engine
    factory = async_sessionmaker(
        bind=async_cache_engine, 
        expire_on_commit=False,
        class_=AsyncSession
    ...
    # Execute cache_test_results with forward path
    await cache_test_results(
        session_factory_func=factory_func,
...
@pytest.mark.asyncio
async def test_confidence_score_calculation(async_cache_engine):
    """Test the confidence score calculation for different path lengths and directions."""
    # Create a session factory directly from the engine
    factory = async_sessionmaker(
        bind=async_cache_engine, 
        expire_on_commit=False,
        class_=AsyncSession
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        # Execute cache_test_results for this scenario
        await cache_test_results(
            session_factory_func=factory_func,
            results_to_cache=results_to_cache,
            path=path,
    ...
    for result in result_confidences:
        ...
        if scenario["is_reverse"]:
            ...
            if forward_result:
                ...
                forward_confidence = forward_result["actual_confidence"]
                assert actual < forward_confidence
                assert abs(actual - (forward_confidence - 0.05)) < 0.01

@pytest.mark.asyncio
async def test_mapping_path_details_contents(async_cache_engine):
