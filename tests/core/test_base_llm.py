...
class MockLLMAnalyzer(BaseLLMAnalyzer):
    ...
    async def analyze_results(
        self, query_results: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> AnalysisResult:
    ...
    async def process_response(
        self, response: Union[str, Dict[str, Any]], analysis_type: str
    ) -> InsightType:
        return InsightType(
...
def llm_config() -> LLMConfig:
    ...
    return LLMConfig(
        model_name="test-model", temperature=0.7, max_tokens=1000, timeout=30
    )
...
class TestLLMConfig:
    ...
    def test_invalid_temperature(self):
        ...
        with pytest.raises(ValidationError) as exc:
            LLMConfig(model_name="test", temperature=2.5, max_tokens=1000)
        assert "temperature" in str(exc.value)

    def test_invalid_max_tokens(self):
        """Test max_tokens validation."""
        with pytest.raises(ValidationError) as exc:
            LLMConfig(model_name="test", temperature=0.7, max_tokens=-1)
...
class TestInsightType:
    ...
    def test_default_values(self):
        ...
        insight = InsightType(category="test", description="test description")
...
class TestBaseLLMAnalyzer:
    ...
    async def test_mock_analyzer(self):
        ...
        result = await analyzer.analyze_results({"test": "data"}, {"context": "test"})
    ...
    async def test_prompt_generation(self):
        ...
        prompts = await analyzer.generate_prompts(
            "test_analysis", {"data": "test"}, {"var": "test"}
        )
    ...
    async def test_response_processing(self):
        ...
        analyzer = MockLLMAnalyzer()
        insight = await analyzer.process_response("Test response", "test_analysis")
