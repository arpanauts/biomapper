...
class TestJobLifecycle:
    ...
    async def test_update_job_status(self, persistence_service):
        ...
        # Update to completed
        updated_job = await persistence_service.update_job_status(
            job.id, JobStatus.COMPLETED, final_results={"success": True}
        )
...
class TestStepManagement:
    ...
    async def test_record_step_lifecycle(self, persistence_service, test_job):
        ...
        step = await persistence_service.record_step_start(
            job_id, 0, "test_step", "TEST_ACTION", {"param": "value"}
        )
    ...
    async def test_record_step_failure(self, persistence_service, test_job):
        ...
        await persistence_service.record_step_start(
            job_id, 0, "failing_step", "FAIL_ACTION", {}
        )
...
class TestCheckpointManagement:
    ...
    async def test_create_checkpoint(self, persistence_service, test_job_and_context):
        ...
        checkpoint = await persistence_service.create_checkpoint(
            job.id, 0, context, checkpoint_type="manual", description="Test checkpoint"
        )
        ...
        assert (
            checkpoint.context_data is not None or checkpoint.storage_path is not None
        )
    ...
    async def test_restore_checkpoint(self, persistence_service, test_job_and_context):
        ...
        assert restored_context.custom_action_data.get(
            "input_identifiers"
        ) == original_context.custom_action_data.get("input_identifiers")
        assert restored_context.custom_action_data.get(
            "output_identifiers"
        ) == original_context.custom_action_data.get("output_identifiers")
        assert (
            restored_context.custom_action_data == original_context.custom_action_data
        )
    ...
    async def test_list_checkpoints(self, persistence_service, test_job_and_context):
        ...
        for i in range(3):
            cp = await persistence_service.create_checkpoint(
                job.id, i, context, checkpoint_type=f"type_{i}"
            )
    ...
    async def test_get_latest_checkpoint(
        self, persistence_service, test_job_and_context
    ):
...
class TestResultStorage:
    ...
    async def test_store_and_retrieve_small_result(self, persistence_service, test_job):
        ...
        # Store result
        storage = await persistence_service.store_result(job_id, 0, "test_result", data)
        ...
        # Retrieve result
        retrieved = await persistence_service.retrieve_result(job_id, 0, "test_result")
    ...
    async def test_store_and_retrieve_large_result(self, persistence_service, test_job):
        ...
        # Retrieve result
        retrieved = await persistence_service.retrieve_result(job_id, 0, "large_result")
...
class TestLoggingAndEvents:
    ...
    async def test_logging(self, persistence_service, test_job):
        ...
        await persistence_service.log(
            job_id, "ERROR", "Test error message", step_index=1
        )
    ...
    async def test_events(self, persistence_service, test_job):
        ...
        await persistence_service.emit_event(
            job_id, "error", {"error": "test error"}, severity="error"
        )
...
class TestMetricsAndCleanup:
    ...
    async def test_get_job_metrics(self, persistence_service):
        ...
        await persistence_service.record_step_failure(job.id, 1, "Test failure")
    ...
    async def test_cleanup_old_data(self, persistence_service):
        ...
        await persistence_service.update_job_status(
            old_job.id, JobStatus.COMPLETED, completed_at=old_date
        )
...
class TestPersistenceIntegration:
    ...
    async def test_full_job_execution_simulation(self, persistence_service):
        ...
        for i, step in enumerate(strategy["steps"]):
            ...
            # Log progress
            await persistence_service.log(
                job.id, "INFO", f"Starting {step['name']}", step_index=i
            )
            ...
            if step["name"] in ["load_data", "process_data"]:
                await persistence_service.create_checkpoint(
                    job.id, i, context, checkpoint_type="before_step"
                )
            ...
            await persistence_service.create_checkpoint(
                job.id, i, context, checkpoint_type="after_step"
            )

            # Emit progress event
            await persistence_service.emit_event(
                job.id, "step_completed", {"step": step["name"], "index": i}
            )
        ...
        await persistence_service.update_job_status(
            job.id, JobStatus.COMPLETED, final_results=final_results
        )
