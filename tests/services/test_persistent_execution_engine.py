...
class MockSuccessAction(BaseStrategyAction):
    ...
    async def execute(
        self, params: Dict[str, Any], context: StrategyExecutionContext
    ) -> Dict[str, Any]:
        return {
            "success": True,
...
class TestBasicExecution:
    ...
    @pytest.mark.asyncio
    async def test_successful_strategy_execution(
        self, execution_engine, persistence_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_strategy_execution_with_failure(
        self, execution_engine, persistence_service
    ):
...
class TestCheckpointing:
    ...
    async def test_automatic_checkpointing(self, execution_engine, persistence_service):
        ...
        strategy = {
            ...
            "checkpoint_policy": {"after_each_step": True},
            "steps": [
                {
    ...
    async def test_resume_from_checkpoint(self, execution_engine, persistence_service):
        ...
        checkpoint = await persistence_service.create_checkpoint(
            job.id, 0, context, checkpoint_type="manual"
        )
...
class TestJobControl:
    ...
    async def test_pause_and_resume_job(self, execution_engine, persistence_service):
        ...
        checkpoint = await persistence_service.create_checkpoint(
            job.id, 0, context, description="Pause checkpoint"
        )
        ...
        # Resume job
        with patch.object(
            execution_engine, "execute_strategy", new_callable=AsyncMock
        ) as mock_execute:
...
class TestLargeResultHandling:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Database session isolation issue - needs refactoring")
    async def test_large_result_external_storage(
        self, execution_engine, persistence_service
    ):
