...
class CacheManagerTest(unittest.TestCase):
    ...
    def test_add_mapping_with_metadata(self):
        ...
        session = self.db_manager.create_session()
        mapping = session.query(EntityMapping).filter(
            EntityMapping.source_id == "HMDB0000002"
        ).first()
        
        metadata = session.query(MappingMetadata).filter(
            MappingMetadata.mapping_id == mapping.id
        ).all()
