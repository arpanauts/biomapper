...
async def test_successful_execution(mock_client, tmp_path):
    ...
    with patch("biomapper_client.BiomapperClient", return_value=mock_client):
        ...
        with patch("sys.argv", ["run_metabolomics_harmonization.py"]):
            ...
            assert (
                call_args.kwargs["strategy_name"]
...
async def test_three_way_flag(mock_client):
    ...
    with patch("biomapper_client.BiomapperClient", return_value=mock_client):
        ...
        with patch("sys.argv", ["run_metabolomics_harmonization.py", "--three-way"]):
            ...
            call_args = mock_client.execute_strategy.call_args
            assert (
                call_args.kwargs["strategy_name"] == "THREE_WAY_METABOLOMICS_COMPLETE"
            )
...
async def test_custom_parameters(mock_client, tmp_path):
    ...
    with patch("biomapper_client.BiomapperClient", return_value=mock_client):
        ...
        with patch(
            "sys.argv",
            ["run_metabolomics_harmonization.py", "--parameters", str(params_file)],
        ):
            result = await main()
...
async def test_execution_failure(mock_client):
    ...
    mock_client.execute_strategy.return_value = {
        ...
        "details": {"failed_step": "load_data", "error_details": "File not found"},
    }
...
async def test_custom_strategy(mock_client):
    ...
    with patch("biomapper_client.BiomapperClient", return_value=mock_client):
        ...
        with patch(
            "sys.argv",
            ["run_metabolomics_harmonization.py", "--strategy", "CUSTOM_STRATEGY"],
        ):
            result = await main()
...
async def test_output_directory_override(mock_client, tmp_path):
    ...
    with patch("biomapper_client.BiomapperClient", return_value=mock_client):
        ...
        with patch(
            "sys.argv",
            ["run_metabolomics_harmonization.py", "--output-dir", str(output_dir)],
        ):
            result = await main()
...
def test_script_size():
    ...
    assert (
        len(code_lines) < 200
    ), f"Script still too large: {len(code_lines)} lines of code"
...
def test_script_dry_run_works():
    ...
    assert result.returncode == 0
    assert (
        "Validating strategy configuration" in result.stderr
        or "Validating strategy configuration" in result.stdout
    )
