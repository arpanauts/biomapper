    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            if node.module and any(
                node.module.startswith(forbidden) for forbidden in forbidden_modules
            ):
                violations.append(f"Forbidden import: from {node.module} import ...")
        elif isinstance(node, ast.Import):
            for alias in node.names:
                if any(
                    alias.name.startswith(forbidden) for forbidden in forbidden_modules
                ):
...
class TestRefactoredScripts:
    ...
    def mock_client(self):
        ...
        with patch("biomapper_client.BiomapperClient") as MockClient:
            ...
            mock_instance.execute_strategy = AsyncMock(
                return_value={
                    "success": True,
    ...
    def test_all_scripts_under_100_lines(self):
        ...
        if scripts_dir.exists():
            for script_path in scripts_dir.glob("*.py"):
                ...
                assert (
                    lines < 100
                ), f"{script_path.name} has {lines} lines (should be < 100)"
    ...
    def test_old_scripts_have_deprecation_warnings(self):
        ...
        for script_path in old_scripts:
            if script_path.exists():
                ...
                ), f"{script_path.name} missing deprecation warning"
                assert (
                    "deprecated and will be removed in v2.0" in content
                ), f"{script_path.name} missing deprecation message"
...
class TestCLIUtils:
    ...
    def test_execution_options_to_dict(self):
        ...
        options = ExecutionOptions(
            checkpoint_enabled=True, retry_failed_steps=False, debug=True, max_retries=5
        )
...
class TestMigrationCompleteness:
    ...
    def test_migration_guide_exists(self):
        ...
        guide_path = (
            Path(__file__).parent.parent.parent
            / "docs"
            / "WRAPPER_SCRIPT_MIGRATION_GUIDE.md"
        )
