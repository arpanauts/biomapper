...
class TestGenerateEnhancementReport:
    ...
    def mock_context(self, sample_metrics):
        ...
        context.get.side_effect = lambda key, default=None: sample_metrics.get(
            key, default
        )
        return context
    ...
    async def test_params_validation(self):
        ...
        params = GenerateEnhancementReportParams(
            metrics_keys=["metrics.test"], output_path="/tmp/output.md"
        )
    ...
    async def test_metrics_aggregation(
        self, action, params, mock_context, sample_metrics
    ):
    ...
    async def test_improvement_calculation(self, action):
        ...
        abs_improvement_1 = action._calculate_absolute_improvement(
            baseline_rate, enhanced_rate
        )
        rel_improvement_1 = action._calculate_relative_improvement(
            baseline_rate, enhanced_rate
        )

        abs_improvement_2 = action._calculate_absolute_improvement(
            baseline_rate, final_rate
        )
        rel_improvement_2 = action._calculate_relative_improvement(
            baseline_rate, final_rate
        )
    ...
    async def test_file_writing(
        self, mock_dirname, mock_makedirs, mock_file, action, params
    ):
    ...
    async def test_file_write_error_handling(
        self, mock_makedirs, mock_file, action, params
    ):
    ...
    async def test_full_report_generation(
        self, mock_makedirs, mock_file, action, params, mock_context
    ):
