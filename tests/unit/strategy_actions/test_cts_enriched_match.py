...
class TestCtsEnrichedMatch:
    ...
    async def test_cts_client_initialization(self, action):
        ...
        with patch(
            "biomapper.core.strategy_actions.cts_enriched_match.CTSClient"
        ) as mock_cts:
            mock_instance = Mock()
    ...
    @pytest.mark.asyncio
    async def test_metabolite_enrichment(
        self, action, mock_cts_client, unmatched_arivale_data
    ):
        ...
        enriched, api_calls = await action._enrich_metabolites(
            unmatched_arivale_data, ["HMDB", "KEGG", "PUBCHEM"], batch_size=10
        )
    ...
    def test_fuzzy_match_with_enriched_names(self, action, nightingale_reference):
        ...
        match = action._fuzzy_match_enriched(
            enriched_metabolite, nightingale_reference, "unified_name", 0.80
        )
    ...
    async def test_full_enrichment_workflow(
        self, action, mock_cts_client, unmatched_arivale_data, nightingale_reference
    ):
    ...
    async def test_no_enrichment_possible(self, action, mock_cts_client):
        ...
        enriched, api_calls = await action._enrich_metabolites(
            metabolites, ["HMDB", "KEGG"], batch_size=10
        )
    ...
    async def test_metrics_tracking(self, action, mock_cts_client):
        ...
        context = {
            "datasets": {
                "unmatched": [{"BIOCHEMICAL_NAME": "test", "HMDB": "HMDB0001"}],
                "reference": [{"unified_name": "test-name"}],
            }
        }
    ...
    async def test_partial_identifier_coverage(self, action, mock_cts_client):
        ...
        enriched, _ = await action._enrich_metabolites(
            metabolites, ["HMDB", "KEGG", "PUBCHEM"], batch_size=10
        )
