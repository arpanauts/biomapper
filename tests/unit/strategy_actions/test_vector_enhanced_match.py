...
class TestVectorEnhancedMatch:
    ...
    def test_client_initialization(self, action):
        ...
        with patch('biomapper.core.strategy_actions.vector_enhanced_match.TextEmbedding') as mock_embed:
            with patch('biomapper.core.strategy_actions.vector_enhanced_match.QdrantClient') as mock_qdrant:
    ...
    def test_match_deduplication(self, action):
        ...
        test_match = next(m for m in deduplicated if m.metabolite['BIOCHEMICAL_NAME'] == 'test')
    ...
    async def test_full_vector_matching_workflow(
        ...
        context = {
            'datasets': {
                'unmatched': unmatched_metabolites
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_empty_search_texts(self, action, mock_embedding_model, mock_qdrant_client):
