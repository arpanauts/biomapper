...
class TestMetaboliteStrategies:
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                try:
                    ...
                    assert isinstance(strategy, dict), f"Invalid YAML structure in {strategy_file}"
    ...
    def test_required_metadata_fields(self, strategy_files):
        ...
        required_fields = [
            'id', 'name', 'version', 'created', 'author',
            'entity_type', 'source_dataset', 'target_dataset', 'bridge_type'
        ]
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                ...
                for field in required_fields:
                    assert field in metadata, f"Missing {field} in {strategy_file} metadata"
                
                # Test entity_type is metabolites
                assert metadata['entity_type'] == 'metabolites', f"Wrong entity_type in {strategy_file}"
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                ...
                for i, step in enumerate(steps):
                    ...
                    assert 'action' in step, f"Step {i} missing action in {strategy_file}"
                    
                    action = step['action']
                    assert 'type' in action, f"Step {i} action missing type in {strategy_file}"
                    assert 'params' in action, f"Step {i} action missing params in {strategy_file}"
    ...
    def test_metabolite_specific_actions(self, strategy_files):
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                ...
                has_metabolite_action = any(action in metabolite_actions for action in action_types)
                assert has_metabolite_action, f"No metabolite-specific actions in {strategy_file}"
            ...
            if 'multi' in strategy_file.name:
                with open(strategy_file, 'r') as f:
                    ...
                    if multi_bridge_steps:
                        ...
                        assert 'bridge_types' in params, f"Missing bridge_types in {strategy_file}"
                        assert isinstance(params['bridge_types'], list), \
                            f"bridge_types not a list in {strategy_file}"
                        assert len(params['bridge_types']) > 1, \
        ...
        for strategy_file in strategy_files:
            if 'lipid' in strategy_file.name:
                with open(strategy_file, 'r') as f:
                    ...
                    assert has_lipid_parsing, f"No lipid-specific parsing in {strategy_file}"
    ...
    def test_cache_configuration(self, strategy_files):
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                ...
                assert 'use_cache' in parameters, f"Missing use_cache parameter in {strategy_file}"
                assert parameters['use_cache'] is True, f"Caching disabled in {strategy_file}"
                
                if 'cache_dir' in parameters:
                    cache_dir = parameters['cache_dir']
                    assert isinstance(cache_dir, str), f"Invalid cache_dir type in {strategy_file}"
    ...
    def test_output_configuration(self, strategy_files):
        ...
        for strategy_file in strategy_files:
            with open(strategy_file, 'r') as f:
                ...
                assert 'output_dir' in parameters, f"Missing output_dir parameter in {strategy_file}"
                
                # Should have at least one EXPORT_DATASET step
                steps = strategy.get('steps', [])
                export_steps = [
                    step for step in steps
                    if step['action']['type'] == 'EXPORT_DATASET'
                ]
    ...
    def test_specific_strategy_validation(self, strategy_name):
        ...
        strategy_file = Path(f"/home/ubuntu/biomapper/configs/strategies/experimental/{strategy_name}.yaml")
...
class TestMetaboliteStrategyContent:
    ...
    def test_hmdb_normalization_steps(self):
        ...
        strategy_file = Path("/home/ubuntu/biomapper/configs/strategies/experimental/met_isr_metab_to_kg2c_hmdb_v1_base.yaml")
        ...
        for step in normalize_steps:
            params = step['action']['params']
            assert 'target_format' in params, "Missing target_format in HMDB normalization"
            assert params['target_format'] in ['HMDB0001234', 'HMDB00001234'], \
    ...
    def test_cts_bridge_configuration(self):
        ...
        for strategy_name in strategy_files:
            strategy_file = Path(f"/home/ubuntu/biomapper/configs/strategies/experimental/{strategy_name}")
            ...
            for step in cts_steps:
                ...
                if 'parallel_requests' in params:
                    ...
                    assert params['parallel_requests'] <= 10, "Too many parallel requests"
            ...
            for step in semantic_steps:
                ...
                if 'model' in params:
                    ...
                    if model.startswith('${'):
                        ...
                        param_name = model.strip('${}').split('.')[-1]  # Get parameter name
                        if param_name in strategy_params:
                            actual_model = strategy_params[param_name]
                            assert actual_model in ['biobert', 'chembert', 'mol-bert'], \
                                f"Unsupported semantic model in parameters: {actual_model}"
                    else:
                        assert model in ['biobert', 'chembert', 'mol-bert'], \
