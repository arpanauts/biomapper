...
from biomapper.core.models.enhanced_strategy import EnhancedStrategy
from biomapper.core.services.control_flow_executor import ControlFlowExecutor
from biomapper.core.services.expression_evaluator import (
...
class TestSafeExpressionEvaluator:
    ...
    def test_nested_variables(self):
        ...
        context = {"steps": {"baseline": {"metrics": {"score": 0.75, "count": 100}}}}
        evaluator = SafeExpressionEvaluator(context)
    ...
    def test_arithmetic_operations(self):
        ...
        assert (
            abs(evaluator.evaluate("${threshold} * 0.9") - 0.72) < 0.0001
    ...
    def test_ternary_operator(self):
        ...
        evaluator = SafeExpressionEvaluator(
            context
        )  # Create new evaluator with updated context
        result = evaluator.evaluate("${size} > 1000 ? 0.9 : 1.0")
        assert result == 1.0
...
class TestConditionEvaluator:
    ...
    def test_all_condition(self):
        ...
        condition = {"type": "all", "all": ["${a} == True", "${b} == True"]}
...
class TestControlFlowExecutor:
    ...
    async def test_sequential_execution(self):
        ...
        strategy_dict = {
            ...
            "steps": [
                {
                    "name": "step1",
                    "action": {"type": "TEST_ACTION", "params": {"value": 1}},
                },
                {
                    "name": "step2",
                    "action": {"type": "TEST_ACTION", "params": {"value": 2}},
                },
            ],
        }
        ...
        with patch(
            "biomapper.core.services.control_flow_executor.get_action_class"
        ) as mock_get_action:
    ...
    async def test_foreach_loop(self):
        ...
        strategy_dict = {
            "name": "test_foreach",
            "parameters": {"datasets": ["data1", "data2", "data3"]},
            "steps": [
                {
        ...
        with patch(
            ...
            total_calls = mock_action.execute.call_count + (
                mock_action.execute_typed.call_count
                if hasattr(mock_action.execute_typed, "call_count")
            ...
            "steps": [
                {
                    ...
                    "action": {"type": "TEST_ACTION", "params": {}},
                    "set_variables": {"counter": "${variables.counter} + 1"},
                }
            ],
        }
    ...
    async def test_error_handling_continue(self):
        ...
        strategy_dict = {
            ...
            "steps": [
                ...
                {"name": "next_step", "action": {"type": "TEST_ACTION", "params": {}}},
            ],
        }
        ...
        with patch(
            ...
            # Also set execute_typed in case it's checked first
            mock_action1.execute_typed = AsyncMock(
    ...
    async def test_dag_execution(self):
        ...
        strategy_dict = {
            ...
            "execution": {"mode": "dag"},
            "steps": [
                {"name": "load_a", "action": {"type": "TEST_ACTION", "params": {}}},
                {"name": "load_b", "action": {"type": "TEST_ACTION", "params": {}}},
                {
                    "name": "process_a",
                    "depends_on": ["load_a"],
                    "action": {"type": "TEST_ACTION", "params": {}},
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            strategy_dict = {
                ...
                "steps": [
                    ...
                    {"name": "step2", "action": {"type": "TEST_ACTION", "params": {}}},
                ],
            }
        ...
        strategy_dict = {
            ...
            "steps": [
                ...
                },
                {
                    ...
                    "action": {
                        "type": "TEST_ACTION",
                        "params": {"input": "${variables.static_value}"},
                    },
                },
            ],
        }
