...
class TestParameterResolver:
    ...
    def test_missing_environment_variable(self, resolver):
        ...
        strategy = {
            'parameters': {
                'missing_var': '${COMPLETELY_MISSING_VAR}'
            }
        }
    ...
    def test_array_index_access(self, resolver):
        ...
        assert '${metadata.files[5].name}' in str(resolved['parameters']['invalid_index'])
    
    def test_mixed_substitution_patterns(self, resolver):
        """Test mixing different substitution patterns in one value."""
        with patch.dict(os.environ, {'BASE_URL': 'https://api.example.com', 'API_VERSION': 'v1'}):
            strategy = {
                'parameters': {
                    'api_endpoint': '${BASE_URL}/${API_VERSION}/data'
                }
            }
            
            resolved = resolver.resolve_strategy_parameters(strategy)
            
            assert resolved['parameters']['api_endpoint'] == 'https://api.example.com/v1/data'
    ...
    def test_nested_substitution(self, resolver):
        ...
        assert resolved['parameters']['full_config'] == 'config.database_production.host'
    ...
    def test_invalid_parameter_reference(self, resolver):
        ...
        assert resolved['parameters']['invalid_ref'] == '${parameters.nonexistent_param}'
    ...
    def test_special_characters_in_values(self, resolver):
        ...
        assert resolved['parameters']['special_chars'] == 'value with spaces and $pecial ch@rs!'
