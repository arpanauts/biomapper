...
class TestStrategyExecutionContext:
    ...
    def test_minimal_context_creation(self):
        ...
        context = StrategyExecutionContext(
            initial_identifier="BRCA1", current_identifier="BRCA1", ontology_type="gene"
        )
    ...
    def test_full_context_creation(self):
        ...
        assert (
            context.custom_action_data["user_metadata"]["project"] == "cancer_research"
        )
    ...
    def test_identifier_tracking(self):
        ...
        assert context.identifier_history == [
            "RS123456",
            "chr1:12345:A:G",
            "ENSG00000139618",
        ]
    ...
    def test_step_results_accumulation(self):
        ...
        context.add_step_result("validation", {"valid": True, "format": "ensembl"})
        ...
        context.add_step_result("mapping", {"error": "Database timeout"}, success=False)
        ...
        context.add_step_result(
            "validation", {"valid": False, "reason": "Invalid checksum"}, success=False
        )
    ...
    def test_custom_action_data_storage(self):
        ...
        context = StrategyExecutionContext(
            initial_identifier="test", current_identifier="test", ontology_type="gene"
        )
        ...
        assert context.get_action_data("species_filter") == {
            "species": ["human", "mouse"]
        }
        ...
        assert (
            context.get_action_data("missing_key", default="default_value")
            == "default_value"
        )
...
class TestProvenanceRecord:
    ...
    def test_provenance_record_creation(self):
        ...
        record = ProvenanceRecord(
            ...
            timestamp=datetime.now(),
            action="protein_mapping",
            details={"method": "exact_match", "confidence": 1.0, "version": "2024.01"},
        )
    ...
    def test_provenance_record_minimal(self):
        ...
        record = ProvenanceRecord(
            source="Manual", timestamp=datetime.now(), action="user_input"
        )
    ...
    def test_provenance_record_validation(self):
        ...
        with pytest.raises(ValidationError):
            ProvenanceRecord(source="", timestamp=datetime.now(), action="test")

        # Empty action should be invalid
        with pytest.raises(ValidationError):
            ProvenanceRecord(source="Test", timestamp=datetime.now(), action="")
