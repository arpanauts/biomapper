...
class TestCustomTransformAction:
    ...
    async def test_column_drop_transform(self, action, sample_data):
        ...
        params = CustomTransformParams(
            ...
            output_key="output",
            transformations=[
                TransformOperation(type="column_drop", params={"columns": ["category"]})
            ],
        )
    ...
    async def test_split_column_transform(self, action):
        ...
        test_data = pd.DataFrame({"combined": ["A_B_C", "D_E_F", "G_H_I"]})
    ...
    async def test_deduplicate_transform(self, action):
        ...
        test_data = pd.DataFrame({"id": ["A", "B", "A", "C"], "value": [1, 2, 1, 3]})
        context = {"datasets": {"input": test_data}}
        ...
        params = CustomTransformParams(
            ...
            transformations=[
                TransformOperation(
                    type="deduplicate", params={"subset": ["id"], "keep": "first"}
                )
            ],
        )
    ...
    async def test_fill_na_transform(self, action):
        ...
        test_data = pd.DataFrame(
            {"id": ["A", "B", None, "D"], "value": [1, None, 3, 4]}
        )
        context = {"datasets": {"input": test_data}}
        ...
        params = CustomTransformParams(
            ...
            transformations=[
                TransformOperation(
                    type="fill_na", params={"method": "value", "value": "MISSING"}
                )
            ],
        )
    ...
    async def test_sort_transform(self, action, sample_data):
        ...
        params = CustomTransformParams(
            ...
            transformations=[
                TransformOperation(
                    type="sort", params={"by": ["score"], "ascending": False}
                )
            ],
        )
    ...
    async def test_multiple_transformations(self, action, sample_data):
        ...
        params = CustomTransformParams(
            ...
            transformations=[
                TransformOperation(
                    type="column_add", params={"columns": {"processed": True}}
                ),
                TransformOperation(
                    type="filter_rows",
                ...
                TransformOperation(
                    type="column_rename", params={"mapping": {"name": "protein_name"}}
                ),
            ],
        )
    ...
    async def test_dataset_not_found(self, action):
        ...
        params = CustomTransformParams(
            input_key="nonexistent", output_key="output", transformations=[]
        )
    ...
    async def test_context_statistics_update(self, action, sample_data):
        ...
        params = CustomTransformParams(
            ...
            transformations=[
                TransformOperation(
                    type="column_add", params={"columns": {"new_col": "test"}}
                )
            ],
        )
