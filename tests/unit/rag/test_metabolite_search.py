...
class TestMetaboliteSearcher:
    ...
    def searcher(self, mock_qdrant_client):
        ...
        with patch('biomapper.rag.metabolite_search.QdrantClient', return_value=mock_qdrant_client):
            with patch('biomapper.rag.metabolite_search.TextEmbedding') as mock_embedding:
                ...
                searcher = MetaboliteSearcher(
                    ...
                    collection_name="test_metabolites"
                )
    ...
    async def test_search_by_name_returns_sorted_results(self, searcher):
        ...
        mock_results = [
            Mock(score=0.92, payload={'hmdb_id': 'HMDB0000122', 'name': 'Cholesterol ester'}),
    ...
    async def test_search_filters_by_threshold(self, searcher):
        ...
        mock_results = [
            Mock(score=0.85, payload={'hmdb_id': 'HMDB0000067', 'name': 'Cholesterol'}),
            Mock(score=0.75, payload={'hmdb_id': 'HMDB0000122', 'name': 'Cholesterol ester'}),
            Mock(score=0.65, payload={'hmdb_id': 'HMDB0000097', 'name': 'Cholestanol'})
        ]
        searcher.client.search.return_value = mock_results
        
        results = await searcher.search_by_name(
            "cholesterol",
            score_threshold=0.8
        )
    ...
    async def test_search_limit_parameter(self, searcher):
        ...
        def mock_search(*args, **kwargs):
            ...
            all_results = [
                Mock(score=0.9 - i*0.01, payload={'hmdb_id': f'HMDB{i:07d}', 'name': f'Compound {i}'})
                for i in range(20)
            ]
