...
class TestCTSClient:
    ...
    async def test_initialize_loads_valid_ids(self, client, mock_session):
        ...
        response2.json = AsyncMock(
            return_value=["Chemical Name", "InChIKey", "PubChem CID"]
        )

        mock_session.get.side_effect = [
            AsyncMock(
                __aenter__=AsyncMock(return_value=response1),
                __aexit__=AsyncMock(return_value=None),
            ),
            AsyncMock(
                __aenter__=AsyncMock(return_value=response2),
                __aexit__=AsyncMock(return_value=None),
            ),
        ]
    ...
    async def test_convert_single_identifier(self, client, mock_session):
        ...
        result = await client.convert(
            "QNAYBMKLOCPYGJ-REOHCLBHSA-N", "InChIKey", "Chemical Name"
        )
    ...
    def test_cache_key_generation(self, client):
        ...
        key1 = client._get_cache_key(
            "convert", from_type="HMDB", to_type="KEGG", id="HMDB001"
        )
        key2 = client._get_cache_key(
            "convert", id="HMDB001", from_type="HMDB", to_type="KEGG"
        )
    ...
    async def test_convert_batch_multiple_targets(self, client):
        ...
        result = await client.convert_batch(
            ["HMDB0000001"], "HMDB", ["InChIKey", "KEGG"]
        )
