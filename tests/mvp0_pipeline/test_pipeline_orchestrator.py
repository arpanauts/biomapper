...
from biomapper.schemas.pipeline_schema import PipelineMappingResult, PipelineStatus
...
class TestPipelineOrchestrator:
    ...
    def mock_qdrant_client(self):
        ...
        with patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.QdrantClient"
        ) as mock:
    ...
    async def test_run_single_mapping_success(self, valid_config, mock_qdrant_client):
        ...
        mock_annotations = {
            ...
            107526: PubChemAnnotation(
                cid=107526, title="beta-D-Glucopyranose", molecular_formula="C6H12O6"
            ),
        }
        ...
        with patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.search_qdrant_for_biochemical_name",
            new_callable=AsyncMock,
        ) as mock_qdrant_search, patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.fetch_pubchem_annotations",
            new_callable=AsyncMock,
        ) as mock_pubchem, patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.select_best_cid_with_llm",
            new_callable=AsyncMock,
        ) as mock_llm:
            ...
            assert (
                result.rationale == "Direct title match with common synonym 'glucose'"
            )
    ...
    @pytest.mark.asyncio
    async def test_run_single_mapping_no_qdrant_hits(
        self, valid_config, mock_qdrant_client
    ):
        """Test mapping with no Qdrant hits."""
        with patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.search_qdrant_for_biochemical_name",
            new_callable=AsyncMock,
        ) as mock_qdrant_search:
            ...
            assert (
                ...
                == "No similar compounds found in Qdrant vector database"
            )
            assert result.qdrant_results == []

    @pytest.mark.asyncio
    async def test_run_single_mapping_insufficient_annotations(
        self, valid_config, mock_qdrant_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_single_mapping_llm_no_match(
        self, valid_config, mock_qdrant_client
    ):
        """Test mapping where LLM finds no suitable match."""
        mock_qdrant_results = [QdrantSearchResultItem(cid=12345, score=0.75)]
        mock_annotations = {12345: PubChemAnnotation(cid=12345, title="Some compound")}
        mock_llm_choice = LLMChoice(
            selected_cid=None,
            llm_confidence=0.2,
    ...
    @pytest.mark.asyncio
    async def test_run_single_mapping_qdrant_error(
        self, valid_config, mock_qdrant_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_single_mapping_pubchem_error(
        self, valid_config, mock_qdrant_client
    ):
    ...
    async def test_run_pipeline_batch(self, valid_config, mock_qdrant_client):
        ...
        with patch.object(
            PipelineOrchestrator, "run_single_mapping", new_callable=AsyncMock
        ) as mock_single:
    ...
    def test_create_orchestrator_from_env(self, mock_qdrant_client):
        ...
        with patch(
            "biomapper.mvp0_pipeline.pipeline_config.create_pipeline_config"
...
class TestPipelineIntegration:
    ...
    async def test_data_flow_between_components(self):
        ...
        with patch("biomapper.mvp0_pipeline.pipeline_orchestrator.QdrantClient"), patch(
            "biomapper.mvp0_pipeline.pipeline_orchestrator.search_qdrant_for_biochemical_name",
            new_callable=AsyncMock,
            ...
            def verify_llm_input(
                original_biochemical_name, candidates_info, anthropic_api_key
            ):
                assert original_biochemical_name == "glucose"
                assert len(candidates_info) == 2
                assert all(
                    hasattr(c, "cid")
                    and hasattr(c, "qdrant_score")
