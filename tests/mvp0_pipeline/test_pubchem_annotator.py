...
from biomapper.mvp0_pipeline.pubchem_annotator import (
    fetch_pubchem_annotations,
    fetch_single_cid_annotation,
)
...
class TestPubChemAnnotator:
    ...
    @pytest.mark.asyncio
    async def test_fetch_single_cid_annotation_success(
        self, mock_httpx_client, mock_response_data
    ):
        """Test successful annotation of a single CID."""
        # Mock successful responses
        mock_prop_response = MagicMock()
        mock_prop_response.status_code = 200
        mock_prop_response.json = MagicMock(
            return_value=mock_response_data["properties"]
        )
        ...
        mock_desc_response.json = MagicMock(
            return_value=mock_response_data["description"]
        )
        ...
        assert (
            result.iupac_name == "(3R,4S,5S,6R)-6-(hydroxymethyl)oxane-2,3,4,5-tetrol"
        )
        assert result.molecular_formula == "C6H12O6"
        assert len(result.synonyms) == 4
        assert (
            result.description
            == "D-glucopyranose is a glucopyranose having D-configuration."
        )
    ...
    async def test_fetch_pubchem_annotations_multiple_cids(self):
        ...
        with patch(
            "biomapper.mvp0_pipeline.pubchem_annotator.fetch_single_cid_annotation"
        ) as mock_fetch:
