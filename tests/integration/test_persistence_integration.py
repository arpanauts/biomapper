...
sys.path.insert(0, str(PathLib(__file__).parent.parent / "biomapper-api"))
...
class FailingTestAction(BaseStrategyAction):
    ...
    async def execute(self, params, context):
        raise Exception(f"Integration test failure: {params.get('error_msg', 'default error')}")
...
async def integration_engine(integration_persistence, integration_action_registry):
    ...
    return PersistentExecutionEngine(integration_persistence, integration_action_registry)
...
class TestEndToEndExecution:
    ...
    @pytest.mark.asyncio
    async def test_complete_workflow_execution(self, integration_engine, integration_persistence):
...
class TestResumeAndRecovery:
    ...
    @pytest.mark.asyncio
    async def test_checkpoint_resume_workflow(self, integration_engine, integration_persistence):
    ...
    @pytest.mark.asyncio
    async def test_step_index_resume_workflow(self, integration_engine, integration_persistence):
        """Test resuming from a specific step index."""
        
        strategy = {
            "name": "step_resume_workflow",
            "steps": [
                {"name": "skip_0", "action": {"type": "INTEGRATION_TEST", "params": {"step": 0}}},
                {"name": "skip_1", "action": {"type": "INTEGRATION_TEST", "params": {"step": 1}}},
                {"name": "execute_2", "action": {"type": "INTEGRATION_TEST", "params": {"step": 2}}},
                {"name": "execute_3", "action": {"type": "INTEGRATION_TEST", "params": {"step": 3}}}
            ]
        }
...
class TestJobControlIntegration:
    ...
    @pytest.mark.asyncio
    async def test_pause_resume_workflow(self, integration_engine, integration_persistence):
...
class TestMetricsAndMonitoring:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_metrics_collection(self, integration_engine, integration_persistence):
            ...
            async def execute(self, params, context):
                ...
                if failure_type == "retryable":
                    ...
                    if failure_count["retryable"] < 3:
                        raise Exception(f"Retryable failure #{failure_count['retryable']}")
