...
class TestMetaboliteStrategiesIntegration:
        ...
        spoke = pd.DataFrame(
            {
                ...
                "identifiers": [
                    "HMDB0000001|InChI=ABCD",
                    "HMDB0000190|InChI=EFGH",
                    "HMDB0000003|InChI=IJKL",
                ],
            }
        )
    ...
    def test_israeli10k_kg2c_strategy_execution(
        self, mock_client, mock_data_files, temp_dir
    ):
        ...
        result = mock_client.execute_strategy(
            "met_isr_metab_to_kg2c_hmdb_v1_base", parameters=strategy_params
        )
    ...
    @pytest.mark.integration
    def test_nightingale_nmr_strategy_execution(
        self, mock_client, mock_data_files, temp_dir
    ):
        ...
        result = mock_client.execute_strategy(
            "met_ukb_nmr_to_kg2c_nightingale_v1_base", parameters=strategy_params
        )
    ...
    @pytest.mark.integration
    def test_multi_bridge_strategy_execution(
        self, mock_client, mock_data_files, temp_dir
    ):
        ...
        result = mock_client.execute_strategy(
            "met_arv_to_kg2c_multi_v1_base", parameters=strategy_params
        )
    ...
    @pytest.mark.integration
    def test_lipidomics_strategy_execution(
        self, mock_client, mock_data_files, temp_dir
    ):
        ...
        result = mock_client.execute_strategy(
            "met_isr_lipid_to_spoke_inchikey_v1_base", parameters=strategy_params
        )
    ...
    def test_multi_source_harmonization(self, mock_client, mock_data_files, temp_dir):
        ...
        result = mock_client.execute_strategy(
            "met_multi_to_unified_semantic_v1_enhanced", parameters=strategy_params
        )
    ...
    def test_semantic_enrichment_pipeline(self, mock_client, mock_data_files, temp_dir):
        ...
        result = mock_client.execute_strategy(
            "met_multi_semantic_enrichment_v1_advanced", parameters=strategy_params
        )
    ...
    def test_strategy_error_handling(self, mock_client, temp_dir):
        ...
        result = mock_client.execute_strategy(
            "met_isr_metab_to_kg2c_hmdb_v1_base", parameters={"output_dir": temp_dir}
        )
    ...
    def test_performance_benchmarks(self, mock_client, mock_data_files, temp_dir):
        ...
        for strategy in test_strategies:
            ...
            execution_result = mock_client.execute_strategy(
                strategy, parameters={"output_dir": temp_dir}
            )
        ...
        for strategy, results in benchmark_results.items():
            ...
            ), f"Invalid execution time for {strategy}"
            assert (
                0.0 <= results["match_rate"] <= 1.0
            ), f"Invalid match rate for {strategy}"
...
class TestMetaboliteStrategiesRealData:
    ...
    @pytest.mark.slow
    @pytest.mark.skipif(
        not Path("/procedure/data/local_data").exists(),
    ...
    def test_nightingale_reference_availability(self):
        ...
        try:
            ...
            for col in required_columns:
                assert (
                    col in df.columns
                ), f"Missing column {col} in Nightingale reference"
