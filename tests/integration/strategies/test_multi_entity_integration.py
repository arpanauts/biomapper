...
cholesterol\t195.0\t<200\tsubj_002"""
    ...
    def test_comprehensive_strategy_structure_validation(self, client: BiomapperClient):
        ...
        try:
            ...
            strategy_path = Path("/home/ubuntu/biomapper/configs/strategies/experimental") / f"{strategy_id}.yaml"
    ...
    def test_clinical_bridge_strategy_structure_validation(self, client: BiomapperClient):
    ...
    def test_strategy_parameter_substitution(self):
        ...
        strategy_path = Path("/home/ubuntu/biomapper/configs/strategies/experimental/multi_arv_ukb_isr_comprehensive_v1_advanced.yaml")
    ...
    def test_all_strategies_have_required_outputs(self):
        ...
        for strategy_file in strategy_files:
            ...
            assert "expected_outputs" in strategy, f"Missing expected_outputs in {strategy_file.name}"
            outputs = strategy["expected_outputs"]
            
            # Test required output categories
            assert "primary" in outputs, f"Missing primary outputs in {strategy_file.name}"
            assert "reports" in outputs, f"Missing reports outputs in {strategy_file.name}"
            
            # Test output paths use parameter substitution
            for category, output_list in outputs.items():
                for output in output_list:
                    assert "path" in output, f"Missing path in {category} output in {strategy_file.name}"
                    assert "${parameters.output_dir}" in output["path"], \
    ...
    def test_action_sequence_logic(self):
        ...
        comprehensive_path = strategies_dir / "multi_arv_ukb_isr_comprehensive_v1_advanced.yaml"
        ...
        load_indices = [i for i, name in enumerate(step_names) if "load" in name.lower()]
        normalize_indices = [i for i, name in enumerate(step_names) if "normalize" in name.lower()]
        
        if load_indices and normalize_indices:
            # At least one load step should come before any normalize step
            assert min(load_indices) < max(normalize_indices), \
...
class TestMultiEntityStrategyPerformance:
    ...
    def test_strategy_complexity_metrics(self):
        ...
        for strategy_name, metrics in complexity_metrics.items():
            ...
            assert metrics["estimated_complexity"] <= 200, \
