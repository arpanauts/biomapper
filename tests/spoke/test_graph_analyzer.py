...
@pytest.mark.asyncio
async def test_discover_node_types_spoke_style(mock_arango_client, mock_spoke_graph_analyzer):
    ...
    analyzer.sample_node_data = AsyncMock(side_effect=lambda node_type, limit=None: 
        compound_samples if node_type == 'Compound' else gene_samples
    )
...
@pytest.mark.asyncio
async def test_discover_relationship_types(mock_arango_client, mock_spoke_graph_analyzer):
    ...
    analyzer._get_node_type_for_id = AsyncMock(side_effect=lambda node_id: 
        'Compound' if node_id == 'Nodes/123' else
        'Gene' if node_id == 'Nodes/456' else
        'Protein' if node_id == 'Nodes/789' else
        'Pathway'
    )
...
async def test_identify_ontology_fields(mock_spoke_graph_analyzer):
    ...
    analyzer.discover_node_types = AsyncMock(return_value={
...
async def test_generate_schema_mapping(mock_spoke_graph_analyzer):
    ...
    analyzer.identify_ontology_fields = AsyncMock(return_value={
        'Compound': [
