...
jaccard_data = overlap_df[["mapping_combo_id", "jaccard_index"]].sort_values(
    "jaccard_index", ascending=True
)
...
match_rates = overlap_df[
    ["mapping_combo_id", "source_match_rate", "target_match_rate"]
].set_index("mapping_combo_id")
...
row_data = overlap_df[["mapping_combo_id", "total_rows", "matched_rows"]].set_index(
    "mapping_combo_id"
)
...
pivot_match_type = match_type_df.pivot(
    index="mapping", columns="match_type", values="count"
).fillna(0)
...
filtered_match_type = match_type_df[
    ~match_type_df["match_type"].isin(["source_only", "target_only"])
]
pivot_match_type = filtered_match_type.pivot(
    index="mapping", columns="match_type", values="count"
).fillna(0)
...
dice_pivot = overlap_df.pivot_table(
    index="source_name", columns="target_name", values="dice_coefficient"
)
...
ax8.scatter(
    overlap_df["source_only_rows"] + overlap_df["matched_rows"],
    overlap_df["target_only_rows"] + overlap_df["matched_rows"],
    s=overlap_df["matched_rows"] * 2,
    alpha=0.6,
)
for idx, row in overlap_df.iterrows():
    ax8.annotate(
        row["mapping_combo_id"],
        (
...
match_type_pivot = match_type_df.pivot(
    index="mapping", columns="match_type", values="count"
).fillna(0)
...
with open(results_dir / "meta_analysis_summary.txt", "w") as f:
    ...
    f.write(
        f"Average match rate: {(overlap_df['matched_rows'].sum() / overlap_df['total_rows'].sum() * 100):.2f}%\n\n"
    )

    f.write("Top 5 mappings by Jaccard Index:\n")
    top_jaccard = overlap_df.nlargest(5, "jaccard_index")[
        ["mapping_combo_id", "jaccard_index"]
    ]
    for _, row in top_jaccard.iterrows():
        f.write(f"  {row['mapping_combo_id']}: {row['jaccard_index']:.4f}\n")

    f.write("\nTop 5 mappings by number of matches:\n")
    top_matches = overlap_df.nlargest(5, "matched_rows")[
        ["mapping_combo_id", "matched_rows"]
    ]
