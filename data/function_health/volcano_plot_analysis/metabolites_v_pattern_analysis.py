...
def create_metabolomics_sample_size_table():
    ...
    # Title
    ax.text(0.5, 0.95, 'Metabolomics Sample Size Patterns: More Variable Than Proteomics', 
            fontsize=18, fontweight='bold', ha='center', transform=ax.transAxes)
    
    # Create table data
    table_data = [
        ['Clinical Test', 'Coverage', '×', 'Metabolite', 'Coverage', '→', 'Expected n', '→', 'V-curve Characteristic'],
        ['─' * 15, '─' * 8, ' ', '─' * 15, '─' * 8, ' ', '─' * 12, ' ', '─' * 20],
        ['GLUCOSE', '95%', '×', 'Most metabolites', '~90%', '→', 'n ≈ 1,400-1,570', '→', 'Very tall, narrow V'],
        ['CHOLESTEROL', '95%', '×', 'Most metabolites', '~90%', '→', 'n ≈ 1,400-1,570', '→', 'Very tall, narrow V'],
        ['TRIGLYCERIDES', '92%', '×', 'Common metabolites', '~85%', '→', 'n ≈ 1,200-1,400', '→', 'Tall, narrow V'],
        ['ALBUMIN', '90%', '×', 'Variable coverage', '10-90%', '→', 'n ≈ 100-1,400', '→', 'Multiple overlapping Vs'],
        ['BILIRUBIN, DIRECT', '30%', '×', 'Most metabolites', '~90%', '→', 'n ≈ 400-500', '→', 'Medium height V'],
        ['COPPER, RBC', '30%', '×', 'Variable coverage', '10-90%', '→', 'n ≈ 50-450', '→', 'Wide range of Vs'],
        ['SELENIUM', '25%', '×', 'Rare metabolites', '~10%', '→', 'n ≈ 30-50', '→', 'Very wide, short V'],
        ['Leptin', '10%', '×', 'Any metabolite', 'varies', '→', 'n ≈ 10-165', '→', 'Lowest, widest Vs'],
    ]
    ...
    for i, row in enumerate(table_data):
        ...
        if i == 0:
            rect = patches.Rectangle((0.02, y_pos - 0.02), 0.96, row_height, 
                                   facecolor='lightcoral', alpha=0.3, transform=ax.transAxes)
            ax.add_patch(rect)
            fontweight = 'bold'
            fontsize = 12
        elif i == 1:
    ...
    ax.text(0.5, diff_y, differences_text,
            fontsize=13, ha='center', transform=ax.transAxes,
            bbox=dict(boxstyle='round,pad=0.5', facecolor='lightyellow', alpha=0.8))
    
    plt.savefig('metabolomics_sample_size_table.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def create_metabolomics_v_curves():
    ...
    ax1.set_title('Metabolomics: More Sample Size Variation', fontsize=16, fontweight='bold')
    ...
    scatter = ax2.scatter(all_r, -np.log10(all_p), c=all_n, cmap='viridis', 
    ...
    ax2.set_title('Metabolomics Volcano Plot Pattern\n(Smoother gradient, less discrete bands)', 
    ...
    # Add annotations
    ax2.text(0.5, 8.5, 'High n region\n(continuous)', fontsize=11, 
             bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.8))
    ax2.text(0.5, 4, 'Mixed n region\n(gradient)', fontsize=11,
             bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
    ax2.text(0.5, 1.5, 'Low n region\n(sparse)', fontsize=11,
             bbox=dict(boxstyle='round', facecolor='lightcoral', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('metabolomics_v_curves.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def create_proteomics_vs_metabolomics_comparison():
    ...
    for n in prot_sample_sizes:
        ...
        r_vals[outlier_idx] = np.random.choice([-1, 1]) * np.random.uniform(0.3, 0.7, n_outliers)
    ...
    scatter1 = ax1.scatter(prot_r, -np.log10(prot_p), c=prot_n, cmap='viridis',
    ...
    ax1.set_xlabel('Correlation (r)', fontsize=14)
    ax1.set_ylabel('-log10(p-value)', fontsize=14)
    ax1.set_title('Proteomics: Discrete Sample Size Groups\n→ Clear "Banded" Pattern', 
                  fontsize=16, fontweight='bold')
    ax1.set_xlim(-0.8, 0.8)
    ax1.set_ylim(0, 8)
    
    # Add text showing discrete groups
    ax1.text(0.95, 0.95, 'Discrete n values:\n72, 193, 464, 1582', 
    ...
    scatter2 = ax2.scatter(metab_r, -np.log10(metab_p), c=metab_n, cmap='plasma',
                          alpha=0.5, s=20, edgecolor='none')
    ...
    ax2.set_xlabel('Correlation (r)', fontsize=14)
    ax2.set_ylabel('-log10(p-value)', fontsize=14)
    ax2.set_title('Metabolomics: Continuous Sample Size Range\n→ Smooth "Gradient" Pattern', 
                  fontsize=16, fontweight='bold')
    ax2.set_xlim(-0.8, 0.8)
    ax2.set_ylim(0, 8)
    
    # Add text showing continuous range
    ax2.text(0.95, 0.95, 'Continuous n range:\n10 to 1,570', 
             transform=ax2.transAxes, fontsize=11,
             bbox=dict(boxstyle='round', facecolor='white', alpha=0.8),
             verticalalignment='top', horizontalalignment='right')
    
    plt.tight_layout()
    plt.savefig('proteomics_vs_metabolomics_patterns.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def create_metabolomics_summary():
    ...
    # Title
    ax.text(0.5, 0.95, 'Metabolomics V-Pattern Characteristics', 
    ...
    plt.savefig('metabolomics_v_pattern_summary.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
if __name__ == "__main__":
    ...
    print("\nGenerated files:")
    print("  - metabolomics_sample_size_table.png: Sample size patterns in metabolomics")
