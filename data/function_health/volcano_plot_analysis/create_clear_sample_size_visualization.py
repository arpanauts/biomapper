...
def create_sample_size_table():
    ...
    ax.text(
        0.5,
        0.95,
        "Sample Size Matrix: How Different Combinations Create Distinct V-Curves",
        fontsize=18,
        fontweight="bold",
        ha="center",
    ...
    plt.savefig(
        "sample_size_matrix_table.png", dpi=300, bbox_inches="tight", facecolor="white"
    )
    plt.close()
...
def create_v_curves_visualization():
    ...
    ax1.set_title(
        "Individual V-Curves for Each Sample Size Group", fontsize=16, fontweight="bold"
    )
    ...
    for n, label, color in reversed(
        sample_groups
    ):  # Draw in reverse order so larger n is on top
        t_values = r_range * np.sqrt(n - 2) / np.sqrt(1 - r_range**2)
        p_theoretical = 2 * (1 - stats.t.cdf(np.abs(t_values), n - 2))
    ...
    for n, _, color in sample_groups:
        ...
        r_vals[outlier_indices] = np.random.choice([-1, 1]) * np.random.uniform(
            0.3, 0.5, n_outliers
        )
        ...
        # Plot points
        ax2.scatter(
            r_vals, -np.log10(p_vals), color=color, alpha=0.4, s=30, edgecolor="none"
        )
    ...
    ax2.set_title(
        "Overlapped V-Patterns (As Seen in Real Data)", fontsize=16, fontweight="bold"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "v_curves_visualization.png", dpi=300, bbox_inches="tight", facecolor="white"
    )
    plt.close()
...
def create_summary_figure():
    ...
    ax.text(
        0.5,
        0.95,
        "Why Volcano Plots Show Multiple V-Patterns",
        fontsize=20,
        fontweight="bold",
        ha="center",
        transform=ax.transAxes,
    ...
    plt.savefig(
        "multiple_v_patterns_summary.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
    )
...
if __name__ == "__main__":
    ...
    print("\nGenerated files:")
    print(
        "  - sample_size_matrix_table.png: Clear table showing sample size combinations"
    )
    print(
        "  - v_curves_visualization.png: Side-by-side comparison of individual vs overlapped curves"
    )
