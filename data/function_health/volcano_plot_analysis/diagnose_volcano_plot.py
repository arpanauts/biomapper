...
metabolomics_data = correlation_results[
    correlation_results["omics_type"] == "metabolomics"
]
...
print(
    f"Proteomics correlations with |r|<0.001: {len(near_zero_prot)} ({len(near_zero_prot)/len(proteomics_data)*100:.1f}%)"
)
...
center_data = proteomics_data[
    (proteomics_data["pearson_r"] > -0.1) & (proteomics_data["pearson_r"] < 0.1)
]
...
# Volcano plot
ax.scatter(
    proteomics_data["pearson_r"],
    -np.log10(proteomics_data["pearson_p"]),
    alpha=0.3,
    s=10,
)
...
ax = axes[1, 1]
# Zoom in on the center
center_data = metabolomics_data[
    (metabolomics_data["pearson_r"] > -0.1) & (metabolomics_data["pearson_r"] < 0.1)
]
...
if len(zero_corr_prot) > 0:
    ...
    zero_by_test = (
        zero_corr_prot.groupby("clinical_test").size().sort_values(ascending=False)
    )
    ...
    zero_by_feature = (
        zero_corr_prot.groupby("omics_feature").size().sort_values(ascending=False)
    )
...
if len(zero_by_test) > 0:
    ...
    print(
        test_zeros[
            ["omics_feature", "omics_feature_name", "n_samples", "pearson_p"]
        ].head(10)
    )
