...
def plot_volcano_with_theoretical_overlay(
    df: pd.DataFrame, omics_type: str, ax: plt.Axes
):
    """
...
def identify_interesting_correlations(
    df: pd.DataFrame,
    omics_type: str,
    ...
    for _, row in omics_data.iterrows():
        p_theo = calculate_theoretical_pvalue(
            np.array([row["pearson_r"]]), row["n_samples"]
        )[0]
    ...
    omics_data["deviation"] = (
        omics_data["log_p_actual"] - omics_data["log_p_theoretical"]
    )
...
def create_comprehensive_visualization():
    ...
    for i, omics_type in enumerate(omics_types[:3]):  # Limit to first 3 types
        ...
        ax.bar(range(len(top_10)), top_10["percentage"], tick_label=top_10["n_samples"])
...
"""
    ...
    for omics_type in omics_types:
        ...
        print(
            f"  Strong effect (|r|>0.3): {(np.abs(omics_data['pearson_r']) > 0.3).sum():,}"
        )
        print(f"  Interesting (both): {len(interesting):,}")
        print(
            f"  Sample size range: {omics_data['n_samples'].min()} - {omics_data['n_samples'].max()}"
        )
        ...
        if len(interesting) > 0:
            ...
            for _, row in interesting.head(5).iterrows():
                print(
                    f"    {row['clinical_test']} vs {row['omics_feature_name']}: r={row['pearson_r']:.3f}, p={row['pearson_p']:.2e}, n={row['n_samples']}"
                )
...
def create_detailed_metabolite_analysis():
    ...
    ax = axes[1, 0]
    test_counts = (
        metabolomics_data.groupby("clinical_test")
        .size()
    ...
    if len(strong_corr) > 0:
        ...
        strong_by_test = (
            strong_corr.groupby("clinical_test")
            .size()
            .sort_values(ascending=False)
            .head(10)
        )
    ...
    for clinical, metabolite_pattern in matches:
        subset = metabolomics_data[
            ...
            & (
                metabolomics_data["omics_feature_name"].str.contains(
                    metabolite_pattern, case=False, na=False
                )
            )
        ]
        if len(subset) > 0:
            ...
            print(
                f"  {clinical} vs {best_match['omics_feature_name']}: r={best_match['pearson_r']:.3f}, p={best_match['pearson_p']:.2e}"
            )
...
if __name__ == "__main__":
    ...
    print(
        "  - metabolite_specific_analysis.png: Detailed metabolite correlation patterns"
    )
