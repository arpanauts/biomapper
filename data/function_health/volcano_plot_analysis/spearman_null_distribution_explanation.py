...
"""
    ...
    ax4.set_title(
        "Parabolic Approximation: -log₁₀(p) ≈ (N-1)ρ²/4.6",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax5.plot(
        rho_range, -np.log10(p_exact), "b-", linewidth=2, label="Exact (t-distribution)"
    )
    ax5.plot(
        rho_range,
        -np.log10(p_approx),
    ...
    ax6.set_title(
        "V-Shape Width: |ρ| threshold ∝ 1/√(N-1)", fontsize=14, fontweight="bold"
    )
    ax6.set_xscale("log")
...
"""
    ...
    for n, color in zip(sample_sizes, colors):
        ...
        ax9.plot(
            rho_range_demo, log_p, color=color, linewidth=3, label=f"N={n}", alpha=0.8
        )

        # Add some scattered points
        n_points = 50
        rho_scatter = np.random.normal(0, 0.05, n_points)
        log_p_scatter = (n - 1) * rho_scatter**2 / 4.6 + np.random.normal(
            0, 0.1, n_points
        )
    ...
    ax9.set_title(
        "Multiple Parabolas = Multiple V-Patterns", fontsize=14, fontweight="bold"
    )
    ax9.legend()
    ...
    plt.tight_layout()
    plt.savefig(
        "spearman_null_distribution_explanation.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
...
def create_simplified_explanation():
    ...
    equations = [
        ...
        (
            0.5,
            0.5,
            r"Therefore: $-\log_{10}(p) \approx \frac{z^2}{4.6} = \frac{(N-1)\rho^2}{4.6}$",
            18,
        ),
    ...
    ax2.set_title(
        "Each Sample Size Creates Its Own Parabola", fontsize=18, fontweight="bold"
    )
...
if __name__ == "__main__":
    print(
        "Creating mathematical explanation of V-shapes based on Spearman's null distribution..."
    )
    ...
    print("\nGenerated files:")
    print(
        "  - spearman_null_distribution_explanation.png: Comprehensive mathematical explanation"
    )
