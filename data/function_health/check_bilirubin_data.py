...
bilirubin_data = correlation_results[correlation_results['clinical_test'] == 'BILIRUBIN, DIRECT']
...
print(f"Range of r values: {common_n_data['pearson_r'].min():.3f} to {common_n_data['pearson_r'].max():.3f}")
print(f"Correlations with |r| < 0.05: {len(common_n_data[np.abs(common_n_data['pearson_r']) < 0.05])}")
...
ax = axes[0, 0]
ax.scatter(bilirubin_prot['pearson_r'], -np.log10(bilirubin_prot['pearson_p']), 
           alpha=0.5, s=20)
...
ax = axes[0, 2]
ax.scatter(bilirubin_prot['pearson_r'], bilirubin_prot['pearson_p'], 
           alpha=0.5, s=20)
ax.set_xlabel('Pearson r')
...
ax = axes[1, 0]
# Plot actual data
ax.scatter(common_n_data['pearson_r'], -np.log10(common_n_data['pearson_p']), 
...
ax = axes[1, 2]
ax.scatter(bilirubin_metab['pearson_r'], -np.log10(bilirubin_metab['pearson_p']), 
...
print("The few significant correlations that deviate from the V are the interesting findings.")