...
class ReferenceFileAuditor:
    ...
    def audit_all_dependencies(self) -> Dict[str, Any]:
        ...
        # Find missing files
        results["missing_files"] = self.find_missing_files(
            all_required, results["existing_files"]
        )
    ...
    def extract_strategy_dependencies(
        ...
        for key, value in parameters.items():
            if isinstance(value, str) and any(
                ref in value.lower() for ref in ["ontology", "reference", "mapping"]
            ):
        ...
        for step in yaml_content.get("steps", []):
            ...
            for param_name, param_value in action_params.items():
                if isinstance(param_value, str):
                    if any(
                        ref in param_name.lower()
                        for ref in ["reference", "ontology", "mapping"]
                    ):
    ...
    def extract_action_dependencies(self, content: str) -> Dict[str, List[str]]:
        ...
        for pattern in file_patterns:
            ...
            for match in matches:
                if any(
                    ref in match.lower() for ref in ["ontology", "reference", "mapping"]
                ):
    ...
    def find_missing_files(
        self, required_files: Set[str], existing_files: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for required_file in required_files:
            ...
            if required_file.endswith("/") and any(
                required_file in existing for existing in existing_files
            ):
                continue
    ...
    def assess_severity(self, file_path: str) -> str:
        ...
        if any(
            critical in file_lower for critical in ["ontology", "mapping", "reference"]
        ):
            return "CRITICAL"
        elif "nightingale" in file_lower:
            return "HIGH"
...
def main():
    ...
    print("Reference file audit complete.")
