...
class UniChemAdapter:
            ...
            if response.status_code == 200:
                ...
                if data:
                    ...
                    for source in data[:3]:  # Show first 3 sources
                        logger.info(f"Source: {source.get('name', 'Unknown')} (ID: {source.get('src_id', 'Unknown')})")
                return True
            else:
                logger.error(f"Failed to connect to UniChem API: {response.status_code}")
                return False
    ...
    async def map_entity(self, source_id, source_type, target_type, **kwargs):
        ...
        logger.info(f"Mapping {source_id} from {source_type} to {target_type} using {self.name}")
        ...
        if not source_ucm_id or not target_ucm_id:
            logger.warning(f"UniChem doesn't support {source_type} to {target_type} mapping")
            return []
        ...
        try:
            ...
            if not results:
                ...
                conn_url = f"{self.base_url}/connectivity/{clean_source_id}/{source_ucm_id}"
            ...
            logger.info(f"Found {len(results)} mappings from {source_id} to {target_type}")
            return results
    ...
    async def store_mapping(self, source_id, source_type, target_id, target_type, confidence, metadata=None):
        """Store a mapping in the cache."""
        logger.info(f"Storing mapping: {source_id} ({source_type}) -> {target_id} ({target_type})")
        return True
...
class SimpleMetadataManager:
    ...
    def update_performance_metrics(self, resource_name, operation, source_type, target_type, elapsed_ms, success):
        """Update performance metrics for a resource."""
        logger.info(f"Updated performance metrics for {resource_name}: {elapsed_ms}ms, success={success}")
        return True
...
class SimpleMetamappingEngine:
    ...
    async def find_mapping_path(self, source_type, target_type):
        ...
        for i in range(len(steps) - 1):
            path.append({
                ...
                "resources": self.metadata_manager.find_resources_by_capability(steps[i], steps[i+1])
            })
    ...
    async def execute_mapping_path(self, source_id, mapping_path, relationship_id=None, **kwargs):
        """Execute a mapping path."""
        logger.info(f"Executing mapping path for {source_id}")
        path_desc = [f"{step['source_type']} → {step['target_type']}" for step in mapping_path]
        ...
        for step_index, step in enumerate(mapping_path):
            ...
            logger.info(f"Step {step_index+1}: Mapping from {source_type} to {target_type}")
            logger.info(f"Current IDs to map: {[id_info['id'] for id_info in current_ids]}")
            ...
            for id_info in current_ids:
                ...
                try:
                    ...
                    if step_results:
                        ...
                        for result in step_results:
                            next_ids.append({
                                "id": result["target_id"],
                                "confidence": base_confidence * result.get("confidence", 1.0),
                                "source": "unichem",
                ...
                if not success:
                    try:
                        ...
                        test_adapter = self.dispatcher.resource_adapters["test_resource"]
                        step_results = await test_adapter.map_entity(
                            current_id, source_type, target_type
                        )
                        
                        if step_results:
                            logger.info(f"Test resource returned {len(step_results)} results")
            ...
            if not next_ids:
                logger.warning(f"Mapping path failed at step {source_type} -> {target_type}")
                return []
            
            current_ids = next_ids
            logger.info(f"After step {step_index+1}, we have {len(current_ids)} mapped entities")
            ...
            try:
                ...
                for result in results:
                    ...
                    self.metadata_manager.cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (source_id, source_type, result["target_id"], target_type, 
                           result["confidence"], path_json, resource_id))
...
class RelationshipMappingExecutor:
    ...
    async def map_entity(self, relationship_id, source_entity, source_ontology=None):
        ...
        cached_result = self.path_finder._get_cached_mapping(relationship_id, source_entity, source_ontology)
        if cached_result:
        ...
        if not path_info:
            logger.warning(f"No relationship mapping path found for {relationship_id}/{source_ontology}")
            return []
        
        logger.info(f"Found relationship path: {path_info['source_ontology']} → {path_info['target_ontology']}")
        ...
        logger.info(f"Ontology path: {ontology_path['source_type']} → {ontology_path['target_type']}")
...
class RelationshipPathFinder:
    ...
    async def discover_paths_for_relationship(self, relationship_id):
        ...
        if not source_prefs:
            logger.warning(f"No ontology preferences for source endpoint {endpoints['source']['id']}")
            # Add default preferences for testing
            source_prefs = [("hmdb", 1), ("chebi", 2), ("pubchem", 3)]
        
        if not target_prefs:
            logger.warning(f"No ontology preferences for target endpoint {endpoints['target']['id']}")
            # Add default preferences for testing
        ...
        for source_ont, source_pref in source_prefs:
            for target_ont, target_pref in target_prefs:
                ...
                if not ont_paths:
                    logger.warning(f"No ontology paths found from {source_ont} to {target_ont}")
                    continue
        ...
        logger.info(f"Discovered {len(paths)} mapping paths for relationship {relationship_id}")
        return paths
    ...
    def _store_relationship_path(self, relationship_id, source_ont, target_ont, path_id, score):
        ...
        try:
            ...
            if existing:
                ...
                self.cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (relationship_id, source_ont, target_ont, path_id, 
                       score, datetime.now().isoformat(), 0))
            
            self.conn.commit()
            logger.info(f"Stored relationship path: {source_ont} → {target_ont} (score: {score})")
            return True
        except Exception as e:
...
async def main():
    ...
    discovered_paths = await path_finder.discover_paths_for_relationship(relationship_id)
    ...
    logger.info(f"Found {len(paths)} relationship mapping paths for relationship {relationship_id}")
    ...
    if paths:
        ...
        for test_id in real_metabolite_ids:
            ...
            if results:
                ...
                for result in results:
                    logger.info(f"  {result['target_id']} (confidence: {result['confidence']})")
        ...
        logger.warning(f"No relationship mapping paths found for relationship {relationship_id}")
