...
async def test_resource_registration() -> ResourceMetadataManager:
    ...
    cache = metadata_manager.register_resource(
        ...
        connection_info={"data_dir": "/home/ubuntu/biomapper/data", "db_name": "mappings.db"},
        priority=10,
    )
...
async def test_mapping_dispatcher(factory: MetadataFactory) -> None:
    ...
    for compound in test_compounds:
        ...
        try:
            ...
            if result:
                resource = result.metadata.get('resource') if hasattr(result, 'metadata') else "unknown"
                response_time = result.metadata.get('response_time_ms') if hasattr(result, 'metadata') else "unknown"
...
async def test_resource_prioritization(metadata_manager: ResourceMetadataManager) -> None:
