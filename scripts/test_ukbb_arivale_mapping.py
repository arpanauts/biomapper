...
async def main():
    ...
    logging.info(f"Executing mapping: {source_endpoint}.{source_property} -> {target_endpoint}.{target_property}")
    ...
    if "results" in mapping_output:
        successful = sum(1 for value in mapping_output["results"].values() if value is not None)
        total = len(mapping_output["results"])
        logging.info(f"Successfully mapped {successful}/{total} gene names to Arivale protein identifiers")
