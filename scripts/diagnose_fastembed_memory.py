...
def log_memory(stage: str):
    ...
    logger.info(f"{stage}:")
    logger.info(f"  Process Memory: {memory_mb:.1f} MB (RSS), {virtual_mb:.1f} MB (VMS)")
    logger.info(f"  System Memory: {system_used_percent:.1f}% used, {system_available_mb:.1f} MB available")
...
def main():
    ...
    logger.info("2. If only large models failed: Use sentence-transformers/all-MiniLM-L6-v2")
    logger.info("3. If system memory is low: Reduce batch sizes and use TF-IDF fallback")
