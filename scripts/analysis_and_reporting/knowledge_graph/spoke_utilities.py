...
from biomapper.spoke.default_schema import (
    get_default_schema_mapping,
    create_spoke_config,
)
...
def check_database_connection(
    host: str, port: int, database: str, username: str = "root", password: str = "ph"
) -> Dict[str, Any]:
    ...
    try:
        ...
        if "Nodes" in [c["name"] for c in collections]:
            ...
            try:
                cursor = db.aql.execute(
                    "FOR doc IN Nodes LIMIT 1 RETURN doc", batch_size=1
                )
                first_node = next(cursor, None)
                if first_node:
                    result["nodes_sample"] = first_node
                    print(
                        f"✓ Successfully sampled a node with keys: {', '.join(first_node.keys())}"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"✓ Found {len(node_types)} node types in {time.time() - start_time:.1f}s"
                        )
                    except Exception as e:
        ...
        if "Edges" in [c["name"] for c in collections]:
            ...
            try:
                cursor = db.aql.execute(
                    "FOR edge IN Edges LIMIT 1 RETURN edge", batch_size=1
                )
                first_edge = next(cursor, None)
                if first_edge:
                    result["edges_sample"] = first_edge
                    print(
                        f"✓ Successfully sampled an edge with keys: {', '.join(first_edge.keys())}"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"✓ Found {len(relationship_types)} relationship types in {time.time() - start_time:.1f}s"
                        )
                    except Exception as e:
