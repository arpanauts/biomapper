...
def explore_spoke_structure(
    ...
    document_collections = [
        c for c in collections if c["type"] == 2
    ...
    try:
        ...
        for node_type in types_to_explore:
            ...
            regular_props = [
                p
                for p in all_props
                if p not in ["_id", "_key", "_rev", "type", "properties"]
            ]
            if regular_props:
                print("Regular properties:")
                for prop in sorted(regular_props):
                    sample_values = [
                        str(node.get(prop, "")) for node in sample_nodes if prop in node
                    ][:3]
                    ...
                    for prop_name, value in props.items():
                        for ont_name in [
                            ...
                            if ont_name in prop_name.lower() and value:
                                print(
                                    f"  - properties.{prop_name}: {value} (likely {ont_name})"
                                )
                                found_ontology = True
                                break
...
def explore_edges_collection(db, limit: int = 10):
    ...
    try:
        ...
        relations_to_explore = [
            ...
            rt["label"] for rt in relationship_types[:5]
        ]  # Use top 5 if specified ones not found
        ...
        for rel_type in relations_to_explore:
            ...
            if sample_edges:
                ...
                edge_props = [
                    k
                    for k in edge.keys()
                    if k not in ["_id", "_key", "_rev", "_from", "_to", "label"]
                ]
                if edge_props:
        ...
        "schema_mapping": {
            "node_types": {
                ...
                "Protein": {
                    ...
                    "property_map": {"properties.uniprot": "uniprot"},
                },
                "Disease": {
...
def main():
    ...
    parser.add_argument(
        "--output", help="Output file for metadata configuration (YAML or JSON)"
    )
    parser.add_argument(
        "--config-only", action="store_true", help="Generate config without exploration"
    )
