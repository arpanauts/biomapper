...
def process_command(args):
    ...
    vector_store = FAISSVectorStore(
        index_path=args.index_path, metadata_path=args.metadata_path
    )

    pipeline = BatchEmbeddingPipeline(
        embedder=embedder, vector_store=vector_store, batch_size=args.batch_size
    )

    # Process data
    stats = pipeline.process_from_jsonl(
        jsonl_path=args.input_file, max_items=args.max_items
    )
...
def search_command(args):
    ...
    search_engine = EmbedderSearchEngine(embedder=embedder, vector_store=vector_store)
    ...
    if args.output_format == "json":
        ...
        formatted_results = search_engine.format_results(
            results=results, format_type=args.output_format
        )
...
def main():
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process data and generate embeddings"
    )
    process_parser.add_argument(
        "--input-file", required=True, help="Input JSONL file to process"
    )
    process_parser.add_argument(
        "--index-path", default=default_index_path, help="Path to save the FAISS index"
    )
    process_parser.add_argument(
        "--metadata-path", default=default_metadata_path, help="Path to save metadata"
    )
    process_parser.add_argument(
        "--model", default=default_config.embedding_model, help="Embedding model name"
    )
    process_parser.add_argument(
        "--batch-size", type=int, default=32, help="Batch size for processing"
    )
    process_parser.add_argument(
        "--max-items", type=int, help="Maximum items to process"
    )
    ...
    search_parser.add_argument(
        "--index-path", default=default_index_path, help="Path to the FAISS index"
    )
    search_parser.add_argument(
        "--metadata-path", default=default_metadata_path, help="Path to metadata"
    )
    search_parser.add_argument(
        "--model", default=default_config.embedding_model, help="Embedding model name"
    )
    ...
    search_parser.add_argument(
        "--filter-types", help="Comma-separated list of types to filter by"
    )
    search_parser.add_argument(
        "--output-format",
    ...
    info_parser = subparsers.add_parser(
        "info", help="Display information about the index"
    )
