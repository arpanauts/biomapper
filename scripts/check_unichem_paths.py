...
async def check_paths():
        ...
        for path in all_paths:
            try:
                ...
                if steps: # Check if steps list is not empty after decoding
                    ...
                    logger.warning(f"Path ID {path.id} has empty 'steps' list after decoding. Skipping.")
            except Exception as e: # Catch potential errors during access/decoding
                 logger.warning(f"Path ID {path.id} encountered error accessing/decoding 'steps': {e}. Skipping.")
    ...
    if valid_paths_details:
        ...
        for p in valid_paths_details:
            steps_str = json.dumps(p['steps']) # Convert steps back to JSON string for logging
            logger.info(f"  ID: {p['id']}, Source: {p['source_type']}, Target: {p['target_type']}, Steps: {steps_str}")
        logger.info("---------------------------------")
    else:
