#!/usr/bin/env python3
"""
Map UKBB protein identifiers (UniProtKB ACs) to QIN gene names.
...
QIN_ENDPOINT_NAME = "QIN_Protein"
TARGET_ENDPOINT_NAME = QIN_ENDPOINT_NAME
TARGET_PROPERTY_NAME = "gene"
TARGET_ONTOLOGY_TYPE = "GENE_NAME"
OUTPUT_FILE_PATH = "/home/ubuntu/biomapper/output/ukbb_to_qin_gene_mapped.tsv"
...
async def main():
    ...
    # 3. Execute mapping from UKBB to QIN
    ...
    # Add QIN_GENE_NAME column to the dataframe
    df_ukbb["QIN_GENE_NAME"] = df_ukbb[UKBB_IDENTIFIER_COLUMN].map(uniprot_to_gene)
    ...
    logger.info(f"Output written successfully. Mapped {df_ukbb['QIN_GENE_NAME'].notna().sum()} records.")

    # Log some statistics
    mapped_count = df_ukbb["QIN_GENE_NAME"].notna().sum()
    total_count = len(df_ukbb)
    logger.info(f"Final mapping statistics: {mapped_count}/{total_count} ({mapped_count/total_count*100:.1f}%) records have QIN gene names")
