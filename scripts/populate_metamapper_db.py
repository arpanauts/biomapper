...
async def populate_data(session: AsyncSession):
    ...
    resources = {
        ...
        "arivale_reverse_lookup": MappingResource(
            ...
            description="Maps Arivale Protein IDs back to UniProt IDs using the Arivale metadata file.",
            client_class_path="biomapper.mapping.clients.arivale_reverse_lookup_client.ArivaleReverseLookupClient",
    ...
    paths = [
        ...
        MappingPath(
            ...
            steps=[
                MappingPathStep(
                    mapping_resource_id=resources[
                        "arivale_lookup"
                    ].id,  # Use the UniProt lookup resource
                    step_order=1,
        ...
        MappingPath(
            name="UniProt_to_Arivale_Direct",
            source_type="UNIPROTKB_AC",
            target_type="ARIVALE_PROTEIN_ID",
            priority=1,
            description="Direct lookup from UniProt AC to Arivale Protein ID using metadata file",
            steps=[
                MappingPathStep(
        ...
        MappingPath(
            name="Arivale_to_UniProt_Direct",
            ...
            steps=[
                MappingPathStep(
                    ...
                    step_order=1,
                    description="Arivale Reverse Lookup: ARIVALE_PROTEIN_ID -> UNIPROTKB_AC",
                )
            ],
        ),
    ]
    ...
    preferences = [
        ...
        OntologyPreference(
            ...
            priority=1,
        ),  # Specify target ontology, not the resource
    ...
    endpoint_prop_configs = [
        EndpointPropertyConfig(
            endpoint_id=endpoints["ukbb_protein"].id,
            property_extraction_config_id=prop_extract_configs[
                3
            ].id,  # Index of UKBB config
        ...
        EndpointPropertyConfig(
            ...
            property_extraction_config_id=prop_extract_configs[
                4
            ].id,  # Index of the Arivale UniProt config
        ...
        EndpointPropertyConfig(
            ...
            endpoint_id=endpoints[
                "ukbb_protein"
            ].id,  # Associate with UKBB Protein arbitrarily for now
            property_name="EnsemblGeneID",
            property_extraction_config_id=prop_extract_configs[
                6
            ].id,  # Ensembl Gene ID extraction
        ),
    ]
