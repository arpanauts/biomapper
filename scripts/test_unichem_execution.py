        ...
        async with aiohttp.ClientSession() as http_session:
            try:
                ...
                if mapping_path.source_type.upper() != "INCHIKEY":
                    logging.error(f"Path {PATH_ID_TO_TEST} source type '{mapping_path.source_type}' does not match expected 'INCHIKEY'.")
                    return

                logging.info(f"Testing path ID {PATH_ID_TO_TEST}: {mapping_path.source_type} -> {mapping_path.target_type}")
                logging.info(f"Input Entity ({mapping_path.source_type}): {INPUT_INCHIKEY}")
                ...
                output_entity, score = await executor.execute_mapping(
                    input_entity=INPUT_INCHIKEY,
                    mapping_path=mapping_path
                )

                logging.info(f"Execution Result:")
                logging.info(f"  Output Entity ({mapping_path.target_type}): {output_entity}")
