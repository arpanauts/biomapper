...
async def migrate_paths(session: AsyncSession):
    ...
    for path in all_paths:
        ...
        if not original_steps_str:
            logger.warning(
                f"Path ID {path.id}: Original path_steps is empty or null. Skipping."
            )
            continue
        ...
        try:
            ...
            if not isinstance(old_steps, list):
                logger.error(
                    f"Path ID {path.id}: path_steps is not a JSON list. Content: {original_steps_str}. Skipping."
                )
                failed_count += 1
                continue
            ...
            if not old_steps:
                logger.warning(
                    f"Path ID {path.id}: path_steps JSON list is empty. Skipping."
                )
                continue
            ...
            for i, old_step in enumerate(old_steps):
                if not isinstance(old_step, dict):
                    logger.error(
                        f"Path ID {path.id}, Step {i+1}: Step is not a dictionary. Content: {old_step}. Path conversion failed."
                    )
                    conversion_failed = True
                    break

                # Extract source, target, and resource
                source_type = extract_value(
                    old_step, ["source_type", "source", "from_type"]
                )
                target_type = extract_value(
                    old_step, ["target_type", "target", "to_type"]
                )
                resource_name: Optional[str] = None
                if (
                    "resources" in old_step
                    and isinstance(old_step["resources"], list)
                    and old_step["resources"]
                ):
                    resource_name = str(
                        old_step["resources"][0]
                ...
                if not source_type or not target_type:
                    logger.error(
                        f"Path ID {path.id}, Step {i+1}: Could not extract source ('{source_type}') or target ('{target_type}'). Path conversion failed."
                    )
                    conversion_failed = True
                    break
                ...
                if resource_id is None:
                    logger.error(
                        f"Path ID {path.id}, Step {i+1}: Could not find resource ID for name '{resource_name}'. Path conversion failed."
                    )
                    conversion_failed = True
                    break
            ...
            if new_steps_str != original_steps_str:
                logger.info(
                    f"Path ID {path.id}: Converted successfully. Original: {original_steps_str}, New: {new_steps_str}"
                )
                paths_to_update.append({"id": path.id, "path_steps": new_steps_str})
            else:
                logger.info(
                    f"Path ID {path.id}: Already in correct format or conversion resulted in no change. Skipping update."
                )
                skipped_no_change += 1

        except json.JSONDecodeError:
            logger.error(
                f"Path ID {path.id}: Failed to decode original path_steps JSON: {original_steps_str}. Skipping."
            )
            failed_count += 1
        except Exception as e:
            logger.error(
                f"Path ID {path.id}: Unexpected error during conversion: {e}. Skipping."
            )
            failed_count += 1
    ...
    if paths_to_update:
        logger.info(
            f"Attempting to update {len(paths_to_update)} paths in the database..."
        )
        try:
            # Using Core API update for potentially better performance with many updates
            update_stmt = (
                update(MappingPath)
                .where(MappingPath.id == sqlalchemy.bindparam("path_id"))
                .values(path_steps=sqlalchemy.bindparam("new_steps"))
            )
            await session.execute(
                update_stmt,
                [
                    {"path_id": p["id"], "new_steps": p["path_steps"]}
