...
# UKBB_Protein uses "PrimaryIdentifier", HPA_Protein uses "gene"
# These should match the property_name in EndpointPropertyConfig table
SOURCE_PROPERTY_NAME = "UniProt"  # UKBB_Protein source property (UniProt column)
TARGET_PROPERTY_NAME = "gene"   # HPA_Protein target property (gene column)
...
async def main(
    ...
    try:
        ...
        # df_input = df_input.head(10)  # Limit to first 10 rows for testing
    ...
    if input_id_column_name not in df_input.columns:
        logger.error(
            f"Error: Input file must contain column '{input_id_column_name}'. Found: {df_input.columns.tolist()}"
        )
        return
    if input_primary_key_column_name not in df_input.columns:
        logger.error(
            f"Error: Input file must contain column '{input_primary_key_column_name}'. Found: {df_input.columns.tolist()}"
        )
        return
    ...
    for source_id, result_data in results_dict.items():
        if result_data and "target_identifiers" in result_data:
            ...
            if target_ids and len(target_ids) > 0:
                ...
                matching_rows = df_input[df_input[input_id_column_name] == source_id]
