...
def validate_against_schema(config: Dict[str, Any], schema_path: Path) -> bool:
    ...
    try:
        ...
        print("✅ Configuration validates against schema")
        return True
                ...
                if param_parts[0] == "parameters":
                    ...
                    for part in param_parts[1:]:
                        if isinstance(current, dict) and part in current:
                            ...
                            issues.append(
                                f"Unresolved parameter reference at {path}: {param_ref}"
                            )
                            break
    ...
    for i, step in enumerate(config.get("steps", [])):
        ...
        for param_name, param_value in params.items():
            ...
            if isinstance(param_value, str) and (
                param_value.endswith("_data")
                or param_value.endswith("_matches")
                or param_value == "nightingale_reference"
            ):
                if param_value not in available_keys and not param_value.startswith(
                    "unmatched."
                ):
                    issues.append(
                        f"Step '{step_name}' references undefined dataset key: {param_value}"
                    )
            elif param_name == "dataset_keys" and isinstance(param_value, list):
                for key in param_value:
                    if key not in available_keys:
                        issues.append(
                            f"Step '{step_name}' references undefined dataset key: {key}"
                        )
...
def main():
    ...
    strategy_file = (
        config_dir / "strategies" / "metabolomics_progressive_enhancement.yaml"
    )
    ...
    print(
        f"   - Expected outcomes: {config.get('metadata', {}).get('expected_outcomes', {})}"
    )
    print(
        f"   - Error handling: {'enabled' if config.get('error_handling', {}).get('log_errors') else 'disabled'}"
    )
    print(
        f"   - Logging level: {config.get('logging', {}).get('level', 'Not specified')}"
    )
    ...
    if qdrant_config.exists():
        ...
        if qdrant_data:
            print(
                f"   ✅ Qdrant configuration loaded: {qdrant_data.get('name', 'Unknown')}"
            )
    else:
