        ...
        async with session.begin():
            ...
            for pref_data in PREFERENCES_TO_ADD:
                ...
                if not endpoint_check or endpoint_check.name != endpoint_name:
                    logger.error(f"Endpoint mismatch or not found: ID {endpoint_id}, Expected Name '{endpoint_name}'. Skipping preference.")
                    error_count += 1
                    continue

                # Check if preference already exists for the endpoint
                stmt = select(OntologyPreference).where(
                    OntologyPreference.endpoint_id == endpoint_id
                ).order_by(OntologyPreference.priority.asc()).limit(1)
                result = await session.execute(stmt)
                existing_pref = result.scalar_one_or_none()

                if existing_pref:
                    if existing_pref.ontology_name == ontology_name:
                         logger.info(f"Preference already exists for {endpoint_name} -> {ontology_name}. Skipping.")
                         skipped_count += 1
                    else:
                         logger.warning(f"Existing preference found for {endpoint_name} but with different ontology: {existing_pref.ontology_name}. Consider manual review.")
                    ...
                    added_count += 1
                    logger.info(f"Adding preference: {endpoint_name} -> {ontology_name}")

        logger.info(f"Finished adding preferences. Added: {added_count}, Skipped: {skipped_count}, Errors: {error_count}")
