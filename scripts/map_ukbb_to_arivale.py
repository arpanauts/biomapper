...
async def main(log_level_str: str):  # Pass log level to main
    ...
    async with async_session_factory() as meta_session:
        ...
        source_id_column, source_file_path, source_identifiers, source_df = (
        ...
        if not source_identifiers:
            ...
            return
        logger.info(f"Successfully loaded source data: {len(source_identifiers)} identifiers")

        # --- Load Target Identifiers (for joining later) ---
        target_id_column, target_file_path, target_ids, target_df = (
            await load_identifiers_from_endpoint(meta_session, TARGET_ENDPOINT_NAME)
        )
        if not target_ids:
            logger.warning("Failed to load target identifiers. Reverse mapping will be skipped.")
            ...
            logger.info(f"Successfully loaded target data: {len(target_ids)} identifiers")
        ...
        if target_ids:
            ...
            logger.info(f"Reverse mapping finished. Successfully mapped {reverse_successful} / {len(target_ids)} target identifiers.")
        else:
            logger.info("Skipping reverse mapping as no target identifiers were loaded.")

    # Check if source_ids were actually loaded before proceeding
    if not source_identifiers:
        logger.error("Source identifiers were not loaded, cannot proceed with forward mapping.")
        return
    ...
    if mapping_results:
        ...
        successfully_mapped_count = sum(1 for v in mapping_results.values() if v is not None)
        ...
        if not successful_mappings.empty:
            ...
            async with AsyncSession(cache_engine) as cache_session:
                logger.info(f"Retrieving metadata for {len(successful_mappings)} successful mappings...")
                for _, row in successful_mappings.iterrows():
                    ...
                    stmt = (
                        ...
                        .where(
                            ...
                            EntityMapping.source_type == "UNIPROTKB_AC", # Correct field and internal name
                            EntityMapping.target_type == "ARIVALE_PROTEIN_ID", # Correct field and internal name
                            EntityMapping.mapping_direction == "forward" # Explicitly forward
                        )
                        .order_by(EntityMapping.last_updated.desc()) # Use correct field name
                    )
                    result = await cache_session.execute(stmt)
                    mapping_entry = result.scalars().first()
                    if mapping_entry:
                        mapping_metadata_dict[source_id] = mapping_entry
                    else:
                         # Should not happen if mapping succeeded, but log if it does
                         logger.warning(f"Could not find EntityMapping metadata for SUCCESSFUL mapping: {source_id} -> {target_id}")
                logger.info(f"Found metadata for {len(mapping_metadata_dict)} mappings.")

        # 4. Add enhanced metadata fields where available using the fetched data
        mapping_df["confidence"] = mapping_df["source_id"].apply(
            lambda x: mapping_metadata_dict.get(x).confidence if x in mapping_metadata_dict else None
        )
        mapping_df["confidence_score"] = mapping_df["source_id"].apply(
            lambda x: mapping_metadata_dict.get(x).confidence_score if x in mapping_metadata_dict else None
        )
        mapping_df["mapping_path_details"] = mapping_df["source_id"].apply(
            lambda x:
                mapping_metadata_dict.get(x).mapping_path_details
                if x in mapping_metadata_dict
                else None
        )
        mapping_df["hop_count"] = mapping_df["source_id"].apply(
            lambda x: mapping_metadata_dict.get(x).hop_count if x in mapping_metadata_dict else None
        )
        mapping_df["mapping_direction"] = mapping_df["source_id"].apply(
            lambda x:
                mapping_metadata_dict.get(x).mapping_direction
                if x in mapping_metadata_dict
        ...
        for col in source_cols:
            source_df_processed = source_df_processed.rename(columns={col: f"source_{col}"})
        ...
        for col in target_cols:
            target_df_processed = target_df_processed.rename(columns={col: f"target_{col}"})
        ...
        # 9. Fix mapping_status for target-only entries
        mask_target_only = final_df["source_id"].isna() & final_df["target_id"].notna()
        final_df.loc[mask_target_only, "mapping_status"] = "TARGET_ONLY"

        # Ensure all rows have a mapping status
        ...
        target_only_count = sum(mask_target_only)
        matched_count = sum(
