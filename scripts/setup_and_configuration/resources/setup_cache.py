...
def main():
    ...
    # Common arguments
    parser.add_argument(
        "--data-dir", help="Data directory for cache files", default=None
    )
    ...
    # Initialize command
    init_parser = subparsers.add_parser("init", help="Initialize the cache database")
    init_parser.add_argument(
        "--reset", action="store_true", help="Reset the database if it already exists"
    )
    ...
    backup_parser.add_argument("--backup-dir", help="Directory for storing backups")
    backup_parser.add_argument(
        "--no-compress", action="store_true", help="Don't compress the backup file"
    )
    ...
    restore_parser.add_argument("--backup-dir", help="Directory containing backups")
    ...
    list_parser.add_argument("--backup-dir", help="Directory containing backups")
    list_parser.set_defaults(func=list_backups)

    # Maintenance command
    maintain_parser = subparsers.add_parser("maintain", help="Run maintenance tasks")
    maintain_parser.add_argument(
        "--no-vacuum", action="store_true", help="Skip database vacuum"
    )
    maintain_parser.add_argument(
        "--no-backup",
        action="store_true",
