...
ramp_config = {"base_url": "https://rampdb.nih.gov/api", "timeout": 30}

# Update RaMP-DB configuration
try:
    # Check if resource exists
    cursor.execute(
        "SELECT id, name, client_type, config FROM resources WHERE name = 'RaMP-DB'"
    )
    ...
    # Verify the changes
    cursor.execute(
        "SELECT id, name, description, client_type, config FROM resources WHERE name = 'RaMP-DB'"
    )
    resource = cursor.fetchone()
