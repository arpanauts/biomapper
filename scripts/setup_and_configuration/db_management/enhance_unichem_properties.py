...
def test_property_extraction(resource_id: int, property_name: str) -> bool:
    ...
    try:
        ...
        if property_name in result and result[property_name]:
            logger.info(
                f"Successfully extracted {property_name}: {result[property_name]}"
            )
            return True
        else:
...
def update_unichem_client_sources():
    ...
    result_init_start = new_content.find(
        "result = self._get_empty_result()", method_body_start
    )
    if result_init_start == -1:
        logger.error(
            "Could not find result initialization in _process_compound_result method"
        )
        return False
    ...
    if empty_result_return_end == -1:
        logger.error(
            "Could not find end of return statement in _get_empty_result method"
        )
        return False

    # Extract the current return statement
    current_empty_result = new_content[
        empty_result_return_start : empty_result_return_end + 1
    ]
    ...
    for source_name, source_id in ADDITIONAL_SOURCES.items():
        source_handling_entries.append(
            f"""            elif src_id == {source_id}:
    ...
    if insertion_point == -1:
        logger.error(
            "Could not find return statement in _process_compound_result method"
        )
        return False
...
try:
        ...
        for source_name, source_id in list(ADDITIONAL_SOURCES.items()):
            ...
            for api_source in unichem_sources:
                if api_source.get("src_id") == source_id:
                    logger.info(
                        f"Validated source {source_name} (ID: {source_id}, Name: {api_source.get('name')})"
                    )
                    found = True
                    break
            ...
            if not found:
                logger.warning(
                    f"Source ID {source_id} for {source_name} not found in API - removing"
                )
                del ADDITIONAL_SOURCES[source_name]
    else:
        logger.warning(
            "Could not fetch UniChem sources from API - proceeding with predefined sources"
        )
    ...
    for config in list(new_configs):
        ...
        if cursor.fetchone():
            logger.info(
                f"Configuration for {config['property_name']} already exists - skipping"
            )
    ...
    conn.commit()
    logger.info(
        f"Successfully added {len(new_configs)} new UniChem property extraction configurations"
    )
    ...
    for config in all_configs:
        logger.info(
            f"  - {config['property_name']} ({config['extraction_method']}): {config['extraction_pattern']}"
        )
