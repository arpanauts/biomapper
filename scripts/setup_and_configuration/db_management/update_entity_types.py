...
try:
    ...
    if "entity_type" not in columns:
        ...
        cursor.execute(
            "ALTER TABLE resources ADD COLUMN entity_type TEXT DEFAULT 'metabolite'"
        )
    ...
    if null_resources:
        ...
        cursor.execute(
            "UPDATE resources SET entity_type = 'metabolite' WHERE entity_type IS NULL"
        )
        print(
            f"Updated {cursor.rowcount} resources with default entity_type = 'metabolite'"
        )
...
print("\nNext steps:")
print(
    "1. Review current property configurations for compatibility with multiple entity types"
)
