...
def setup_pubchem_configs(overwrite=False):
    ...
    if check_existing_configs(resource_id) and not overwrite:
        print(
            f"PubChem already has property extraction configurations. Use --overwrite to replace them."
        )
        return

    if overwrite:
        cursor.execute(
            "DELETE FROM property_extraction_configs WHERE resource_id = ?",
            (resource_id,),
        )
    ...
    configs = [
        ...
        (
            ...
            "description",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Computed Descriptors')].Information[?(@.Name=='IUPAC Name')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "synonyms",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Synonyms')].Section[?(@.TOCHeading=='Depositor-Supplied Synonyms')].Information[?(@.Name=='Depositor-Supplied Synonyms')].StringValueList",
            "list",
        ),
        # Chemical properties
        (
            ...
            "formula",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Molecular Formula')].Information[?(@.Name=='Molecular Formula')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "molecular_weight",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Chemical and Physical Properties')].Section[?(@.TOCHeading=='Computed Properties')].Information[?(@.Name=='Molecular Weight')].NumValue",
            "number",
        ),
        (
            "PUBCHEM",
            "inchi",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Computed Descriptors')].Information[?(@.Name=='InChI')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "inchikey",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Computed Descriptors')].Information[?(@.Name=='InChIKey')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "smiles",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Computed Descriptors')].Information[?(@.Name=='Canonical SMILES')].StringValue",
            "string",
        ),
        ...
        (
            ...
            "hmdb_id",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Other Identifiers')].Information[?(@.Name=='HMDB')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "kegg_id",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Other Identifiers')].Information[?(@.Name=='KEGG')].StringValue",
            "string",
        ),
        (
            "PUBCHEM",
            "chebi_id",
            "json_path",
            "$.Record.Section[?(@.TOCHeading=='Names and Identifiers')].Section[?(@.TOCHeading=='Other Identifiers')].Information[?(@.Name=='ChEBI')].StringValue",
            "string",
        ),
    ]
...
def setup_kegg_configs(overwrite=False):
    ...
    if check_existing_configs(resource_id) and not overwrite:
        print(
            f"KEGG already has property extraction configurations. Use --overwrite to replace them."
        )
        return
    ...
    configs = [
        ...
        (
            ...
            "smiles",
            "regex",
            r"db:pubchem-substance\s+exact_mass\s+\d+\.\d+\s+smiles\s+(.*?)(?=\n)",
            "string",
        ),
        (
            "KEGG",
            "inchi",
            "regex",
            r"db:pubchem-substance\s+inchi\s+(.*?)(?=\n)",
            "string",
        ),
    ]
...
def setup_unichem_configs(overwrite=False):
    ...
    if check_existing_configs(resource_id) and not overwrite:
        print(
            f"UniChem already has property extraction configurations. Use --overwrite to replace them."
        )
        return
...
def setup_refmet_configs(overwrite=False):
    ...
    if check_existing_configs(resource_id) and not overwrite:
        print(
            f"RefMet already has property extraction configurations. Use --overwrite to replace them."
        )
        return
...
def setup_rampdb_configs(overwrite=False):
    ...
    if check_existing_configs(resource_id) and not overwrite:
        print(
            f"RaMP-DB already has property extraction configurations. Use --overwrite to replace them."
        )
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Set up property extraction configurations for resources"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing property configurations",
    )
    parser.add_argument(
        "--resource", type=str, help="Only configure a specific resource (by name)"
    )
