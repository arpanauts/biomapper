...
try:
    ...
    for name, config in resource_configs.items():
        ...
        if resource:
            ...
            try:
                current_config = (
                    json.loads(current_config_json) if current_config_json else {}
                )
    ...
    # Verify the changes
    cursor.execute(
        "SELECT id, name, description, client_type, entity_type, config FROM resources"
    )
    ...
    for resource in resources:
        print(
            f"ID: {resource[0]}, Name: {resource[1]}, Client Type: {resource[3]}, Entity Type: {resource[4]}"
        )
