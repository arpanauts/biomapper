    ...
    def _get_embedding_model(self):
        ...
        try:
            ...
            if self.embedding_approach == "fastembed-mini":
                ...
                raise ValueError(
                    f"Unknown embedding approach: {self.embedding_approach}"
                )
            ...
            logging.info(
                f"Memory after model init: {memory_after:.1f} MB (+{memory_used:.1f} MB)"
            )
        ...
        try:
            async for batch in self.processor.process_metabolite_batch(self.batch_size):
                try:
                    ...
                    memory_after = process.memory_info().rss / 1024 / 1024
                    logging.debug(
                        f"Embedding generation used {memory_after - memory_before:.1f} MB"
                    )
                    ...
                    # Progress logging
                    if (
                        total_processed % 250 == 0
                    ):  # More frequent logging for smaller batches
                        memory_current = process.memory_info().rss / 1024 / 1024
                        logging.info(
                            f"Progress: {total_processed:,} metabolites processed, Memory: {memory_current:.1f} MB"
                        )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Memory-efficient HMDB metabolite loading"
    )
    parser.add_argument(
        "--xml-path", type=Path, required=True, help="Path to HMDB XML file"
    )
    parser.add_argument(
        "--qdrant-url", type=str, default="localhost:6333", help="Qdrant URL"
    )
    parser.add_argument(
        "--collection-name",
        type=str,
        default="hmdb_metabolites_lite",
        help="Collection name",
    )
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Batch size (smaller = less memory)"
    )
    parser.add_argument(
        "--embedding-approach",
        type=str,
        default="fastembed-mini",
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"hmdb_qdrant_lightweight_{args.embedding_approach}.log"
            ),
        ],
    )
