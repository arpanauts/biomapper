...
SAMPLE_SEARCH_TERMS = {
    ...
    "pathway": ["glycolysis", "krebs cycle", "electron transport", "oxidative phosphorylation"],
    "all": ["glucose", "hemoglobin", "HBB", "diabetes", "glycolysis"]  # Mixed entities for general search testing
}
...
def entity_aware_verification(db_path: Path, resource_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for resource in resources:
            if 'entity_type' not in resource:
                cursor.execute(
                    "SELECT entity_type FROM resources WHERE id = ?",
                    (resource['id'],)
                )
        ...
        for resource in resources:
            ...
            if test_ids:
                logger.info(f"Using entity-specific test IDs for {resource_name} (entity_type: {entity_type})")
    ...
    for entity_type, resources in by_entity_type.items():
        ...
        for resource in resources:
            ...
            if total_props > 0:
                ...
                print(f"  - Property extraction rate: {success_rate:.1f}% ({successful_props}/{total_props})")
            else:
...
def save_entity_aware_report(results: Dict[int, Any], output_path: Optional[Path] = None) -> None:
    """
        ...
        for id_key, extractions in result.property_extractions.items():
            ...
            for prop_name, extraction in extractions.items():
                if hasattr(extraction, '__dict__'):
                    resource_result["property_extractions"][id_key][prop_name] = extraction.__dict__
                else:
                    resource_result["property_extractions"][id_key][prop_name] = extraction
    ...
    report = {
        ...
        "summary": {
            ...
            "by_entity_type": {
                entity_type: {
                    "count": len(resources),
                    "success_rate": sum(1 for r_id, r in resources.items() if r["success"]) / len(resources) if resources else 0
                } for entity_type, resources in by_entity_type.items()
            }
        },
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Entity-aware verification of resources")
    ...
    parser.add_argument("--output", type=str, help="Output path for the verification report", default="entity_verification_report.json")
    args = parser.parse_args()
    
    try:
        results = entity_aware_verification(
            Path(args.db),
            args.resource
        )
