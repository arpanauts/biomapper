...
async def run_three_way_pipeline():
    ...
    # Initialize context
    context = {"custom_action_data": {}, "results": {}}
    ...
    try:
        ...
        if result.success:
            logger.info(
                f"✓ Loaded {len(context['results'].get('israeli10k_data', []))} Israeli10K metabolites"
            )
        else:
        ...
        if result.success:
            logger.info(
                f"✓ Loaded {len(context['results'].get('ukbb_data', []))} UKBB metabolites"
            )
        else:
        ...
        if result.success:
            logger.info(
                f"✓ Loaded {len(context['results'].get('arivale_data', []))} Arivale metabolites"
            )
        else:
        ...
        if os.environ.get("OPENAI_API_KEY"):
            logger.info(
                "Tier 3: Semantic matching using embeddings and LLM validation..."
            )
            ...
            logger.info(
                f"✓ Semantic matching found {semantic_matches} additional matches"
            )
        else:
        ...
        if result.success:
            ...
            logger.info(
                f"✓ Found {three_way} metabolites present in all three datasets"
            )
