...
from biomapper.core.strategy_actions.nightingale_nmr_match import NightingaleNmrMatchAction
from biomapper.core.strategy_actions.build_nightingale_reference import BuildNightingaleReferenceAction
from biomapper.core.strategy_actions.baseline_fuzzy_match import BaselineFuzzyMatchAction
from biomapper.core.strategy_actions.cts_enriched_match import CtsEnrichedMatchAction
from biomapper.core.strategy_actions.vector_enhanced_match import VectorEnhancedMatchAction
from biomapper.core.strategy_actions.generate_enhancement_report import GenerateEnhancementReport
from biomapper.core.strategy_actions.calculate_set_overlap import CalculateSetOverlapAction
            ...
            try:
                ...
                if action_type == 'LOAD_DATASET_IDENTIFIERS':
                    ...
                    if 'custom_action_data' in self.context:
                        logger.info(f"Custom action data: {list(self.context['custom_action_data'].keys())}")
                        if 'datasets' in self.context['custom_action_data']:
                            logger.info(f"Loaded datasets: {list(self.context['custom_action_data']['datasets'].keys())}")
    ...
    async def execute_action(self, action_type: str, params: Dict[str, Any]):
        ...
        if hasattr(action, 'execute_typed'):
            ...
            result = await action.execute_typed(
                ...
                current_ontology_type=self.context.get('current_ontology_type', 'metabolite'),
                params=typed_params,
