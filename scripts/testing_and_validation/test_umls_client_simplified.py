#!/usr/bin/env python
"""
Test script for the simplified UMLSClientSimplified

This script tests the simplified UMLSClientSimplified with sample metabolite names.
"""
...
# Dynamically import the UMLSClientSimplified
spec = importlib.util.spec_from_file_location(
    "umls_client_simplified", 
    "/home/ubuntu/biomapper/biomapper/mapping/clients/umls_client_simplified.py"
)
umls_client_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(umls_client_module)
UMLSClientSimplified = umls_client_module.UMLSClientSimplified
...
# Sample metabolite names for testing (using a smaller set)
TEST_METABOLITES = [
    "glucose",
    "cholesterol",
    "lactate",
...
async def test_umls_client(target_db: str = "CHEBI"):
    """
    Test the simplified UMLSClientSimplified with sample metabolite names.
    ...
    logger.info(f"Testing simplified UMLSClientSimplified with target_db={target_db}")
    
    # Initialize the client
    client = UMLSClientSimplified(config={"target_db": target_db, "api_key": api_key})
