#!/usr/bin/env python
"""
Test the TranslatorNameResolverClient on real metabolite data.

This script processes three different metadata files, adding name resolver 
results as new columns to each, and saves the results as new TSV files.
"""
...
async def process_chemistries_metadata():
    ...
    # Extract the "Name" column values (process all rows)
    names = df["Name"].tolist()
    
    # Initialize the TranslatorNameResolverClient with a default target_db
    # We'll override this for each specific resolution call
    ...
    try:
        ...
        for target_db in TARGET_DBS:
            results[target_db] = await resolve_names(client, names, target_db)
        
        # Add results as new columns
        for target_db in TARGET_DBS:
            # Add identifier column
            df[f"{target_db}_IDs"] = df["Name"].apply(
                lambda name: ",".join(results[target_db].get(name, (None, None))[0] or [])
            )
            
            # Add confidence column
            df[f"{target_db}_Confidence"] = df["Name"].apply(
                lambda name: results[target_db].get(name, (None, None))[1]
            )
...
async def process_ukbb_nmr_metadata():
    ...
    # Extract the "title" column values (process all rows)
...
async def process_metabolomics_metadata():
    ...
    # Extract the "BIOCHEMICAL_NAME" column values (process all rows)
