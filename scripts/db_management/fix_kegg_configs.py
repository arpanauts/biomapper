...
def fix_kegg_configs(conn, dry_run=True):
    ...
    print(
        f"{'ID':<5} {'Property Name':<20} {'Current Pattern':<50} {'Proposed Pattern':<50} {'Update?':<8}"
    )
    print("-" * 135)
    ...
    for config in configs:
        ...
        if property_name in improved_patterns:
            ...
            needs_update = (
                ...
                or config["extraction_method"] != proposed_method
            )
            ...
            print(
                f"{config['id']:<5} {property_name:<20} {current_display:<50} {proposed_display:<50} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix KEGG property extraction configurations."
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't make any changes to the database"
    )
    parser.add_argument(
        "--db-path", type=str, default="data/metamapper.db", help="Path to the database"
    )
