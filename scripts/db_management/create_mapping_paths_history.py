...
def log_mapping_path_change(
    ...
    if not path and event_type == 'deleted':
        ...
        if not last_entry:
            raise ValueError(f"No history found for deleted mapping path ID {mapping_path_id}")
        ...
        cursor.execute(
            ...
            """,
            (
                mapping_path_id, last_entry['source_type'], last_entry['target_type'],
                last_entry['path_steps'], last_entry['performance_score'],
                last_entry['success_rate'], last_entry['usage_count'],
                'deleted', reason, previous_id
            )
        )
    else:
        ...
        cursor.execute(
            ...
            """,
            (
                path['id'], path['source_type'], path['target_type'], 
                path['path_steps'], path['performance_score'], 
                path['success_rate'], path['usage_count'],
                event_type, reason, previous_id
            )
        )
...
def compare_path_versions(
    ...
    cursor.execute(
        "SELECT * FROM mapping_paths_history WHERE id = ?",
        (history_id1,)
    )
    ...
    if not version1 or not version2:
        raise ValueError(f"One or both history entries not found: {history_id1}, {history_id2}")
    ...
    for field in ['performance_score', 'success_rate', 'usage_count']:
        if v1[field] != v2[field]:
            differences[field] = {
                "old": v1[field],
                "new": v2[field],
                "change": v2[field] - v1[field] if (v1[field] is not None and v2[field] is not None) else None
            }
    ...
    try:
        ...
        if steps1 != steps2:
            differences['path_steps'] = {
                ...
                "new": steps2,
                "changes": {
                    "steps_added": len(steps2) - len(steps1) if len(steps2) > len(steps1) else 0,
                    "steps_removed": len(steps1) - len(steps2) if len(steps1) > len(steps2) else 0,
                    "steps_modified": sum(1 for i in range(min(len(steps1), len(steps2))) if steps1[i] != steps2[i])
                }
            }
    except (json.JSONDecodeError, TypeError):
        differences['path_steps'] = {
            "error": "Could not parse path steps as JSON"
        }
    ...
    differences['metadata'] = {
        ...
        "time_difference": v2['timestamp'] - v1['timestamp'] if (v1['timestamp'] and v2['timestamp']) else None
    }
...
try:
    ...
    if mapping_paths_count > 0 and history_count == 0:
        print(f"Logging initial state for {mapping_paths_count} existing mapping paths...")
    ...
    print("   log_mapping_path_change(mapping_path_id=1, event_type='updated', reason='Improved performance')")
