...
def update_metabolitescsv_endpoint(conn, arivale_id):
    ...
    conn.commit()
    print(f"Updated MetabolitesCSV (ID: {metabolites_id}) to reference Arivale as parent.")
...
def create_clinical_labs_endpoint(conn, arivale_id):
    ...
    """, (
        ...
        "file",
        json.dumps({
            "file_path": os.path.join(ARIVALE_DATA_DIR, "chemistries_metadata.tsv"),
            "delimiter": "\t"
        }),
        arivale_id,
        "clinical_labs",
...
def create_proteomics_endpoint(conn, arivale_id):
    ...
    """, (
        ...
        "file",
        json.dumps({
            "file_path": os.path.join(ARIVALE_DATA_DIR, "proteomics_corrected.tsv"),
            "metadata_file": os.path.join(ARIVALE_DATA_DIR, "proteomics_metadata.tsv"),
            "delimiter": "\t"
        }),
        arivale_id,
        "proteomics",
...
def setup_clinical_labs_property_configs(conn, labs_id):
    ...
    labs_configs = [
        (labs_id, 'loinc', 'Labcorp LOINC ID', 'column', json.dumps({"column_name": "Labcorp LOINC ID"}), None),
        (labs_id, 'loinc', 'Quest LOINC ID', 'column', json.dumps({"column_name": "Quest LOINC ID"}), None),
        (labs_id, 'labcorp_id', 'Labcorp ID', 'column', json.dumps({"column_name": "Labcorp ID"}), None),
        (labs_id, 'quest_id', 'Quest ID', 'column', json.dumps({"column_name": "Quest ID"}), None),
        (labs_id, 'name', 'Name', 'column', json.dumps({"column_name": "Name"}), None),
        (labs_id, 'display_name', 'Display Name', 'column', json.dumps({"column_name": "Display Name"}), None)
    ]
...
def setup_proteomics_property_configs(conn, proteomics_id):
    ...
    proteomics_configs = [
        (proteomics_id, 'uniprot', 'UniProt ID', 'pattern', 
         json.dumps({"pattern": "^(CAM|CRE|CVD2|CVD3|DEV|INF|IRE|MET|NEU1|NEX|ODA|ONC2|ONC3)_([A-Z0-9]+)$", "group": 2}), 
         "extract_uniprot_from_column_name"),
        (proteomics_id, 'chip_id', 'Chip ID', 'pattern', 
         json.dumps({"pattern": "^(CAM|CRE|CVD2|CVD3|DEV|INF|IRE|MET|NEU1|NEX|ODA|ONC2|ONC3)_([A-Z0-9]+)$", "group": 1}), 
         None),
        (proteomics_id, 'public_client_id', 'Client ID', 'column', 
         json.dumps({"column_name": "public_client_id"}), 
         None),
        (proteomics_id, 'sample_id', 'Sample ID', 'column', 
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if header_line:
                ...
                for i, line in enumerate(data_lines):
                    ...
                    print(f"  Row {i+1}: {row[:min(5, len(row))]}..." + (" [truncated]" if len(row) > 5 else ""))
            else:
...
def setup_ontology_preferences(conn, endpoint_ids):
    ...
    for endpoint_id, ontology_type, preference_level in all_preferences:
        ...
        if cursor.fetchone()[0] > 0:
            print(f"Ontology preference for {ontology_type} already exists for endpoint {endpoint_id}")
            continue
...
def update_endpoint_relationships(conn):
    ...
    print(f"Found {len(relationships)} relationship members with hierarchical endpoints.")
...
def main():
    ...
    try:
        ...
        inspect_file_columns(os.path.join(ARIVALE_DATA_DIR, "metabolomics_metadata.tsv"))
        ...
        print("2. Three child endpoints for different data types: metabolomics, clinical labs, and proteomics")
