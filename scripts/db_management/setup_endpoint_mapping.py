...
def setup_endpoints(conn):
    ...
    cursor.execute("SELECT COUNT(*) FROM endpoints WHERE name IN ('MetabolitesCSV', 'SPOKE')")
...
def setup_mapping_resources(conn):
    ...
    cursor.execute("SELECT COUNT(*) FROM mapping_resources WHERE name IN ('UniChem', 'KEGG')")
    if cursor.fetchone()[0] > 0:
...
def setup_endpoint_property_configs(conn):
    ...
    metabolitescsv_configs = [
        ...
        (7, 'pubchem', 'PubChem ID', 'column', json.dumps({"column_name": "PUBCHEM"}), None),
        (7, 'cas', 'CAS Number', 'column', json.dumps({"column_name": "CAS"}), None),
        (7, 'name', 'Compound Name', 'column', json.dumps({"column_name": "BIOCHEMICAL_NAME"}), None)
    ]
    ...
    spoke_configs = [
        (8, 'chebi', 'ChEBI ID', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.identifier = $id AND c.source = \"ChEBI\" RETURN c"}), 
         None),
        (8, 'hmdb', 'HMDB ID', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.identifier = $id AND c.source = \"HMDB\" RETURN c"}), 
         None),
        (8, 'pubchem', 'PubChem ID', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.identifier = $id AND c.source = \"PubChem\" RETURN c"}), 
         None),
        (8, 'drugbank', 'DrugBank ID', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.identifier = $id AND c.source = \"DrugBank\" RETURN c"}), 
         None),
        (8, 'chembl', 'ChEMBL ID', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.identifier = $id AND c.source = \"ChEMBL\" RETURN c"}), 
         None),
        (8, 'inchikey', 'InChIKey', 'query', 
         json.dumps({"cypher": "MATCH (c:Compound) WHERE c.inchikey = $id RETURN c"}), 
         None)
    ]
