...
def test_patterns(kegg_id="C00031"):  # Default to glucose
    ...
    print(f"{'ID':<5} {'Property Name':<20} {'Method':<15} {'Current Pattern':<50} {'Result':<30}")
    print("-" * 120)
    ...
    for config in configs:
        ...
        print(f"{config['id']:<5} {property_name:<20} {method:<15} "
    ...
    if updated_patterns:
        ...
        for i, update in enumerate(updated_patterns):
            print(f"{i+1}. {update['property_name']}: {update['old_pattern']} -> {update['new_pattern']}")
        
        choice = input("\nDo you want to update these patterns? (y/n): ").strip().lower()
        if choice == 'y':
            for update in updated_patterns:
                rows_updated = update_property_config(update['id'], update['method'], update['new_pattern'])
                if rows_updated:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test KEGG property extraction patterns.")
    parser.add_argument("--kegg-id", default="C00031", help="KEGG compound ID to test against")
