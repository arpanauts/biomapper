#!/usr/bin/env python3
"""
Update property_extraction_configs table to support entity_types in the metamapper database.
This script adds an entity_type column to the property_extraction_configs table and ensures
...
try:
    # Check if property_extraction_configs table exists
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='property_extraction_configs'")
    if not cursor.fetchone():
        print("property_extraction_configs table does not exist in the database!")
        sys.exit(1)
    ...
    if "entity_type" not in columns:
        print("Adding entity_type column to property_extraction_configs table...")
        cursor.execute("ALTER TABLE property_extraction_configs ADD COLUMN entity_type TEXT DEFAULT 'metabolite'")
        print("Column added successfully")
    else:
        print("entity_type column already exists in property_extraction_configs table")
    
    # Get all existing property extraction configs
    cursor.execute("""
        SELECT pec.id, pec.resource_id, pec.property_name, r.name as resource_name, r.entity_type as resource_entity_type
        FROM property_extraction_configs pec
        JOIN resources r ON pec.resource_id = r.id
    """)
    ...
    for config in property_configs:
        ...
        if resource_entity_type == "all" and property_name in DEFAULT_PROPERTY_ENTITY_TYPES:
            property_entity_type = DEFAULT_PROPERTY_ENTITY_TYPES[property_name]
        
        # Update the property_config with the determined entity_type
        cursor.execute(
            "UPDATE property_extraction_configs SET entity_type = ? WHERE id = ?",
    ...
    # Verify the updates
    cursor.execute("""
        SELECT pec.id, pec.property_name, pec.entity_type as property_entity_type, 
               r.name as resource_name, r.entity_type as resource_entity_type
        FROM property_extraction_configs pec
        JOIN resources r ON pec.resource_id = r.id
        ORDER BY r.name, pec.property_name
    """)
    updated_configs = cursor.fetchall()
    
    print("\nProperty extraction configurations with entity types:")
    print("=================================================")
    ...
    for config in updated_configs:
        ...
        print(f"  - {config['property_name']} (Entity Type: {config['property_entity_type']})")
    ...
    if null_configs:
        print("\nWARNING: The following property extraction configs have NULL entity_type:")
        for config in null_configs:
            print(f"  - ID: {config['id']}, Property: {config['property_name']}, Resource ID: {config['resource_id']}")
        
        # Set default entity_type for any NULL values
        cursor.execute("UPDATE property_extraction_configs SET entity_type = 'metabolite' WHERE entity_type IS NULL")
        print(f"Updated {cursor.rowcount} property extraction configs with default entity_type = 'metabolite'")
    
    # Commit changes
    conn.commit()
    print(f"\nProperty extraction configuration entity types updated successfully ({update_count} configs updated)")
    
except Exception as e:
    print(f"Error updating property extraction configuration entity types: {e}")
    conn.rollback()
    sys.exit(1)
