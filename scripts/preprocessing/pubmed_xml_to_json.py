...
def xml_to_standardized_json(xml_file: str, output_file: str, max_articles: Optional[int] = None) -> int:
                ...
                for event, elem in context:
                    if elem.tag == "PubmedArticle":
                        try:
                            ...
                            if article_data and "id" in article_data and ("title" in article_data or "abstract" in article_data):
                                # Build primary text from important fields
                                primary_text_parts = []
                                
                                if "title" in article_data:
                                    primary_text_parts.append(f"Title: {article_data['title']}")
                                    
                                if "abstract" in article_data:
                                    primary_text_parts.append(f"Abstract: {article_data['abstract']}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Convert PubMed XML to standardized JSON format")
    ...
    parser.add_argument("--max-articles", type=int, help="Maximum number of articles to process per file")
    ...
    for xml_file in tqdm(xml_files[:args.max_files] if args.max_files else xml_files, desc="Processing XML files"):
    ...
    logging.info(f"Total processing complete: {total_articles} articles from {files_processed} files")
