...
def process_batch(
    ...
    for i in range(0, len(xml_files), batch_size):
        ...
        logger.info(
            f"Processing batch {i//batch_size + 1}/{(len(xml_files) + batch_size - 1)//batch_size} ({len(batch)} files)"
        )
        ...
        for xml_file in batch:
            ...
            output_file = os.path.join(
                output_dir, file_name.replace(".xml.gz", ".jsonl")
            )
...
def generate_embeddings(
    json_dir, output_index, output_metadata, embedder_cli_script, batch_size=5
):
    ...
    for i in range(0, len(json_files), batch_size):
        ...
        logger.info(
            f"Embedding batch {i//batch_size + 1}/{(len(json_files) + batch_size - 1)//batch_size} ({len(batch)} files)"
        )
        ...
        for json_file in batch:
            try:
                ...
                logger.error(
                    f"Unexpected error embedding {os.path.basename(json_file)}: {e}"
                )
                success = False
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of XML files to process in each batch",
    )
    parser.add_argument(
        "--max-articles",
        type=int,
        help="Maximum articles to extract per XML file. If not specified, process all articles",
    )
    parser.add_argument(
        "--xml-to-json-script",
        required=True,
        help="Path to the XML to JSON conversion script",
    )
    parser.add_argument(
        "--embedder-cli-script", required=True, help="Path to the embedder CLI script"
    )
    parser.add_argument(
        "--skip-embedding",
        action="store_true",
        help="Skip embedding generation and only process XML files",
    )
    parser.add_argument(
        "--embedding-batch-size",
        type=int,
        default=5,
        help="Number of JSON files to process in each embedding batch",
    )
    ...
    if args.skip_embedding:
        ...
        end_time = time.time()
        logger.info(
            f"XML to JSON conversion completed in {(end_time - start_time) / 60:.2f} minutes"
        )
        return
    ...
    if embedding_success:
        ...
        logger.warning(
            "Some errors occurred during embedding generation. Check the log for details."
        )

    end_time = time.time()
    logger.info(
        f"Full PubMed processing completed in {(end_time - start_time) / 60:.2f} minutes"
    )
