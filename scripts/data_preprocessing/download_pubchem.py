...
async def verify_existing_file(
    ...
    try:
        ...
        logging.info(
            f"MD5 mismatch for existing file {file.base_name}, will redownload"
        )
        return False
...
async def verify_all_existing_files(
    session: aiohttp.ClientSession, files: List[PubchemFile], output_dir: Path
) -> None:
    ...
    with tqdm(total=len(existing_files), desc="Verifying") as progress_bar:
        for file in existing_files:
            await verify_existing_file(
                session, file, output_dir / file.base_name, files
            )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        with tqdm(
            ...
            while remaining_files:
                ...
                tasks = [
                    asyncio.create_task(
                        download_file(session, file, DOWNLOAD_DIR, progress_bar)
                    )
                    for file in batch
                ]
                ...
                if remaining_files:
                    delay = calculate_retry_delay(
                        min(f.attempts for f in remaining_files)
                    )
