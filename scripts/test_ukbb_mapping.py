...
ARIVALE_METADOLOMICS_FILE = Path("/procedure/data/local_data/ARIVALE_SNAPSHOTS/metabolomics_metadata.tsv")
...
async def main():
    ...
    logging.getLogger('biomapper.mapping.relationships.executor').setLevel(logging.DEBUG)

    logging.info(f"Starting mapping test for relationship ID: {RELATIONSHIP_ID_TO_TEST}")
    logging.info(f"Source ontology: {SOURCE_ONTOLOGY}")
    logging.info(f"Target: UKBB Metabolites (via NAME)")
    logging.info(f"Processing max {MAX_ROWS_TO_TEST} rows from {ARIVALE_METADOLOMICS_FILE}")
    ...
    ConfigSessionLocal = async_sessionmaker(bind=config_engine, expire_on_commit=False, class_=AsyncSession)
        ...
        async with aiohttp.ClientSession() as http_session:
            ...
            async with ConfigSessionLocal() as config_db_session:
                ...
                with open(ARIVALE_METADOLOMICS_FILE, 'r', newline='') as csvfile:
                    ...
                    for _ in range(13): # Based on the file view, header is line 14 (0-indexed 13)
                        next(csvfile)
                    ...
                    if SOURCE_ONTOLOGY not in reader.fieldnames:
                        logging.error(f"Source ontology column '{SOURCE_ONTOLOGY}' not found in CSV headers.")
                        return

                    for row in reader:
                        if processed_rows >= MAX_ROWS_TO_TEST:
                            logging.info(f"Reached max rows to test ({MAX_ROWS_TO_TEST}). Stopping.")
                            break
                        ...
                        if source_entity_value and ';' not in source_entity_value: 
                            ...
                            try:
                                ...
                                if not mapping_results:
                                    logging.warning(f"   No mapping found for {SOURCE_ONTOLOGY} {source_entity_value}.")
                                else:
                                ...
                                logging.error(f"   Error mapping {source_entity_value}: {e}", exc_info=True)
                        
                            processed_rows += 1
                        elif source_entity_value and ';' in source_entity_value:
                            logging.warning(f"Skipping row {reader.line_num}: Multiple {SOURCE_ONTOLOGY} IDs found ('{source_entity_value}'). Simple MVP handles single IDs.")
