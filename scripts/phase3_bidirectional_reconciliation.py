...
def perform_bidirectional_validation(
    ...
    for idx, row in reconciled_df.iterrows():
        ...
        if ukbb_id in ukbb_to_arivale_index:
            ...
            if all_forward_arivale_ids:
                ...
                # Set the one_to_many_source_col flag if there are multiple unique target IDs
                # FIX: A source mapping to multiple targets indicates one_to_many_source
                reconciled_df.at[idx, one_to_many_source_col] = len(unique_sorted_arivale_ids) > 1
        ...
        if arivale_id in arivale_to_ukbb_index:
            ...
            if all_reverse_ukbb_ids:
                ...
                # Set the one_to_many_target_col flag if there are multiple unique source IDs
                # FIX: Multiple sources mapping to this target indicates one_to_many_target
                reconciled_df.at[idx, one_to_many_target_col] = len(unique_sorted_ukbb_ids) > 1
        ...
        for arivale_id in arivale_ids_with_only_reverse:
            ...
            for reverse_mapping in reverse_mappings:
                ...
                if all_reverse_ukbb_ids:
                    ...
                    # Set the one_to_many_target_col flag if there are multiple unique source IDs
                    # FIX: Multiple sources mapping to this target indicates one_to_many_target
                    new_row[one_to_many_target_col] = len(unique_sorted_ukbb_ids) > 1
    ...
    # For one_to_many_source_col (sources that map to multiple targets)
    # FIX: This checks if a SOURCE has multiple TARGETS, so it should set one_to_many_source_col
    for idx, row in reconciled_df.iterrows():
    ...
    # For one_to_many_target_col (targets that are mapped by multiple sources)
    # FIX: This checks if a TARGET has multiple SOURCES, so it should set one_to_many_target_col
    for idx, row in reconciled_df.iterrows():
        # Only update if not already set
