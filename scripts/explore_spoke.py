...
def connect_to_spoke(host="localhost", port=8529, username="root", password="ph", database="spoke23_human"):
...
def get_database_structure(db):
    ...
    structure["collection_summary"] = {
        ...
        "document_collections": len([c for c in collections if c["type"] == 2 and not c["name"].startswith("_")]),
        "edge_collections": len([c for c in collections if c["type"] == 3 and not c["name"].startswith("_")]),
        "system_collections": len([c for c in collections if c["name"].startswith("_")]),
    }
...
def analyze_spoke_structure(structure):
    ...
    analysis = {
        "overview": {
            ...
            "document_collections": structure["collection_summary"]["document_collections"],
    ...
    for name, collection in structure["collections"].items():
        if collection["type"] == "document":
            ...
            id_fields = [f for f in fields if f in ("id", "identifier", "uuid", "name", "identifier")]
        ...
        # Analyze edge collections
            ...
            fields = [f for f in collection["schema"]["fields"].keys() if f not in ("_from", "_to")]
...
def extract_id_patterns(samples):
    ...
    for sample in samples:
        for field in id_fields:
            if field in sample and isinstance(sample[field], str):
                ...
                if len(patterns[field]["examples"]) < 5 and id_value not in patterns[field]["examples"]:
    ...
    for field, pattern in patterns.items():
        ...
        if total > 0:
            pattern["has_colon_percent"] = round(pattern["contains_colon"] / total * 100, 1)
            pattern["has_dash_percent"] = round(pattern["contains_dash"] / total * 100, 1)
...
def analyze_edge_connections(samples):
    ...
    # Convert counters to dictionaries
    connections["from_collections"] = dict(connections["from_collections"].most_common())
...
def generate_documentation(structure, analysis):
    ...
    for name, info in sorted(analysis["node_types"].items(), key=lambda x: x[1]["count"], reverse=True):
    ...
    for name, info in sorted(analysis["edge_types"].items(), key=lambda x: x[1]["count"], reverse=True):
        # Get sample properties (top 5)
        sample_props = ", ".join(info["fields"][:5]) if len(info["fields"]) > 0 else "None"
...
def main():
    ...
    for collection in structure_simplified["collections"].values():
        collection["sample_documents"] = f"{len(collection['sample_documents'])} samples (removed to reduce file size)"
