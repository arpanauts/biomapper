...
async def test_relationship_path_discovery():
    ...
    if relationship:
        ...
        if "MetabolitesCSV" in endpoints and "SPOKE" in endpoints:
            ...
            """
            result = await session.execute(text(query), {"source_id": source_id, "target_id": target_id})
    ...
    paths = await pathfinder.discover_relationship_paths(relationship_id, force_rediscover=True)
    
    if not paths:
        logger.warning(f"No mapping paths discovered for relationship {relationship_id}")
    else:
...
async def test_relationship_mapping_execution():
    ...
    logger.info(f"Found relationship ID: {relationship_id}")
    logger.info(f"Source endpoint: {relationship.source_name} (ID: {source_endpoint_id})")
    logger.info(f"Target endpoint: {relationship.target_name} (ID: {target_endpoint_id})")
    ...
    if not results:
        ...
        for result in results:
            logger.info(f"  {result['source_id']} ({result['source_type']}) -> {result['target_id']} ({result['target_type']}) [confidence: {result['confidence']}]")
...
async def check_cache_entries():
    ...
    for entry in entries:
        ...
        logger.info(f"  {entry.source_id} ({entry.source_type}) -> {entry.target_id} ({entry.target_type}) [confidence: {entry.confidence}, relationship: {rel_id}]")
...
async def main():
    ...
    try:
        ...
        if "endpoint_relationships" in tables:
            ...
            logger.error("The endpoint_relationships table does not exist. Running db setup first")
            # Run the table creation script
            import subprocess
            setup_path = os.path.join(os.path.dirname(__file__), '../db_management/create_endpoint_mapping_tables.py')
