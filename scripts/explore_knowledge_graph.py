...
async def explore_knowledge_graph(analyzer: KnowledgeGraphAnalyzer, output_file: Optional[str] = None) -> None:
    ...
    if output_file:
        ...
        # Print summary
        print(f"Generated mapping with {len(schema_mapping.node_type_mappings)} field mappings")
        print("\nSample mappings:")
        for mapping in schema_mapping.node_type_mappings[:5]:  # Show first 5
            print(f"- {mapping.node_type}.{mapping.property_path} -> {mapping.ontology_type}")
...
async def main():
    ...
    parser.add_argument("--output", help="Output file for schema mapping (YAML or JSON)")
