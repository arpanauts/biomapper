...
async def analyze_spoke_and_register(
        ...
        if metadata.source_node_types and metadata.target_node_types:
            for source_type in metadata.source_node_types:
                for target_type in metadata.target_node_types:
                    ...
                    if source_type == target_type and source_type != "Compound" and source_type != "Gene":
                        continue
    ...
    print(f"Generated {len(schema_mapping.node_type_mappings)} ontology field mappings.")
        ...
        schema_mapping={
            "node_types": {
                m.node_type: {
                    ...
                    "property_map": {
                        m.property_path: m.ontology_type
                    }
                }
            ...
            for example_id in example_ids:
                ...
                if nodes:
                    ...
                    if node_id:
                        ...
                        if "compound_to_gene" in [c.name for c in resource_registration.capabilities]:
                            ...
                            if genes:
                                ...
                                if verbose:
                                    for i, gene in enumerate(genes[:5]):  # Show up to 5 genes
                        ...
                        if "compound_to_disease" in [c.name for c in resource_registration.capabilities]:
                            ...
                            if diseases:
                                ...
                                if verbose:
                                    for i, disease in enumerate(diseases[:5]):  # Show up to 5 diseases
...
async def main():
    ...
    parser.add_argument("--sample-size", type=int, default=5, help="Number of samples to retrieve")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--output", help="Output file for resource registration (YAML or JSON)")
