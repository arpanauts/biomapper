...
async def verify_setup(searcher: MetaboliteSearcher, test_compounds: list):
    ...
    for compound in test_compounds:
        try:
            ...
            if results:
                logger.info(f"✓ Found {len(results)} matches for '{compound}' (top score: {results[0]['score']:.3f})")
                logger.info(f"  Best match: {results[0]['name']} ({results[0]['hmdb_id']})")
            else:
...
async def main():
    ...
    logger.info(f"Starting HMDB metabolite loading process")
    ...
    try:
        ...
        if stats['total_processed'] > 0:
            logger.info(f"Processing rate: {stats['total_processed'] / elapsed_time:.1f} metabolites/second")
            success_rate = (stats['total_processed'] / (stats['total_processed'] + stats['total_errors'])) * 100
        ...
        if not args.skip_verification and stats['total_processed'] > 0:
            ...
            # Summary
            successful_searches = sum(1 for r in verification_results.values() if r.get('found', False))
            ...
            if successful_searches == len(test_compounds):
                ...
                logger.warning("⚠ Some test compounds not found - setup may need tuning")
            else:
