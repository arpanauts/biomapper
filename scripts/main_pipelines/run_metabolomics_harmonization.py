...
Examples:
  ...
  %(prog)s --parameters custom.json  # Custom parameters
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Validate strategy without executing'
    )
    parser.add_argument(
        '--debug',
        action='store_true',
            ...
            if result.get('success', False):
                ...
                if 'step_results' in result:
                    ...
                    for step in result['step_results']:
                        if step.get('status') == 'success':
                            ...
                            logger.info(f"  ✅ {step['step_id']}: {input_count} → {output_count}")
                        else:
                            error = step.get('details', {}).get('error', 'Unknown error')
                ...
                if 'summary' in result:
                    ...
                    if 'total_metabolites_processed' in summary:
                        logger.info(f"🧪 Total metabolites processed: {summary['total_metabolites_processed']}")
                    if 'final_match_rate' in summary:
                        logger.info(f"🎯 Final match rate: {summary['final_match_rate']:.1%}")
                    if 'improvement_over_baseline' in summary:
                        logger.info(f"📈 Improvement over baseline: +{summary['improvement_over_baseline']:.1%}")
                ...
                if 'step_results' in result:
                    successful_steps = [s for s in result['step_results'] if s.get('status') == 'success']
                    if successful_steps:
                        logger.info(f"✅ {len(successful_steps)} steps completed successfully before failure")
