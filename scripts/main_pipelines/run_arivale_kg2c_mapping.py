#!/usr/bin/env python
"""
Execute Arivale to KG2C protein mapping using the MVP action pipeline.
...
Results are saved to results/Arivale_KG2C/ directory.
"""
import asyncio
import logging
import sys
...
async def main():
    """Execute the Arivale to KG2C protein mapping pipeline."""

    logger.info("Starting Arivale to KG2C protein mapping...")
    ...
    try:
        ...
        async with BiomapperClient(base_url="http://localhost:8000") as client:
            ...
            result = await client.execute_strategy(
                strategy_name="ARIVALE_KG2C_PROTEIN_MAPPING", context=context
            )

        # Log success
        logger.info("✅ Arivale to KG2C protein mapping completed successfully!")
        ...
        if "summary" in result and "step_results" in result["summary"]:
            ...
            for step in analysis_steps:
                if step.get("status") == "success":
                    ...
                    logger.info("📁 Results saved to: results/Arivale_KG2C/")
