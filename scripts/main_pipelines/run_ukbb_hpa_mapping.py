#!/usr/bin/env python
"""
Execute UKBB to HPA protein mapping using the MVP action pipeline.

This script demonstrates the complete workflow:
1. Load UKBB protein data
2. Load HPA protein data  
3. Merge with UniProt historical resolution
4. Calculate set overlap and generate visualizations

Results are saved to results/UKBB_HPA/ directory.
"""
import asyncio
import logging
import sys
...
async def main():
    """Execute the UKBB to HPA protein mapping pipeline."""

    logger.info("Starting UKBB to HPA protein mapping...")
    ...
    try:
        ...
        async with BiomapperClient(base_url="http://localhost:8000") as client:
            ...
            result = await client.execute_strategy(
                strategy_name="UKBB_HPA_PROTEIN_MAPPING", context=context
            )

        # Log success
        logger.info("✅ UKBB to HPA protein mapping completed successfully!")
        ...
        if "summary" in result and "step_results" in result["summary"]:
            ...
            for step in analysis_steps:
                if step.get("status") == "success":
                    ...
                    logger.info("📁 Results saved to: results/UKBB_HPA/")
