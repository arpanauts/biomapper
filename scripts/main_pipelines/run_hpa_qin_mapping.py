#!/usr/bin/env python
"""
Execute HPA to QIN protein mapping using the MVP action pipeline.
...
Results are saved to results/HPA_QIN/ directory.
"""
import asyncio
import logging
import sys
...
async def main():
    """Execute the HPA to QIN protein mapping pipeline."""

    logger.info("Starting HPA to QIN protein mapping...")
    ...
    try:
        ...
        async with BiomapperClient(base_url="http://localhost:8000") as client:
            ...
            result = await client.execute_strategy(
                strategy_name="HPA_TO_QIN_PROTEIN_MAPPING", context=context
            )

        # Log success
        logger.info("✅ HPA to QIN protein mapping completed successfully!")
        ...
        if "summary" in result and "step_results" in result["summary"]:
            ...
            for step in analysis_steps:
                if step.get("status") == "success":
                    ...
                    logger.info("📁 Results saved to: results/HPA_QIN/")
