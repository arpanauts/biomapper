#!/usr/bin/env python
"""
Execute HPA to SPOKE protein mapping using the MVP action pipeline.

This script demonstrates the complete workflow:
1. Load HPA protein data
2. Load SPOKE protein data  
3. Merge with UniProt historical resolution
4. Calculate set overlap and generate visualizations

Results are saved to results/HPA_SPOKE/ directory.
"""
import asyncio
import logging
import sys
...
async def main():
    """Execute the HPA to SPOKE protein mapping pipeline."""

    logger.info("Starting HPA to SPOKE protein mapping...")
    ...
    try:
        ...
        async with BiomapperClient(base_url="http://localhost:8000") as client:
            ...
            result = await client.execute_strategy(
                strategy_name="HPA_TO_SPOKE_PROTEIN_MAPPING", context=context
            )

        # Log success
        logger.info("✅ HPA to SPOKE protein mapping completed successfully!")
        ...
        if "summary" in result and "step_results" in result["summary"]:
            ...
            for step in analysis_steps:
                if step.get("status") == "success":
                    ...
                    logger.info("📁 Results saved to: results/HPA_SPOKE/")
