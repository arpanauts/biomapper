...
def get_temp_batch_status(temp_dir):
    ...
    for batch_dir in glob.glob(os.path.join(temp_dir, "batch_*")):
        ...
        batches[batch_name] = {
            ...
            "input_files_size": sum(os.path.getsize(f) for f in input_files) if input_files else 0,
            "output_files_size": sum(os.path.getsize(f) for f in output_files) if output_files else 0,
        }
...
def main():
    parser = argparse.ArgumentParser(description="Monitor the progress of PubMed data processing.")
    parser.add_argument("--data-dir", required=True, help="Base directory for PubMed data processing")
    ...
    while True:
        ...
        print("=" * 70)
        print(f"PUBMED DATA PROCESSING MONITOR - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 70)
        
        # JSON conversion progress
        json_count = get_json_file_count(json_dir)
        json_size = sum(os.path.getsize(f) for f in glob.glob(os.path.join(json_dir, "*.jsonl")))
        ...
        if batches:
            ...
            for batch_name, status in batches.items():
                print(f"- {batch_name}:")
                print(f"  - Input files: {status['input_files']} ({format_size(status['input_files_size'])})")
                print(f"  - Output files: {status['output_files']} ({format_size(status['output_files_size'])})")
        else:
        ...
        try:
            disk_info = subprocess.check_output("df -h | grep -E '/$|/home'", shell=True).decode('utf-8')
            print(disk_info)
        except:
