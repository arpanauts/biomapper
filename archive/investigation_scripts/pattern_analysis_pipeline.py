...
class PatternAnalyzer:
    ...
    def run_complete_analysis(self) -> Dict[str, Any]:
        ...
        pattern_candidates = self.identify_pattern_candidates(
            results["duplication_analysis"]
        )
    ...
    def analyze_validation_patterns(self) -> Dict[str, Any]:
        ...
        for entity_path in self.entity_paths:
            ...
            for action_file in Path(entity_path).rglob("*.py"):
                ...
                try:
                    ...
                    if (
                        "input_key not in context" in content
                        or "datasets" not in content
                    ):
                        validation_patterns["input_key_validation"].append(
                            {
                                "file": str(action_file),
                                "lines": self.extract_validation_block(
                                    content, "input_key"
                                ),
                                "exact_match": True,
                            }
                        ...
                        validation_patterns["dataset_existence_check"].append(
                            {
                                ...
                                "exact_match": self.calculate_similarity_score(
                                    content, "datasets"
                                )
                                > 0.9,
                            }
                    ...
                    if "column" in content.lower() and (
                        "not found" in content.lower() or "missing" in content.lower()
                    ):
        ...
        return {
            ...
            "summary": {
                "total_patterns": sum(
                    len(patterns) for patterns in validation_patterns.values()
                ),
                "exact_matches": sum(
                    1
                    for patterns in validation_patterns.values()
                    for p in patterns
                    if p["exact_match"]
                ),
                "cross_entity_usage": self.count_cross_entity_usage(
                    validation_patterns
                ),
            },
        }
                ...
                try:
                    ...
                    if "ContextError" in content or "context" in content.lower():
                        error_patterns["context_error_raising"].append(
                            {
                                "file": str(action_file),
                                "pattern": self.extract_error_pattern(
                    ...
                    if (
                        "DatasetNotFoundError" in content
                        or "dataset.*not found" in content.lower()
                    ):
                        error_patterns["dataset_not_found_error"].append(
                            {
                                "file": str(action_file),
                                "pattern": self.extract_error_pattern(
                                    content, "DatasetNotFound"
                                ),
                            }
                        )
    ...
    def find_cross_entity_patterns(self, duplication_results: Dict) -> List[Dict]:
        ...
        validation_patterns = duplication_results.get("validation_patterns", {}).get(
            "patterns", {}
        )
                ...
                cross_patterns.append(
                    {
                        ...
                        "lines_duplicated": sum(
                            len(inst.get("lines", [])) for inst in instances
                        ),
                        "abstraction_benefit": self.calculate_abstraction_benefit(
                            instances
                        ),
                    }
                )
                ...
                cross_patterns.append(
                    {
                        ...
                        "locations": [inst["file"] for inst in instances],
                        "lines_duplicated": sum(
                            inst.get("lines_count", 0) for inst in instances
                        ),
                        "abstraction_benefit": self.calculate_abstraction_benefit(
                            instances
                        ),
    ...
    def identify_pattern_candidates(
        self, duplication_analysis: Dict
    ...
    def evaluate_pattern(self, pattern: PatternCandidate) -> PatternEvaluation:
        ...
        lines_saved = (
            pattern.total_duplicated_lines - pattern.abstraction_overhead_lines
        )
        ...
        total_score = sum(
            scores[criterion] * self.weights[criterion] for criterion in scores
        )

        recommendation = (
            "abstract" if total_score >= self.abstraction_threshold else "keep_separate"
        )
    ...
    def document_entity_specific_characteristics(self) -> Dict[str, Any]:
        ...
        separation_scores = {}
        complexity_scores = [
            char["id_systems"]["complexity_score"] for char in characteristics.values()
        ]
        ...
        for entity, char in characteristics.items():
            ...
            complexity_diff = abs(
                char["id_systems"]["complexity_score"] - avg_complexity
            )
            ...
            if (
                "fuzzy"
                in char["processing_characteristics"]["matching_strategy"].lower()
            ):
        ...
        return {
            ...
            "recommendation": "MAINTAIN_ENTITY_SEPARATION"
            if min(separation_scores.values()) > 25
            else "CONSIDER_ABSTRACTION",
        }
    ...
    def analyze_complexity(self) -> Dict[str, Any]:
        ...
        for entity_path in self.entity_paths:
            ...
            if complexities:
                complexity_results[entity_name]["average_complexity"] = np.mean(
                    complexities
                )
    ...
    def generate_recommendations(
        self, pattern_evaluations: List[Dict]
    ) -> Dict[str, Any]:
                ...
                recommended_abstractions.append(
                    {
                        ...
                        if evaluation["evaluation"]["total_score"] > 40
                        else "MEDIUM",
                    }
                )
            else:
        ...
        return {
            ...
            "summary": {
                ...
                "abstraction_percentage": len(recommended_abstractions)
                / len(pattern_evaluations)
                * 100
    ...
    def extract_statistics_pattern(self, content: str) -> Optional[str]:
        ...
        for line in lines:
            if "statistics" in line.lower() and (
                "=" in line or "update" in line.lower()
            ):
                in_stats_block = True
    ...
    def appears_in_multiple_entities(
        self, instances: List[Dict], min_entities: int = 2
    ) -> bool:
    ...
    def assess_divergence_likelihood(self, pattern: Dict) -> bool:
        ...
        return pattern["pattern_type"] in [
            "validation_input_key_validation",
            "context_statistics_updates",
        ]
    ...
    def assess_testability(self, pattern: Dict) -> bool:
        ...
        return (
            "validation" in pattern["pattern_type"]
            or "context" in pattern["pattern_type"]
        )
    ...
    def assess_entity_specificity(self, pattern: Dict) -> bool:
        ...
        return (
            "matching" in pattern["pattern_type"]
            or "identification" in pattern["pattern_type"]
        )
    ...
    def calculate_confidence(
        self, scores: Dict[str, float], pattern: PatternCandidate
    ) -> float:
        ...
        confidence = (
            sum(high_confidence_factors) - sum(low_confidence_factors)
        ) / 8 + 0.5
...
def main():
    ...
    print(
        f"📈 Lines analyzed: {results['analysis_metadata']['total_lines_analyzed']:,}"
    )
    print(
        f"🔍 Patterns identified: {results['analysis_metadata']['patterns_identified']}"
    )
