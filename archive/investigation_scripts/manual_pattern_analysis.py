...
def analyze_dataset_validation_patterns():
    ...
    for file_path in action_files:
        ...
        try:
            ...
            result_storage = re.findall(r'context\["datasets"\]\[.*\] = .*', content)
            if result_storage:
        ...
        try:
            ...
            if 'if "statistics" not in context' in content:
                patterns["statistics_initialization"].append(
                    {"file": str(file_path), "entity": get_entity_from_path(file_path)}
                )

            # Statistics updates - find all context["statistics"] assignments
            stats_updates = re.findall(r'context\["statistics"\].*=.*', content)
            if stats_updates:
    ...
    for file_path in action_files:
        ...
        try:
            ...
            if "input_key: str = Field" in content:
                input_key_match = re.search(r"input_key: str = Field\([^)]+\)", content)
                if input_key_match:
            ...
            # Column parameters
            column_params = re.findall(r"\w*column\w*: str = Field\([^)]+\)", content)
            if column_params:
...
def analyze_error_handling_patterns():
    ...
    for file_path in action_files:
        ...
        try:
            ...
            # KeyError patterns
            key_errors = re.findall(r"raise KeyError\([^)]+\)", content)
            if key_errors:
            ...
            beneficial.append(
                {
                    ...
                    "recommendation": "HIGH"
                    if len(input_checks) > 4 and len(entities) >= 3
                    else "MEDIUM",
                }
            )
        ...
        for i in range(len(input_key_params)):
            for j in range(i + 1, len(input_key_params)):
                sim = calculate_pattern_similarity(
                    input_key_params[i]["pattern"], input_key_params[j]["pattern"]
                )
            ...
            beneficial.append(
                {
                    ...
                    "cross_entity_usage": len(
                        set(p["entity"] for p in input_key_params)
                    ),
...
def identify_entity_specific_patterns(all_patterns: Dict) -> Dict[str, List[Dict]]:
    ...
    entity_specific = {"proteins": [], "metabolites": [], "chemistry": []}
            ...
            if isinstance(instances, list) and instances:
                ...
                for entity, count in entity_usage.items():
                    if count >= 3 and entity != "unknown":
                        ...
                        other_usage = sum(
                            c for e, c in entity_usage.items() if e != entity
                        )
...
def main():
    ...
    print(
        f"  Dataset validation patterns: {sum(len(p) for p in dataset_patterns.values())}"
    )
    ...
    for abstraction in beneficial_abstractions:
        print(
            f"  - {abstraction['pattern_name']}: {abstraction['usage_count']} usages across {abstraction['cross_entity_usage']} entities"
        )
        print(
            f"    Recommendation: {abstraction['recommendation']}, Score: {abstraction['benefit_score']}"
        )
    ...
    for entity, patterns in entity_specific.items():
        if patterns:
            ...
            for pattern in patterns[:3]:  # Show first 3
                print(
                    f"    - {pattern['pattern_name']}: {pattern['usage_in_entity']}x in {entity}, {pattern['usage_in_others']}x elsewhere"
                )
    ...
    results = {
        ...
        "summary": {
            "total_patterns_found": sum(
                sum(len(p) for p in cat.values()) for cat in all_patterns.values()
            ),
            "beneficial_abstractions_count": len(beneficial_abstractions),
            "high_priority_abstractions": len(
                [a for a in beneficial_abstractions if a["recommendation"] == "HIGH"]
            ),
            "entity_specific_count": sum(
                len(patterns) for patterns in entity_specific.values()
            ),
        },
    }
    ...
    print(
        f"🎯 {results['summary']['beneficial_abstractions_count']} beneficial abstractions identified"
    )
