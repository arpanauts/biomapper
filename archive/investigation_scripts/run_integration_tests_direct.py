...
class DirectIntegrationTester:
    ...
    def get_available_strategies(self) -> Dict[str, List[str]]:
        ...
        for strategy_name in self.service.strategies.keys():
            ...
            if "protein" in strategy_lower or "prot" in strategy_lower:
                ...
                for term in ["metabol", "nightingale", "nmr", "cts"]
            ):
    ...
    async def test_strategy(
        self, strategy_name: str, entity_type: str, dataset_size: int = 1000
    ) -> StrategyTestResult:
        ...
        try:
            ...
            parameters = {**test_files, "dataset_size": dataset_size, "test_mode": True}
            ...
            print(
                f"    ✓ Success in {execution_time:.1f}s, {len(output_keys)} outputs, {total_rows} total rows"
            )
            return test_result
    ...
    async def run_comprehensive_tests(self):
        ...
        print(
            f"Loaded {len(self.service.strategies)} strategies from {self.strategies_dir}"
        )
    ...
    def generate_report(self):
        ...
        for entity, stats in by_entity.items():
            ...
            print(
                f"  {entity.title()}: {stats['passed']}/{total} passed ({success_rate:.1f}%), avg {avg_time:.1f}s"
            )
        ...
        if passed_tests > 0:
            ...
            for size, times in sorted(by_size.items()):
                ...
                print(
                    f"  {size} rows: avg={avg_time:.1f}s, p95={p95_time:.1f}s, throughput={throughput:.1f} rows/s"
                )
