...
class RAGMapper(BaseMapper[T], ABC):
    ...
    def __init__(
        self, vector_store: Any, llm_client: Any, confidence_threshold: float = 0.8
    ):
    ...
    async def map_entity(
        ...
        similar_docs = await self.vector_store.get_similar(
            text, threshold=self.confidence_threshold
        )
    ...
    async def _generate_mapping(
        self, text: str, similar_docs: List[T], context: Optional[Dict[str, Any]]
    ) -> MappingResult:
