...
class LLMConfig(BaseModel):
    ...
    temperature: float = Field(
        0.7, ge=0.0, le=2.0, description="Temperature for LLM sampling"
    )
    max_tokens: int = Field(1000, gt=0, description="Maximum tokens in LLM response")
...
class AnalysisResult(BaseModel):
    ...
    entities: List[SPOKEEntity] = Field(
        default_factory=list, description="Analyzed SPOKE entities"
    )
    ...
    confidence_scores: Dict[str, float] = Field(
        default_factory=dict, description="Confidence scores for insights"
    )
    metadata: Dict[str, Any] = Field(
