...
class CTSClient(BaseMappingClient):
    ...
    async def _load_valid_id_types(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self._valid_from_ids)} source types and {len(self._valid_to_ids)} target types"
            )
    ...
    def _validate_id_types(self, from_type: str, to_type: str) -> None:
        ...
        if self._valid_from_ids and from_type_mapped not in self._valid_from_ids:
            raise ValueError(
                f"Invalid source ID type: {from_type}. Valid types: {sorted(self._valid_from_ids)}"
            )
        if self._valid_to_ids and to_type_mapped not in self._valid_to_ids:
            raise ValueError(
                f"Invalid target ID type: {to_type}. Valid types: {sorted(self._valid_to_ids)}"
            )
    ...
    async def convert(
        self, identifier: str, from_type: str, to_type: str, use_cache: bool = True
    ) -> Optional[List[str]]:
        ...
        cache_key = self._get_cache_key(
            "convert", from_type=from_type_api, to_type=to_type_api, id=identifier
        )
        if use_cache:
        ...
        # Make request
        endpoint = (
            f"convert/{quote(from_type_api)}/{quote(to_type_api)}/{quote(identifier)}"
        )
        ...
        for identifier in identifiers:
            ...
            for to_type in to_types:
                ...
                converted = await self.convert(
                    identifier, from_type, to_type, use_cache
                )
    ...
    async def score_inchikeys(
        ...
        """
        cache_key = self._get_cache_key(
            "score", from_type=from_type, id=identifier, algo=algorithm.value
        )
        cached = self._get_from_cache(cache_key)
        if cached is not None:
            return cached  # type: ignore
    ...
    async def enrich_metabolite(
        self, identifier: str, id_type: str, target_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        for target_type in target_types:
            if target_type != id_type:
                if result_idx < len(results) and not isinstance(
                    results[result_idx], Exception
                ):
    ...
    async def map_identifiers(
        ...
        if not config:
            raise ValueError(
                "CTS client requires config with 'from_type' and 'to_type'"
            )
