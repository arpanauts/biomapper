...
class UniChemClient(CachedMappingClientMixin, BaseMappingClient):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Initialized UniChemClient with source_db={self.source_db}, target_db={self.target_db}"
        )
    ...
    async def _get_compound_mappings(
    ...
    async def map_identifiers(
        ...
        logger.info(
            f"Mapping {len(identifiers)} identifiers from {source_db} to {target_db}"
        )
        ...
        if identifiers_to_lookup:
            ...
            for i in range(0, len(identifiers_to_lookup), chunk_size):
                ...
                tasks = [
                    self._get_compound_mappings(identifier, source_db)
                ...
                for j, identifier in enumerate(chunk):
                    ...
                    if isinstance(mapping_result, Exception):
                        logger.error(
                            f"Error mapping {identifier}: {str(mapping_result)}"
                        )
                    ...
                    target_ids = (
                        mapping_result.get(target_db, []) if mapping_result else []
                    )

                    if target_ids:
                        logger.debug(
                            f"Mapped {identifier} to {len(target_ids)} {target_db} IDs: {target_ids}"
                        )
    ...
    async def reverse_map_identifiers(
        ...
        logger.info(
            f"Reverse mapping {len(identifiers)} identifiers from {source_db} to {target_db}"
        )
