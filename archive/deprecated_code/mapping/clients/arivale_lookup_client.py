...
class ArivaleMetadataLookupClient(
        ...
        try:
            ...
            if key_column not in df.columns or value_column not in df.columns:
                ...
                raise ClientInitializationError(
                    ...
                    details={
                        "file_path": file_path,
                        "missing_columns": [key_column, value_column],
                    },
                )
    ...
    async def map_identifiers(
        ...
        for identifier in identifiers:
            ...
            if direct_match_found:
                result_tuple = self.format_result(
                    [self._lookup_map[identifier_stripped]], identifier_stripped
                )
                ...
                components = [
                    comp.strip()
                    for comp in identifier_stripped.replace("_", ",").split(",")
                    if comp.strip()
                ]
        ...
        for original_id, result_tuple_val in client_format_results.items():
            (
                mapped_ids_list,
                _,
            ) = result_tuple_val  # We don't need successful_component for the final structure here
            if mapped_ids_list and len(mapped_ids_list) > 0:
                ...
                final_primary_ids_set.add(
                    primary_mapped_id
                )  # Add to set of all unique mapped IDs
                final_input_to_primary[
                    original_id
                ] = primary_mapped_id  # Map input to its primary
            else:
                final_errors.append(
                    {
    ...
    async def reverse_map_identifiers(
        ...
        for identifier in identifiers:
            identifier_processed = identifier.strip().strip(
                '"'
            )  # Apply .strip('"') here
        ...
        for original_arivale_id, result_tuple_val in client_format_results.items():
            (
                mapped_source_ids_list,
                _,
            ) = result_tuple_val  # We don't need successful_component for reverse map
