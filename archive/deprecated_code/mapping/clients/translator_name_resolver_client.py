...
class TranslatorNameResolverClient(CachedMappingClientMixin, BaseMappingClient):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Initialized TranslatorNameResolverClient with target_db={self.target_db}"
        )
    ...
    async def _perform_request(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning(
                    f"HTTP error during Name Resolution API request: {str(e)}"
                )
                if attempt < max_retries:
                    delay = backoff_factor * (2**attempt)
                    logger.info(
    ...
    def _filter_matches_by_target_db(
    ...
    async def _lookup_entity_name(
        self, name: str, target_biolink_type: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            matches = await self._perform_request(
                self._config["base_url"], params=params
            )
    ...
    async def map_identifiers(
        self,
        names: List[str],
        target_biolink_type: str = "biolink:SmallMolecule",
        **kwargs,
        ...
        match_threshold = float(
            call_config.get("match_threshold", self.match_threshold)
        )

        if not target_db:
            logger.warning(
                "No target database specified, results may not be filtered properly"
            )
        ...
        if names_to_lookup:
            ...
            for i in range(0, len(names_to_lookup), chunk_size):
                ...
                tasks = [
                    self._lookup_entity_name(name, target_biolink_type)
                    for name in chunk
                ]
                ...
                for j, name in enumerate(chunk):
                    ...
                    if target_ids:
                        logger.debug(
                            f"Mapped {name} to {len(target_ids)} identifiers: {target_ids}"
                        )
