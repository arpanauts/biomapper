...
from typing import Dict, Any, List, TypeVar, Generic, Type, cast
...
class TypedStrategyAction(Generic[TParams, TResult], ABC):  # Simplified for validation
    ...
    async def execute(
        ...
        if is_mvp_action:
            ...
            # Check if context is already a StrategyExecutionContext
            if isinstance(context, StrategyExecutionContext):
                typed_context = context
            else:
                # Convert dict context to StrategyExecutionContext for other actions
                typed_context = self._convert_context(
                    context, current_identifiers, current_ontology_type
                )
        ...
        try:
            ...
            result_dict = self._convert_result_to_dict(
                typed_result, current_identifiers, current_ontology_type
            )

            # Update the original context dict with any changes from typed context
            if not is_mvp_action and not isinstance(context, StrategyExecutionContext):
    ...
    def _convert_context(
        self, context_dict: Dict[str, Any], identifiers: List[str], ontology_type: str
    ) -> Any:
        """
        ...
        initial_identifier = context_dict.get(
            "initial_identifier", identifiers[0] if identifiers else ""
        )
        current_identifier = context_dict.get(
            "current_identifier", identifiers[0] if identifiers else ""
        )
        ...
        if isinstance(raw_step_results, list):
            ...
            step_results_dict = {}
        elif isinstance(raw_step_results, dict):
            step_results_dict = raw_step_results
        else:
            # Handle invalid types gracefully by defaulting to empty dict
            step_results_dict = {}

        # Clean up provenance data to match ProvenanceRecord schema
        raw_provenance = context_dict.get("provenance", [])
        cleaned_provenance = []
        ...
        for prov_item in raw_provenance:
            if isinstance(prov_item, dict):
                ...
                cleaned_prov = ProvenanceRecord(
                    source=prov_item.get("source", prov_item.get("action", "unknown")),
                    timestamp=prov_item.get("timestamp", datetime.now()),
                    action=prov_item.get("action", "unknown"),
                    details=prov_item.get("details", prov_item),
                )
                cleaned_provenance.append(cleaned_prov)
            # Skip None or non-dict provenance items gracefully
    ...
    def _convert_result_to_dict(
        self, typed_result: TResult, input_identifiers: List[str], ontology_type: str
    ) -> Dict[str, Any]:
        """
        ...
        standard_result = {
            "input_identifiers": result_dict.get(
                "input_identifiers", input_identifiers
            ),
            "output_identifiers": result_dict.get("output_identifiers", []),
            "output_ontology_type": result_dict.get(
    ...
    def _update_context_dict(
        self, context_dict: Dict[str, Any], typed_context: StrategyExecutionContext
    ) -> None:
        """
