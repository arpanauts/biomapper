...
class TrackProgressiveStats(TypedStrategyAction[TrackProgressiveStatsParams, ActionResult]):
    ...
    async def execute_typed(
        ...
        try:
            ...
            if (params.stage_id == 0 or params.is_baseline_stage) and params.track_unique_entities:
                if params.entity_id_column in stage_df.columns:
                    ...
                    if params.is_baseline_stage:
                        logger.info(f"📊 Baseline Statistics Captured: {unique_entities} unique entities from {total_rows} total rows")
                    else:
                        logger.info(f"📊 Input Statistics Captured at Stage 0: {unique_entities} unique entities from {total_rows} total rows")
            ...
            # FIXED: Use new_unique_entities which correctly tracks new entities found in this stage
            if params.track_unique_entities and 'new_unique_entities' in locals():
                new_matches = new_unique_entities  # Use the correct unique entity count
                logger.info(f"Stage {params.stage_id}: Using new_unique_entities ({new_unique_entities}) for new_matches calculation")
            else:
                # Fallback to old logic if unique tracking is disabled
                ...
                logger.warning(f"Stage {params.stage_id}: Using fallback new_matches calculation ({new_matches}) - unique tracking disabled")
            ...
            # Calculate cumulative statistics - FIX: Track unique entities, not rows
            if params.is_baseline_stage:
                # Baseline stage doesn't contribute to cumulative matches
                cumulative_matched = 0
                cumulative_unique_matched = 0
                new_matches = 0
            else:
                cumulative_matched = matched_records
                cumulative_unique_matched = unique_matched if 'unique_matched' in locals() else matched_records
                ...
                if previous_stages:
                    ...
                    # FIX: Track cumulative unique entities correctly
                    cumulative_unique_matched = last_stage.get('cumulative_unique_matched', 0) + new_unique_entities
            ...
            stage_stats = {
                ...
                'cumulative_unique_matched': cumulative_unique_matched if 'cumulative_unique_matched' in locals() else cumulative_matched,  # FIX: Track unique cumulative
