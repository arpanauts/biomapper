...
class MergeDatasetsParams(BaseModel):
    ...
    dataset1_key: Optional[str] = Field(
        None, description="Alias for input_key (deprecated)"
    )
    ...
    def handle_dataset1_alias(cls, values):
        """Map dataset1_key to input_key for backward compatibility."""
        if 'dataset1_key' in values and 'input_key' not in values:
            values['input_key'] = values['dataset1_key']
        return values
    ...
    def validate_and_convert_params(self):
        ...
        if not self.dataset_keys:
            raise ValueError(
                "Either dataset_keys or input_key/dataset2_key must be provided"
            )
