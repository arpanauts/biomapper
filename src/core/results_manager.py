...
class ResultsPathManager:
    ...
    def format_version_folder(version: str) -> str:
        ...
        # Split on dash to handle alpha/beta/rc versions separately
        if '-' in version:
            main_version, suffix = version.split('-', 1)
            # Replace dots with underscores in main version only
            main_version_clean = main_version.replace('.', '_')
            version_clean = f"{main_version_clean}-{suffix}"
        else:
...
def setup_strategy_output_paths(
    ...
    # Set strategy_name in context if it wasn't there
    if 'strategy_name' not in context:
