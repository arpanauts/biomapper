...
class BaseMappingClient(abc.ABC):
    ...
    @staticmethod
    def format_result(
        target_ids: Optional[List[str]] = None, component_id: Optional[str] = None
...
class FileLookupClientMixin:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Additional arguments to pass to the parent class.
        """
        # Save key names BEFORE calling super().__init__ because validation happens there
    ...
    def get_required_config_keys(self) -> List[str]:
        ...
        parent_keys = (
            super().get_required_config_keys()
            if hasattr(super(), "get_required_config_keys")
            else []
        )
        # Use default key names if attributes not yet initialized
        file_path_key = getattr(self, '_file_path_key', 'file_path')
        key_column_key = getattr(self, '_key_column_key', 'key_column')
        value_column_key = getattr(self, '_value_column_key', 'value_column')
