...
from src.core.exceptions import ClientInitializationError, ClientExecutionError
...
class UniProtHistoricalResolverClient(CachedMappingClientMixin, BaseMappingClient):
        ...
        try:
            async with self.semaphore:
                async with aiohttp.ClientSession(timeout=request_timeout) as session:
                    logger.debug(
                        f"Querying UniProt REST API: {self.base_url} with params: {params}"
                    )
                    ...
                    async with session.get(self.base_url, params=params) as response:
                        if response.status == 200:
                            ...
                            logger.info(
                                f"UniProtClient DEBUG: Response for query [{query}]: {len(data.get('results', []))} results"
                            )
                            return data
                        else:
            ...
            # Re-raise ClientExecutionError without wrapping
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error during UniProt API query: {e}", exc_info=True
            )
            raise ClientExecutionError(
    ...
    async def _check_as_secondary_accessions(
        self, ids: List[str]
    ) -> Dict[str, List[str]]:
    ...
    async def _check_as_primary_accessions(self, ids: List[str]) -> Dict[str, bool]:
        ...
        if "results" in results:
            logger.info(
                f"UniProtClient DEBUG: Found {len(results['results'])} entries in API response"
            )
            for entry in results["results"]:
                primary_acc = entry.get("primaryAccession")
                if not primary_acc:
                    logger.debug(
                        f"UniProtClient DEBUG: Entry without primaryAccession: {entry}"
                    )
                    continue
                ...
                if primary_acc in primary_map:
                    ...
                    logger.info(
                        f"UniProtClient DEBUG: Found primary accession match: {primary_acc}"
                    )
                else:
                    logger.debug(
                        f"UniProtClient DEBUG: Primary accession {primary_acc} not in our input list"
                    )
        else:
            logger.warning(
                f"UniProtClient DEBUG: No 'results' key in API response: {results.keys()}"
            )

        logger.info(
            f"UniProtClient DEBUG: Primary accession check results: {primary_map}"
        )
        return primary_map
    ...
    async def _resolve_batch(self, batch_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for acc_id in batch_ids:
            ...
            if uniprot_pattern.match(cleaned_id):
                ...
                logger.debug(
                    f"ID {cleaned_id} failed validation - doesn't match UniProt format"
                )
                invalid_ids.append(acc_id)
        ...
        logger.info(
            f"UniProtClient DEBUG: _resolve_batch processing {len(valid_ids)} valid IDs: {valid_ids}"
        )

        # Track whether API calls failed to distinguish between "not found" and "API error"
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"UniProtClient DEBUG: Secondary accession check found: {len([k for k,v in secondary_map.items() if v])} IDs as secondary"
                )
            except ClientExecutionError as e:
                logger.warning(f"Secondary accession check failed: {e}. Using empty results.")
                secondary_map = {id: [] for id in valid_ids}
            ...
            logger.info(
                f"UniProtClient DEBUG: Checking {len(non_secondary_ids)} IDs as potential primary accessions: {non_secondary_ids}"
            )
            try:
                primary_map = await self._check_as_primary_accessions(non_secondary_ids)
            except ClientExecutionError as e:
                logger.warning(f"Primary accession check failed: {e}. Using empty results.")
                primary_map = {id: False for id in non_secondary_ids}
                api_error_occurred = True
                api_error_message = str(e)
            ...
            # If API errors occurred, mark unfound IDs as API errors rather than obsolete
            if api_error_occurred:
                for acc_id in valid_ids:
                    if not results[acc_id]["found"]:
                        # This ID wasn't found, but it might be due to API error rather than being obsolete
                        results[acc_id] = {
                            "found": False,
                            "is_primary": False,
                            "is_secondary": False,
                            "is_obsolete": False,  # Not obsolete, just couldn't check due to API error
                            "primary_ids": [],
                            "organism": None,
                            "gene_names": [],
                            "api_error": True,
            ...
            for acc_id, info in results.items():
                if info["found"]:
                    logger.info(
                        f"  {acc_id}: Found=True, Primary={info['is_primary']}, PrimaryIDs={info['primary_ids']}"
                    )
                elif info.get("api_error"):
                    logger.info(f"  {acc_id}: Found=False (API error)")
                else:
    ...
    def _preprocess_ids(self, identifiers: List[str]) -> List[str]:
        ...
        for identifier in identifiers:
            ...
            parts = [
                part.strip()
                for p_comma in identifier.split(",")
                for part in p_comma.split("_")
                if part.strip()
            ]
            for part in parts:
    ...
    async def map_identifiers(
        ...
        processed_results: Dict[str, Tuple[Optional[List[str]], Optional[str]]] = {
            identifier: (None, "error:not_processed")
        ...
        if not non_cached_ids:
            ...
            for orig_id in original_identifiers:
                ...
                if current_processed_ids_for_orig:
                    final_results[orig_id] = processed_results.get(
                        current_processed_ids_for_orig[0],
                        (None, "error:split_id_not_found"),
                    )
                else:
        ...
        logger.info(
            f"Resolving {len(non_cached_ids)} preprocessed UniProt IDs from API (total {len(processed_identifiers)} preprocessed, from {len(original_identifiers)} original requested)."
        )
        ...
        for i in range(0, len(non_cached_ids), batch_size):
            ...
            try:
                ...
                for identifier, raw_result_dict in batch_results_for_processed.items():
                    final_primary_ids: Optional[List[str]] = raw_result_dict.get(
                        "primary_ids"
                    )
                    ...
                    api_error = raw_result_dict.get("api_error", False)

                    if api_error:
                        # API error occurred, mark as batch processing failed
                        metadata_str = f"error:batch_processing_failed:{raw_result_dict.get('api_error_message', 'Unknown API error')}"
                        ...
                        if final_primary_ids and len(final_primary_ids) == 1:
                            ...
                            final_primary_ids and len(final_primary_ids) > 1
                        ):  # Secondary that demerged
                        ...
                        found
                    ):  # Not primary, not secondary, but _resolve_batch marked as found
                        if final_primary_ids and len(final_primary_ids) > 1:
                            metadata_str = "demerged"
                    ...
                    transformed_result: Tuple[Optional[List[str]], Optional[str]] = (
                        final_primary_ids,
                        metadata_str,
                    )
                ...
                for identifier in batch:
                    if (
                        identifier not in processed_results
                        or processed_results[identifier][1] == "error:not_processed"
                    ):
            ...
            if len(split_parts) == 1:
                ...
                for part_id in split_parts:
                    ...
                    if part_meta_str:
                        ...
                        collected_metadata_details.append(
                            f"{part_id}:error:missing_metadata"
                        )
                        has_error_in_parts = True

                final_agg_primary_ids: Optional[List[str]] = (
                    sorted(list(collected_primary_ids))
                    if collected_primary_ids
                    else None
                )

                # Construct the aggregated metadata string
                final_agg_metadata_str: str
                unique_metadata_strings = sorted(
                    list(set(collected_metadata_details))
                )  # Unique, sorted details from each part

                if has_error_in_parts:
                    final_agg_metadata_str = "composite:error_in_parts|" + "|".join(
                    ...
                    for part_id_check in split_parts:
                        ...
                        _, meta_check = processed_results.get(
                            part_id_check, (None, "")
                        )  # Default meta to empty if not found
                    ...
                    if all_strictly_obsolete:
                        ...
                        final_agg_metadata_str = (
                            "composite:no_primary_ids_found|"
                            + "|".join(unique_metadata_strings)
                        )
                else:  # Has primary IDs and no errors in parts
                    final_agg_metadata_str = "composite:resolved|" + "|".join(
                        unique_metadata_strings
                    )
        ...
        primary_count = (
            secondary_count
        ) = demerged_count = obsolete_count = error_count = 0
