...
class MockMappingExecutor:
    ...
    async def map_entity(self, relationship_id, source_entity, source_ontology=None, confidence_threshold=0.0):
    ...
    def _get_best_relationship_path(self, relationship_id, source_ontology):
        try:
            ...
            print(f"No mapping path found for {relationship_id}/{source_ontology}, creating one")
            ...
            self.cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (relationship_id, source_ontology, "chebi", path_id[0], 
                    0.9, 0, datetime.now().isoformat()))
            self.conn.commit()
...
async def test_relationship_mapping():
    ...
    try:
        ...
        print(f"Discovered {len(paths)} mapping paths for relationship {relationship_id}")
        for i, path in enumerate(paths):
            ...
            for j, step in enumerate(steps):
                print(f"  Step {j+1}: {step.get('source_type', step.get('from_type'))} → {step.get('target_type', step.get('to_type'))}")
        ...
        try:
            ...
            if count == 0:
                ...
                conn.execute("""
                    ...
                    (relationship_id, source_ontology, target_ontology, ontology_path_id, 
                     performance_score, usage_count, last_discovered)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (relationship_id, "hmdb", "chebi", 1, 0.9, 0, datetime.now().isoformat()))
                conn.commit()
            ...
            if results:
                ...
                for result in results:
                    print(f"  {result['target_id']} (confidence: {result['confidence']})")
            else:
