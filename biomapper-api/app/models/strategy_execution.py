...
class ExecutionOptions(BaseModel):
    ...
    checkpoint_enabled: bool = Field(
        default=True, description="Enable checkpointing for recovery"
    )
    ...
    progress_callback_url: Optional[str] = Field(
        default=None, description="URL to POST progress updates"
    )
    timeout_seconds: int = Field(
        default=3600, description="Maximum execution time in seconds"
    )
    retry_failed_steps: bool = Field(
        default=True, description="Automatically retry failed steps"
    )
    max_retries: int = Field(
        default=3, description="Maximum number of retries per step"
    )
    parallel_actions: bool = Field(
        default=False, description="Execute independent actions in parallel"
    )
    validate_prerequisites: bool = Field(
        default=True, description="Check prerequisites before execution"
    )
...
class StrategyExecutionRequest(BaseModel):
    ...
    options: ExecutionOptions = Field(
        default_factory=ExecutionOptions, description="Execution options"
    )
    tags: List[str] = Field(
        default_factory=list, description="Tags for job organization and filtering"
    )
    description: Optional[str] = Field(
        default=None, description="Optional job description"
    )
...
class StrategyExecutionResponse(BaseModel):
    ...
    strategy_name: Optional[str] = Field(
        default=None, description="Name of the strategy being executed"
    )
    ...
    sse_url: Optional[str] = Field(
        default=None, description="Server-Sent Events URL for updates"
    )
...
class Checkpoint(BaseModel):
    ...
    context_snapshot: Dict[str, Any] = Field(
        default_factory=dict, description="Snapshot of execution context"
    )
    can_resume: bool = Field(
        default=True, description="Whether execution can resume from this checkpoint"
    )
...
class CancelResponse(BaseModel):
    ...
    cleanup_performed: bool = Field(
        default=False, description="Whether cleanup operations were performed"
    )
...
class PrerequisiteCheck(BaseModel):
    ...
    category: str = Field(
        ..., description="Category: file, directory, service, credential"
    )
    passed: bool = Field(..., description="Whether the check passed")
    message: str = Field(..., description="Check result message")
    details: Optional[Dict[str, Any]] = Field(default=None)
    required: bool = Field(
        default=True, description="Whether this is a required prerequisite"
    )
...
class PrerequisiteReport(BaseModel):
    ...
    can_proceed: bool = Field(
        default=False, description="Whether execution can proceed"
    )
    recommendations: List[str] = Field(
        default_factory=list, description="Recommendations for fixing failed checks"
    )
...
class ActionInfo(BaseModel):
    ...
    produces_context: List[str] = Field(
        default_factory=list, description="Context keys produced"
    )
