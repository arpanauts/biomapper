...
class Job(Base):
    ...
    last_updated = Column(TIMESTAMP(timezone=True), nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    checkpoints = relationship("ExecutionCheckpoint", back_populates="job", cascade="all, delete-orphan")
    logs = relationship("ExecutionLog", back_populates="job", cascade="all, delete-orphan")
    results = relationship("ResultStorage", back_populates="job", cascade="all, delete-orphan")
...
class ExecutionCheckpoint(Base):
    ...
    job_id = Column(UUID(as_uuid=True), ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False)
    step_index = Column(Integer, nullable=False)
    checkpoint_type = Column(String(50), default="automatic")  # automatic, manual, before_step, after_step, error
    ...
    # Indexes
    __table_args__ = (
        Index("idx_checkpoints_job_step", "job_id", "step_index"),
    )
...
class ExecutionLog(Base):
    ...
    # Categorization
    category = Column(String(50), nullable=True)  # action, validation, error, performance
    component = Column(String(100), nullable=True)  # executor, action, checkpoint, storage
...
class ResultStorage(Base):
    ...
    # Storage options
    storage_type = Column(String(50), nullable=False)  # inline, s3, filesystem, compressed
    inline_data = Column(JSON, nullable=True)  # For small results
    external_path = Column(Text, nullable=True)  # For large results
    size_bytes = Column(BigInteger, nullable=False)
    
    # Metadata
    content_type = Column(String(100), nullable=True)  # application/json, text/csv, etc.
...
class JobEvent(Base):
    ...
    # Event details
    event_type = Column(String(50), nullable=False)  # progress, status_change, error, checkpoint, complete
