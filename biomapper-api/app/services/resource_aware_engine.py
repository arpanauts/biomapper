...
class ResourceAwareExecutionEngine(EnhancedExecutionEngine):
    ...
    async def execute_strategy(
        ...
        try:
            ...
            await progress_tracker.log_message(
                "INFO", "Checking resource requirements for strategy"
            )

            # Check resource requirements
            required_resources = await self.resource_manager.get_resource_requirements(
                strategy
            )
            ...
            if required_resources:
                ...
                for resource_name in required_resources:
                    ...
                    if resource.status == ResourceStatus.UNAVAILABLE:
                        ...
                        if config and config.auto_start:
                            ...
                            success = await self.resource_manager.start_resource(
                                resource_name
                            )
                            if not success:
                                unavailable_resources.append(resource_name)
                        ...
                        status = await self.resource_manager.check_resource(
                            resource_name
                        )
                ...
                if unavailable_resources:
                    ...
                    await progress_tracker.log_message("ERROR", error_msg)
                ...
                if degraded_resources:
                    ...
                    await progress_tracker.log_message("WARNING", warning_msg)

                # All resources are available
                await progress_tracker.log_message(
                    "INFO", "All required resources are available"
                )
            else:
                await progress_tracker.log_message(
                    "INFO", "Strategy has no external resource requirements"
                )
    ...
    async def pre_execution_checks(self, strategy: Dict[str, Any]) -> Dict[str, Any]:
        """
