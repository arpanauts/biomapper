...
from app.models.strategy_execution import JobStatus, StepInfo
        ...
        try:
            ...
            for index, step in enumerate(steps[start_index:], start=start_index):
                ...
                if not await self.evaluate_condition(step.get("condition"), context):
                    await progress_tracker.log_message(
                        "INFO", f"Skipping step {step_name} due to condition",
                        step_name
                    )
                    continue
                ...
                if step_result.get("status") == "failed" and step.get("is_required", True):
                ...
                # Log step completion
                await progress_tracker.log_message(
                    "INFO", f"Completed step {step_name}",
                    step_name, {"duration_ms": step_result.get("duration_ms")}
                )
    ...
    async def execute_step_with_retry(
        ...
        while retry_count <= max_retries:
            try:
                ...
                if retry_count > max_retries or not job.options.get("retry_failed_steps", True):
                    # Final failure
