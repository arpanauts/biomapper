...
class PrerequisiteChecker:
    ...
    async def _check_input_files(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
    ...
    async def _check_output_dirs(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
        ...
        default_output = strategy.get("config", {}).get("output_dir", "/tmp/biomapper/results")
        check = await self._check_directory_writable(
            default_output,
    ...
    async def _check_services(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
        ...
        if any("vector" in str(step).lower() or "qdrant" in str(step).lower() 
               for step in strategy.get("steps", [])):
    ...
    async def _check_credentials(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
        """Check that required API keys and credentials are configured."""
        checks = []
        
        # Check for OpenAI API key if LLM operations are used
        if any("llm" in str(step).lower() or "openai" in str(step).lower() 
               for step in strategy.get("steps", [])):
            check = PrerequisiteCheck(
                name="OpenAI API Key",
                category="credential",
                passed=bool(os.getenv("OPENAI_API_KEY")),
                message="OpenAI API key is configured" if os.getenv("OPENAI_API_KEY") 
    ...
    async def _check_system_resources(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
    ...
    async def _check_dependencies(self, strategy: Dict[str, Any]) -> List[PrerequisiteCheck]:
    ...
    async def _check_file_exists(
        ...
        if exists and readable:
            ...
            message = f"File exists but is not readable"
            passed = False
        else:
    ...
    async def _check_directory_writable(
        ...
        if not path.exists() and create_if_missing:
            ...
            if writable:
                message = f"Directory exists and is writable"
                passed = True
            else:
                message = f"Directory exists but is not writable"
                passed = False
        elif path.exists():
    ...
    async def _check_qdrant_connection(self) -> PrerequisiteCheck:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{qdrant_url}/collections", timeout=5) as response:
                    if response.status == 200:
                        data = await response.json()
                        collection_count = len(data.get("result", {}).get("collections", []))
                        message = f"Qdrant is accessible ({collection_count} collections)"
                        passed = True
                    else:
