...
class PersistenceService:
    ...
    async def update_job_status(
        self, job_id: uuid.UUID, status: JobStatus, **kwargs
    ) -> Job:
    ...
    async def record_step_start(
        ...
        if job:
            progress = (
                (step_index / job.total_steps * 100) if job.total_steps > 0 else 0
            )
        ...
        await self.db.commit()
        await self.log(
            job_id, "INFO", f"Step {step_name} started", step_index=step_index
        )
        await self.emit_event(
            job_id, "step_started", {"step_name": step_name, "step_index": step_index}
        )
    ...
    async def record_step_completion(
        ...
        stmt_select = select(ExecutionStep).where(
            ExecutionStep.job_id == job_id, ExecutionStep.step_index == step_index
        )
        ...
        if step and step.started_at:
            duration_ms = int(
                (datetime.utcnow() - step.started_at).total_seconds() * 1000
            )
        else:
            duration_ms = None
        ...
        if len(json.dumps(results)) >= settings.MAX_INLINE_STORAGE_SIZE:
            await self.store_result(job_id, step_index, "step_output", results)

        await self.db.commit()
        await self.log(
            job_id, "INFO", f"Step {step.step_name} completed", step_index=step_index
        )
        await self.emit_event(
            job_id,
            "step_completed",
    ...
    async def create_checkpoint(
        ...
        checkpoint = ExecutionCheckpoint(
            ...
            context_data=json.loads(serialized_context.decode())
            if not use_external and not compressed
            else None,
    ...
    async def retrieve_result(
        self, job_id: uuid.UUID, step_index: int, result_key: str
    ) -> Any:
    ...
    async def _serialize_context(self, context: StrategyExecutionContext) -> bytes:
        ...
        context_dict = {
            ...
            "step_results": {
                k: v.model_dump() for k, v in context.step_results.items()
            },
    ...
    async def _deserialize_context(self, data: bytes) -> StrategyExecutionContext:
    ...
    async def cleanup_old_data(self, days: int = 30):
        ...
        stmt = delete(ResultStorage).where(ResultStorage.expires_at < datetime.utcnow())
        await self.db.execute(stmt)
