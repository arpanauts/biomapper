...
class CSVService:
    ...
    async def save_file(self, file: UploadFile) -> Tuple[Session, Path]:
        ...
        logger.info(
            f"Created session {session.session_id} with directory {session_dir}"
        )
        ...
        try:
            ...
            logger.info(
                f"Saved file {file.filename} ({file_size} bytes) to {file_path}"
            )
            ...
            logger.exception(
                f"Error saving file for session {session.session_id}: {str(e)}"
            )
            raise HTTPException(
    ...
    async def get_columns(self, session: Session) -> List[str]:
        ...
        if not session.file_path:
            logger.error(
                f"No file path in session {session.session_id}. Session metadata: {session.metadata}"
            )
            raise HTTPException(
        ...
        if not session.file_path.exists():
            logger.error(
                f"File does not exist: {session.file_path} for session {session.session_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        try:
            ...
            logger.info(
                f"Successfully read {len(columns)} columns from {session.file_path}"
            )
            return columns
        except Exception as e:
    ...
    async def preview_data(self, session: Session, limit: int = 10) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Generating data preview for session {session.session_id}, limit={limit}"
        )
        ...
        try:
            ...
            # Read preview rows
            logger.info(
                f"Reading preview data ({limit} rows) for session {session.session_id}"
            )
            # Use load_tabular_file to properly handle comments
            ...
            logger.info(
                f"Successfully generated preview for session {session.session_id}: {len(result['columns'])} columns, {len(rows)} preview rows"
            )
            return result
            ...
            logger.exception(
                f"Error generating preview for session {session.session_id}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Error reading CSV file: {str(e)}",
    ...
    async def list_server_files(
        ...
        try:
            ...
            logger.exception(
                f"Error listing server files from {directory_path}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def load_server_file(self, file_path: str) -> Tuple[Session, Path]:
        ...
        try:
            ...
            logger.info(
                f"Created session {session.session_id} with symlink to file {file_path}"
            )
