...
class PersistentExecutionEngine:
    ...
    async def execute_strategy(
        ...
        try:
            ...
            if resume_from_checkpoint:
                checkpoint_data = await self.persistence.restore_checkpoint(
                    resume_from_checkpoint
                )
                context = checkpoint_data["context"]
                resume_from_step = checkpoint_data["step_index"] + 1
                logger.info(
                    f"Resuming job {job_id} from checkpoint at step {resume_from_step}"
                )
                ...
                if best_checkpoint:
                    ...
                    context = checkpoint_data["context"]
                    logger.info(
                        f"Loaded checkpoint at step {best_checkpoint.step_index}"
                    )
            ...
            if not context:
                ...
                context.custom_action_data["strategy_name"] = strategy.get(
                    "name", "unknown"
                )
                ...
                try:
                    ...
                    if output_size > 1024 * 100:  # > 100KB
                        ...
                        context.custom_action_data[
                            f"step_{i}_output_ref"
                        ] = f"stored:{job_id}:{i}:step_output"
                    ...
                    await self.persistence.record_step_failure(
                        job_id, i, str(e), error_traceback=traceback.format_exc()
                    )
                    ...
                    if retry_policy.get("action") == "retry":
                        ...
                        retry_count = context.custom_action_data.get(
                            f"step_{i}_retries", 0
                        )

                        if retry_count < max_retries:
                            # Increment retry count
                            context.custom_action_data[f"step_{i}_retries"] = (
                                retry_count + 1
                            )
                            ...
                            # Retry the step
                            logger.info(
                                f"Retrying step {i} (attempt {retry_count + 1}/{max_retries})"
                            )
            ...
            final_results = {
                ...
                "completed_steps": len(
                    [
                        k
                        for k in context.custom_action_data.keys()
    ...
    async def pause_job(self, job_id: uuid.UUID) -> bool:
        ...
        # Update status
        await self.persistence.update_job_status(job_id, JobStatus.PAUSED)
    ...
    async def resume_job(self, job_id: uuid.UUID) -> bool:
        ...
        task = asyncio.create_task(
            self.execute_strategy(
                job_id, job.strategy_config, resume_from_checkpoint=checkpoint.id
            )
        )
    ...
    async def cancel_job(self, job_id: uuid.UUID) -> bool:
        ...
        # Update status
        await self.persistence.update_job_status(
            job_id, JobStatus.CANCELLED, completed_at=datetime.utcnow()
        )
    ...
    def _find_best_checkpoint(
        self, checkpoints: List, target_step: int
    ) -> Optional[Any]:
    ...
    def _should_checkpoint_before(self, step: Dict, strategy: Dict) -> bool:
        ...
        critical_actions = checkpoint_policy.get(
            "before_actions",
            ["EXECUTE_MAPPING_PATH", "MERGE_DATASETS", "CALCULATE_THREE_WAY_OVERLAP"],
        )
    ...
    def _should_checkpoint_after(self, step: Dict, strategy: Dict) -> bool:
        ...
        important_actions = checkpoint_policy.get(
            "after_actions",
            [
