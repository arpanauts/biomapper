...
class ResourceManager:
    ...
    async def initialize(self):
        ...
        if DOCKER_AVAILABLE:
            ...
            logger.warning(
                "Docker library not installed - container management disabled"
            )
        ...
        logger.info(
            f"Resource manager initialized with {len(self.resources)} resources"
        )
    ...
    async def start_monitoring(self):
        ...
        for name, config in self.config.items():
            if config.health_check_interval > 0:
                task = asyncio.create_task(self._monitor_resource(name, config))
        ...
        for name, config in self.config.items():
            ...
            if status != ResourceStatus.HEALTHY:
                if config.auto_start:
                    ...
                    results[name] = False
                    logger.error(
                        f"Required resource {name} is not available and auto-start is disabled"
                    )
            else:
                results[name] = True
    ...
    async def _check_qdrant(self, config: ResourceConfig) -> ResourceStatus:
        ...
        try:
            ...
            logger.debug(
                f"Qdrant healthy - {len(collections.collections)} collections found"
            )
    ...
    async def start_qdrant(self, config: ResourceConfig) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Qdrant container failed to become healthy within 30 seconds"
                )
                return False
    ...
    async def _check_external_api(self, config: ResourceConfig) -> ResourceStatus:
        ...
        if not url:
            logger.warning("No URL configured for external API resource")
            return ResourceStatus.UNKNOWN
    ...
    async def start_docker_container(self, config: ResourceConfig) -> bool:
        ...
        try:
            ...
            try:
                ...
                container = self.docker_client.containers.run(
                    image, name=container_name, ports=ports, detach=True, remove=False
                )
    ...
    async def stop_resource(self, name: str) -> bool:
        ...
        try:
            if (
                config.type == ResourceType.DOCKER_CONTAINER
                or config.type == ResourceType.QDRANT
            ):
                container_name = config.config.get("container_name")
        ...
        while True:
            try:
                ...
                if status != resource.status:
                    ...
                    logger.info(
                        f"Resource {name} status changed: {old_status} -> {status}"
                    )
                    ...
                    if status == ResourceStatus.UNAVAILABLE and config.auto_start:
                        ...
                        if retry_count <= config.max_retries:
                            logger.warning(
                                f"Resource {name} unavailable, attempting restart (retry {retry_count}/{config.max_retries})"
                            )
                            success = await self.start_resource(name)
                            if success:
                                retry_count = 0
                            ...
                            logger.error(
                                f"Resource {name} failed after {config.max_retries} retries"
                            )
