...
class TestResourceManager:
    ...
    @pytest.mark.asyncio
    async def test_check_docker_container_running(self, resource_manager, mock_docker_client):
    ...
    @pytest.mark.asyncio
    async def test_check_docker_container_not_found(self, resource_manager, mock_docker_client):
    ...
    @pytest.mark.asyncio
    async def test_start_qdrant_existing_container(self, resource_manager, mock_docker_client):
    ...
    @pytest.mark.asyncio
    async def test_start_qdrant_create_container(self, resource_manager, mock_docker_client):
    ...
    async def test_ensure_required_resources_with_auto_start(self, resource_manager):
        ...
        # Mock methods
        resource_manager.check_resource = AsyncMock(return_value=ResourceStatus.UNAVAILABLE)
    ...
    async def test_monitor_resource_recovery(self, resource_manager):
        ...
        task = asyncio.create_task(
            resource_manager._monitor_resource("qdrant", config)
        )
    ...
    async def test_cleanup(self, resource_manager):
        ...
        resource_manager.health_check_tasks = {
            "task1": task1,
            "task2": task2
        }
...
class TestResourceAwareExecution:
    ...
    async def test_pre_execution_checks_all_ready(self):
        ...
        mock_manager.get_resource_requirements = AsyncMock(return_value=["qdrant", "postgres"])
