    ...
    if detailed_output:
        ...
        for result in detailed_output.results:
            ...
            if result.qdrant_similarity_score is not None:
                print(
                    f"  Qdrant Similarity Score: {result.qdrant_similarity_score:.4f}"
                )
                ...
                if result.target_ids:
                    ...
                    if "all_scores" in result.metadata:
                        print(
                            f"  All scores: {[f'{s:.3f}' for s in result.metadata['all_scores']]}"
                        )
                print(
                    f"  Score interpretation: {result.metadata.get('score_interpretation', 'N/A')}"
                )
            else:
