...
async def main():
    ...
    logger.info(f"Database initialized at {os.path.join(config.data_dir, config.db_name)}")
    ...
    for mapping in glucose_results:
        logger.info(f"  → {mapping['target_type']}:{mapping['target_id']} "
    ...
    for mapping in chebi_results:
        ...
        logger.info(f"  → {mapping_type}: {mapping['target_type']}:{mapping['target_id']} "
                    f"(confidence: {mapping['confidence']:.2f})")
