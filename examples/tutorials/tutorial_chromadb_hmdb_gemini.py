...
def parse_llm_response(
    response_text: str,
) -> tuple[str, str, float, str, str, float, str]:
...
"""
        ...
        try:
            ...
            similar_compounds_text = "\n".join(
                [
                    f"{i+1}. {compound.name} (HMDB ID: {compound.hmdb_id})"
            ...
            logger.info(
                f"GEMINI_API_KEY length: {len(os.getenv('GEMINI_API_KEY') or '')}"
            )

            # Step 1: Ask Gemini to find most similar compound
            candidates_brief = [
                f"{i+1}. {doc.name} (HMDB: {doc.hmdb_id})"
                for i, doc in enumerate(filtered_compounds)
            ]
...
Respond with just the number of the most similar compound, e.g. if compound #3 is most similar, respond with: 3"""
            ...
            api_prompt = {
                "model": "gemini-pro",
                "messages": [{"content": ranking_prompt}],
            }
...
Respond with just a percentage, e.g.: 85%"""
        ...
        if confidence >= 0.8:
            ...
            results_df.at[
                idx, "explanation"
            ] = f"Strong match with {confidence*100:.0f}% confidence based on chemical similarity"
        else:
            ...
            results_df.at[
                idx, "explanation"
            ] = f"No confident match found. Best guess has {confidence*100:.0f}% confidence"
